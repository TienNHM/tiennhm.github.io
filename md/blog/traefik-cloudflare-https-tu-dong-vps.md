...
> Nguồn: https://tiennhm.io.vn/blog/traefik-cloudflare-https-tu-dong-vps
> Một VPS, 14 container, 13 hostname, tất cả nằm sau Cloudflare và đều cần HTTPS tự động. Bài viết mổ kiến trúc Traefik v3 + Cloudflare đang chạy thật: vì sao ACME HTTP-01 cấp được cert lần đầu nhưng chắc chắn trượt lúc gia hạn, vì sao DNS-01 giải quyết được nhưng lại đẻ ra một ràng buộc mới, và vì sao thiếu router bắt-tất thì Cloudflare trả lỗi 526.
...
## Kiến trúc: một proxy, một network, label trên từng container
...
## Bẫy 1: SSL mode Flexible gây vòng lặp chuyển hướng
...
## Bẫy 2: HTTP-01 và cái vòng luẩn quẩn
ACME **HTTP-01** là kiểu xác thực mặc định mà hầu hết hướng dẫn Traefik dùng. Cách nó hoạt động: Let's Encrypt gọi `http:///.well-known/acme-challenge/` ở **cổng 80**, và server phải trả về đúng nội dung.
...
Dòng `resolvers` nhỏ nhưng đáng có. Traefik phải đợi bản ghi TXT lan truyền trước khi báo Let's Encrypt kiểm tra. Nếu nó hỏi resolver của host mà resolver đó vừa cache một câu trả lời NXDOMAIN cho `_acme-challenge.`, việc chờ sẽ kéo dài vô ích hoặc trượt hẳn. Hỏi thẳng `1.1.1.1` và `8.8.8.8` thì bỏ qua được lớp cache đó.
...
## Bẫy 3: DNS-01 đẻ ra một ràng buộc mới
...
## Bẫy 4: thiếu router bắt-tất thì Cloudflare trả 526
...
## Bẫy 5: mọi app đều thấy IP của Cloudflare
...
## Vận hành: những gì tôi quan sát được sau 9 ngày
...
**Danh sách kiểm khi đặt Traefik phía sau Cloudflare**

- [ ] Cloudflare SSL/TLS đặt Full (strict), tuyệt đối không Flexible
- [ ] Dùng DNS-01, không dùng HTTP-01, kể cả khi HTTP-01 đang chạy được
- [ ] Token Cloudflare: Zone Read + DNS Edit, giới hạn đúng zone và đúng IP VPS
- [ ] Khai resolvers 1.1.1.1 và 8.8.8.8 cho DNS challenge
- [ ] Đặt exposedbydefault=false để không route nhầm database hay adminer
- [ ] Thêm forwardedheaders.trustedips bằng dải IP Cloudflare
- [ ] Thêm router bắt-tất có priority=1 và tls.domains tường minh
- [ ] acme.json chmod 600 trước khi khởi động lần đầu
- [x] Mỗi zone mới cần token thấy được zone đó — kiểm tra log sau khi thêm domain

Mục cuối là mục tôi học được đắt nhất, và nó vẫn đang nằm trong log của tôi.

---

## Vài điều tôi cố tình chưa làm
...
- [2.3 — 1. Internet hoạt động như thế nào](https://tiennhm.io.vn/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-02-computer-science-basics/2.3-how-the-internet-works) — Theo chân một request từ lúc gõ URL tới lúc pixel hiện lên: DNS, TCP, TLS, HTTP.
