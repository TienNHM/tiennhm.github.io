...
Bài này đào sâu [bài Index](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/15-index) và [bài tối ưu truy vấn](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/28-sql-query-optimization) trong series học SQL 30 ngày. Mọi con số là kết quả chạy thật trên PostgreSQL 16.11.
...
## Những dạng làm mất sargability
...
**`CAST` ngầm** là dạng khó thấy nhất, vì bạn không tự viết ra nó. So sánh một cột số với một chuỗi, hoặc so `varchar` với `nvarchar`, có thể khiến database tự chèn phép chuyển kiểu lên **cột** và giết index. Đây là nguồn lỗi kinh điển trong các hệ thống có cột kiểu không nhất quán — phần chọn kiểu dữ liệu nằm ở [bài cấu trúc bảng](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/02-table-structure-and-data-types).
...
## Khi không viết lại được: index trên biểu thức
...
## Tác hại phụ ít người để ý: planner ước lượng sai
...
**Khi một truy vấn chậm dù đã có index**

- [ ] Chạy EXPLAIN ANALYZE, xem có Seq Scan trên bảng lớn không
- [ ] Tìm hàm hoặc phép tính bọc quanh cột trong WHERE
- [ ] Kiểm tra CAST ngầm do so sánh khác kiểu dữ liệu
- [ ] So rows ước lượng với actual rows — lệch nhiều là dấu hiệu mất sargability
- [ ] Với index phức hợp, kiểm tra điều kiện có phủ cột trái nhất không
- [ ] Chạy ANALYZE nếu thống kê đã cũ
- [x] Viết lại thành khoảng giá trị, hoặc tạo index trên biểu thức
...
|---|---|
| [Index](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/15-index) | Cấu trúc index, cách tạo và khi nào nên tạo |
| [Hiệu năng truy vấn](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/20-query-performance) | Đọc execution plan, nhận biết điểm nghẽn |
| [Tối ưu truy vấn SQL](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/28-sql-query-optimization) | Các kỹ thuật viết lại truy vấn |
| [Cấu trúc bảng và kiểu dữ liệu](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/02-table-structure-and-data-types) | Kiểu dữ liệu nhất quán để tránh CAST ngầm |
| [Thiết kế database](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/27-database-design-best-practices) | Cân bằng giữa đọc và ghi khi đặt index |

Ở phía ứng dụng, một dạng chậm khác hẳn nhưng hay bị nhầm với chuyện này là [vấn đề N+1 query trong EF Core](https://tiennhm.io.vn/blog/ef-core-n-plus-1-query): ở đó mỗi câu lệnh đều nhanh, cái chậm nằm ở **số lần** đi lại chứ không ở kế hoạch thực thi.

---

## Câu hỏi thường gặp

### Vì sao có index rồi mà truy vấn vẫn Seq Scan?
