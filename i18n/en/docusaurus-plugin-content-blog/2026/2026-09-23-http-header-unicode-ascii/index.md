---
title: "Can an HTTP Header Carry Vietnamese? ASCII, obs-text, and Where .NET Draws the Line"
...
# Can an HTTP header carry Vietnamese?
...
This post branches off [a production incident](/blog/cloudflare-header-broke-dotnet-httpclient) where the header `cf-ipcity: Hồ Chí Minh`, injected by Cloudflare, made `HttpClient` throw. There I stopped at the symptom. Here I answer the question that symptom raises: **what exactly is an HTTP header allowed to contain?**
...
## What a header value actually is, grammatically {#ngu-phap-field-value}
...
## Why `í` gets through and `ồ` does not {#i-lot-o-khong}
...
## What happens when two sides pick different encodings {#mojibake}
...
## Where .NET draws the line {#dotnet-vach-ranh-gioi}

In the [incident post](/blog/cloudflare-header-broke-dotnet-httpclient) I measured this specifically on .NET 9.0.4, and the results are worth repeating because they show .NET taking a clear position:
...
## Three correct ways to put diacritics in a header {#cach-ma-hoa-dung}
...
## Do HTTP/2 and HTTP/3 change any of this? {#http2-http3}
...
<FAQSection
  ...
  items={[
    ...
    {
      question: "Does switching to HTTP/2 or HTTP/3 fix non-ASCII header errors?",
      answer: "No. HTTP/2 compresses headers with HPACK and HTTP/3 uses QPACK, but both compress byte strings rather than redefining header values as Unicode text. The semantics still come from RFC 9110 and the obs-text constraint is unchanged, so upgrading the protocol version does not address the encoding problem."
    },
    {
      question: "Why does .NET throw instead of just sending the non-ASCII bytes?",
      answer: "Because both alternatives lead to silent data corruption. If .NET wrote Latin-1, characters such as ồ would turn into question marks with nobody noticing. If it wrote UTF-8, the receiver might decode it as Latin-1 and get mojibake. Throwing HttpRequestException at send time is the louder choice but the more honest one: you find out immediately, instead of discovering corrupted data weeks later in a report."
    }
  ]}
/>
...
Read on: [the production incident that led here](/blog/cloudflare-header-broke-dotnet-httpclient), and [how to forward headers safely in ASP.NET Core](/blog/forward-http-header-an-toan-aspnet-core).
