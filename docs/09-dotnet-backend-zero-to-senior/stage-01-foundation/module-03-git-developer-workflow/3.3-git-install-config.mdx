...
description: "Cấu hình một lần dùng cả đời: danh tính, xử lý ký tự xuống dòng giữa Windows và Linux, SSH key, .gitignore cho .NET, và vì sao đừng commit file .env."
sidebar_position: 3
---
...
<Checklist
  title="Danh sách rà soát cấu hình Git"
  items={[
    { text: "user.name và user.email đã đặt đúng, và đúng cho từng loại dự án." },
    { text: "Kho có file .gitattributes quy định ký tự xuống dòng cho cả đội." },
    { text: "Dùng SSH key ed25519 có passphrase, không dùng mật khẩu." },
    { text: ".gitignore đã chặn bin/, obj/, và mọi file cấu hình có bí mật." },
    { text: "Không có file bí mật nào đang được Git theo dõi (kiểm tra bằng git ls-files)." },
    { text: "Đã bật pull.rebase và fetch.prune." },
    { text: "Biết quy trình phải làm khi lỡ đẩy một bí mật lên." },
  ]}
/>

## Bài tập áp dụng

1. **Tìm bí mật đang bị theo dõi.** Chạy `git ls-files | grep -iE "\.env|secret|appsettings\..*\.json|\.pfx"` trong mọi kho bạn có. Với mỗi kết quả, trả lời: file này có chứa giá trị thật không?

2. **Tự tạo lỗi xuống dòng.** Trên một kho thử, đặt `core.autocrlf false`, tạo file có `CRLF`, commit. Đổi sang `input` rồi chạy `git diff`. Quan sát cách Git báo cáo. Sau đó thêm `.gitattributes` và làm lại.

3. **Diễn tập thu hồi.** Tạo một token thử trên một dịch vụ bất kỳ, commit nó vào kho riêng, push. Sau đó thực hiện đủ quy trình ở mục 3.3.5 — thu hồi trước, dọn lịch sử sau — và tính thời gian từ lúc phát hiện tới lúc token bị vô hiệu.
...
<FAQSection
  items={[
    {
      question: "Vì sao phải đặt user.name và user.email trước khi commit?",
      answer: "Vì hai giá trị này được ghi vĩnh viễn vào từng commit. Đặt sai thì toàn bộ lịch sử từ đó tới lúc phát hiện đều mang tên sai, và sửa lại đòi viết lại lịch sử — việc rất phiền khi đã đẩy lên kho chung."
    },
    {
      question: "core.autocrlf giải quyết vấn đề gì?",
      answer: "Windows kết thúc dòng bằng CRLF còn Linux và macOS dùng LF. Không xử lý thì mở một file do hệ điều hành khác tạo rồi lưu lại sẽ khiến Git báo toàn bộ file đã thay đổi dù không ai sửa gì. Trên Windows đặt true, trên Linux và macOS đặt input."
    },
    {
      question: "Vì sao nên dùng .gitattributes thay vì chỉ đặt core.autocrlf?",
      answer: "Vì core.autocrlf là cấu hình của từng máy, nên mỗi người trong đội có thể đặt khác nhau. File .gitattributes nằm trong kho và đi theo kho, nên mọi người hành xử giống nhau bất kể cấu hình cá nhân. Với dự án có cả Windows lẫn Linux thì gần như bắt buộc."
    },
    {
      question: "Thêm file vào .gitignore sau khi đã commit thì có tác dụng không?",
      answer: "Không. .gitignore chỉ có tác dụng với file chưa được Git theo dõi. File đã commit vẫn tiếp tục được theo dõi. Phải gỡ bằng git rm --cached để ngừng theo dõi mà vẫn giữ file trên đĩa, rồi commit thay đổi đó."
    },
    {
      question: "Lỡ đẩy một bí mật lên thì việc đầu tiên phải làm là gì?",
      answer: "Thu hồi bí mật đó và tạo cái mới — đổi mật khẩu database, huỷ token API. Không phải xoá commit. Lý do là giữa lúc push và lúc phát hiện, bí mật đã có thể được clone về máy khác, bị CI đọc, hoặc bị công cụ quét lập chỉ mục. Dọn lịch sử là việc làm sau và chỉ để tránh lộ thêm."
    },
    {
      question: "pull.rebase true có lợi gì?",
      answer: "Không có nó, mỗi lần git pull khi nhánh đã phân kỳ sẽ tạo một merge commit vô nghĩa kiểu Merge branch main of github.com. Bật rebase thì các commit của bạn được đặt lại lên trên phần mới lấy về, lịch sử thành một đường thẳng và dễ đọc hơn nhiều."
    }
  ]}
/>
