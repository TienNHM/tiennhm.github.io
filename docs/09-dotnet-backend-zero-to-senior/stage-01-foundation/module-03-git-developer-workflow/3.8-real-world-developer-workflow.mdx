---
title: "3.8 — 7. Developer Workflow thực tế"
...
```mermaid
flowchart LR
    A[Nhận việc] --> B[Đồng bộ main]
    B --> C[Tạo nhánh]
    C --> D[Làm + commit nhỏ]
    D --> E[Đồng bộ lại main]
    E --> F[Push + mở PR]
    F --> G[CI + review]
    G --> H[Merge + xoá nhánh]
```
...
git checkout -b feature/CRM-142-revenue-report

# 3. Làm việc, commit nhỏ và thường xuyên
git add -p && git commit -m "feat: add revenue query by branch"

# 4. Trước khi mở PR, đồng bộ lại
git fetch origin && git rebase origin/main

# 5. Đẩy lên và mở PR
git push -u origin feature/CRM-142-revenue-report

# 6. Sau khi merge, dọn
git checkout main && git pull
git branch -d feature/CRM-142-revenue-report
git fetch --prune
```
...
```mermaid
flowchart TD
    M[main — chỉ chứa bản đã phát hành] --> R[release/x.y]
    D[develop — tích hợp] --> R
    F1[feature/a] --> D
    F2[feature/b] --> D
    R --> M
    H[hotfix/z] --> M
...
```mermaid
flowchart LR
    F1[feature — sống 1 ngày] --> M[main]
    F2[feature — sống 1 ngày] --> M
    F3[feature — sống 1 ngày] --> M
    M --> P[Deploy tự động]
```
...
git checkout main && git pull
git checkout -b hotfix/negative-order-total

# 2. Sửa NHỎ NHẤT CÓ THỂ — chỉ chặn máu, không tái cấu trúc
git commit -am "fix: reject orders with negative total"

# 3. Vẫn mở PR, vẫn để CI chạy
git push -u origin hotfix/negative-order-total

# 4. Merge và deploy

# 5. Gộp ngược về nhánh phát triển nếu mô hình có nhánh đó
git checkout develop && git merge hotfix/negative-order-total
```
...
```text
origin/fix/order-total-hotfix     2 days ago     An
origin/feat/revenue-report        9 days ago     Bình
origin/feat/payment-integration   5 weeks ago    Cường
origin/refactor/split-service     3 months ago   Dũng
