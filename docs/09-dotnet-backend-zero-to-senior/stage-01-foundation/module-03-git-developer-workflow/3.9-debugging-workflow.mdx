---
title: "3.9 — 8. Debugging Workflow"
...
```mermaid
flowchart LR
    A[1 — Tái hiện<br/>ổn định] --> B[2 — Khoanh vùng]
    B --> C[3 — Tìm nguyên nhân gốc]
    C --> D[4 — Sửa]
    D --> E[5 — Viết test<br/>chống tái diễn]
```
...
Đây chính là `O(log n)` từ [bài 1.8](/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-01-programming-logic/1.7-data-structures-and-big-o), áp dụng vào lịch sử commit.
...
Đây là lý do **mỗi commit phải build được** như [bài 3.5](3.4-basic-git-workflow.mdx) đã nói. Một commit hỏng build nằm giữa dải tìm kiếm làm hỏng cả quy trình.
...
Lưu ý về `dotnet-counters`: nếu thấy bộ nhớ cao mà GC không giảm, đọc lại [bài 2.10](/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-02-computer-science-basics/2.9-dotnet-runtime-and-ecosystem) — Server GC giữ bộ nhớ là hành vi cố ý, không phải rò rỉ.
...
Mười nghìn commit chỉ cần **13 lần** kiểm tra. Đây chính là O(log n) mà [bài 1.8](../module-01-programming-logic/1.7-data-structures-and-big-o.mdx) mô tả, và cũng là kỹ thuật chia đôi ở [bài 1.7](../module-01-programming-logic/1.6-problem-solving-mindset.mdx) — chỉ khác là áp dụng lên lịch sử Git thay vì lên một hàm.
...
2. **Trả mã trong phản hồi lỗi.** Đặt vào trường `traceId` của `ProblemDetails` như [bài 8.9](../../stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.8-global-exception-handling.mdx) mô tả. Người dùng báo lỗi kèm mã, bạn tra ra ngay.
3. **Dùng chuẩn W3C Trace Context.** ASP.NET Core đã hỗ trợ sẵn qua `Activity.Current?.Id`, theo định dạng `00-<trace-id>-<span-id>-01`. Dùng chuẩn này thì mã tương quan tương thích với các hệ thống truy vết phân tán như OpenTelemetry và Jaeger, chủ đề của [Module 17](../../stage-05-senior-engineering/module-17-distributed-systems/).
...
- **Bài trước:** [3.8 — 7. Developer Workflow thực tế](3.8-real-world-developer-workflow.mdx)
- **Bài tiếp theo:** [3.10 — 9. Conventional Commits](3.10-conventional-commits.mdx)
