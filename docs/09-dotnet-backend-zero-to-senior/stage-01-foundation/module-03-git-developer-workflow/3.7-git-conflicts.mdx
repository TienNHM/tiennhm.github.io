...
```mermaid
flowchart TD
    A[git merge] --> B{Hai nhánh có sửa<br/>cùng vùng của cùng file?}
    B -->|Không| C[Merge tự động thành công]
    B -->|Có| D[XUNG ĐỘT<br/>Git dừng lại và hỏi bạn]
```
...
echo "TỪ NHÁNH B" > f.txt && git commit -q -am "edit from B"

git checkout -q main
echo "TỪ MAIN" > f.txt && git commit -q -am "edit from main"
...
| Sau `>>>>>>>` | `branch-b` | Tên nhánh, để bạn biết phần dưới đến từ đâu |
...
**Trong `merge`** (đang ở `main`, gộp `branch-b` vào):

```bash
git checkout main
git merge branch-b            # xung đột
...
**Trong `rebase`** (đang ở `branch-b`, rebase lên `main`):
...
|---|---|---|
| `git merge branch-b` (đang ở `main`) | `main` | `branch-b` |
| `git rebase main` (đang ở `branch-b`) | **`main`** | **`branch-b`** |
...
{
    public decimal CalculateRevenue(int customerId) { /* ... */ }
}
```
...
```csharp
public decimal CalculateAnnualRevenue(int customerId) { /* ... */ }
```
...
{
    public void ExportReport(int id) => _customerService.CalculateRevenue(id);
}
```
...
git merge branch-b     # OK, KHÔNG có xung đột
```
...
```bash
dotnet build
# error CS1061: 'CustomerService' does not contain a definition for 'CalculateRevenue'
```
