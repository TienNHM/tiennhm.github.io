---
title: "3.2 — 2. Cài đặt và cấu hình Git"
...
git commit -m "chore: untrack local development config"
```
...
```mermaid
flowchart TD
    A[Lỡ commit một bí mật] --> B{Đã push chưa?}
    B -->|Chưa| C[git reset hoặc amend<br/>rồi commit lại]
    B -->|Rồi| D[COI NHƯ ĐÃ LỘ<br/>THU HỒI bí mật ngay]
    D --> E[Tạo khoá mới]
    E --> F[Dọn lịch sử — tuỳ chọn]
```
...
git commit -m "chore: untrack config file that leaked secrets"
```
...
git commit -m "chore: untrack production config"
git push
```
