---
title: "2.9 — 8. .NET Runtime và Ecosystem"
description: "Từ mã C# tới mã máy: IL, JIT, AOT. Cách GC hoạt động và vì sao nó ngốn 96% RAM container dù app đang rảnh. Kèm lịch phát hành LTS và cách chọn thư viện NuGet."
sidebar_position: 9
---
...
Bài [Vì sao .NET ngốn 96% RAM container dù app đang rảnh?](/blog/dotnet-workstation-gc-giam-ram-container) đi vào chi tiết và đo tác động của việc chuyển sang Workstation GC:
...
<Checklist
  title="Danh sách rà soát runtime và hệ sinh thái"
  items={[
    { text: "Phiên bản .NET đang dùng vẫn còn trong thời hạn hỗ trợ." },
    { text: "Dự án chạy trong container đã cân nhắc Workstation GC so với Server GC, có đo trước khi chọn." },
    { text: "Không có cache tĩnh nào phình vô hạn, và mọi đăng ký sự kiện đều được gỡ." },
    { text: "Máy chủ production chỉ cài runtime, không cài SDK." },
    { text: "Nếu dùng self-contained, có quy trình build lại khi runtime có bản vá bảo mật." },
    { text: "Mọi gói NuGet trong dự án đều còn được bảo trì và tương thích phiên bản đang dùng." },
    { text: "Đã đo thời gian khởi động nguội, và cân nhắc AOT nếu nó là vấn đề." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Mã C# đi qua những bước nào trước khi thành mã máy?",
      answer: "Roslyn biên dịch C# thành IL và đóng vào file .dll; IL không phụ thuộc CPU hay hệ điều hành. Lúc chạy, JIT dịch từng phương thức sang mã máy ngay trước lần gọi đầu tiên rồi giữ lại kết quả. Vì vậy lần gọi đầu luôn chậm hơn, và cùng một .dll chạy được trên nhiều nền tảng."
    },
    {
      question: "Vì sao đối tượng sống ngắn lại rẻ?",
      answer: "Vì GC chia bộ nhớ thành ba thế hệ dựa trên quan sát rằng phần lớn đối tượng chết rất trẻ. Gen 0 được dọn rất thường xuyên và rất rẻ. Thứ đắt là Gen 2, nơi chứa đối tượng sống lâu. Nên tạo nhiều object nhỏ trong một request rồi vứt đi là bình thường; thứ gây đau là cache tĩnh phình mãi hoặc sự kiện quên gỡ đăng ký."
    },
    {
      question: "Vì sao container .NET hay hiển thị mức RAM rất cao?",
      answer: "Vì ASP.NET Core mặc định dùng Server GC, chế độ này tạo một heap cho mỗi CPU và cố tình không trả bộ nhớ lại cho hệ điều hành ngay, do trả rồi xin lại rất tốn. Biểu đồ RAM vì thế leo sát giới hạn rồi nằm yên. Đó không phải rò rỉ, nhưng vẫn có thể khiến orchestrator giết container."
    },
    {
      question: "Khi nào nên chuyển sang Workstation GC?",
      answer: "Khi chạy trong container bị giới hạn RAM chặt và mức bộ nhớ đang là vấn đề thật. Đây là đánh đổi có thật: bớt bộ nhớ nhưng giảm thông lượng, vì chỉ còn một heap thay vì một heap cho mỗi CPU. Nên đo cả hai chỉ số trước khi quyết định."
    },
    {
      question: "Framework-dependent và self-contained khác nhau thế nào?",
      answer: "Framework-dependent cho gói nhỏ vài MB nhưng máy đích phải cài sẵn .NET runtime. Self-contained đóng gói cả runtime nên chạy ở đâu cũng được, đổi lại nặng hơn 60 MB và quan trọng hơn: mỗi lỗ hổng bảo mật trong runtime đều bắt bạn build lại và deploy lại ứng dụng."
    },
    {
      question: "Nên chọn phiên bản .NET nào?",
      answer: "Với hệ thống nghiệp vụ chạy nhiều năm thì chọn bản LTS, tức các số chẵn, được hỗ trợ 3 năm. Bản STS số lẻ chỉ được hỗ trợ 18 tháng. Chạy phiên bản đã hết hỗ trợ nghĩa là không còn nhận bản vá bảo mật, nên đó là rủi ro chứ không chỉ là bất tiện."
    }
  ]}
/>
