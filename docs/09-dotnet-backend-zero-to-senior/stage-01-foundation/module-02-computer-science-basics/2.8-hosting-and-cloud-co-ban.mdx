---
title: "2.8 — 7. Hosting và Cloud cơ bản"
description: "Từ máy chủ vật lý tới serverless: bạn đang đổi quyền kiểm soát lấy sự tiện lợi. Kèm vai trò của reverse proxy, vì sao chọn base image sai làm chết ứng dụng, và điều kiện để mở rộng ngang."
sidebar_position: 8
---
...
Hệ quả cần nhớ: vì proxy đứng giữa, **ứng dụng không còn thấy IP thật của người dùng**. Nó thấy IP của proxy. Thông tin gốc nằm trong các header `X-Forwarded-For`, `X-Forwarded-Proto`. Cấu hình `ForwardedHeaders` cho đúng là việc bắt buộc — và làm sai thì thành lỗ hổng, vì header có thể bị giả mạo. Bài [Forward header trong ASP.NET Core](/blog/forward-http-header-an-toan-aspnet-core) phân tích vì sao "forward hết" là một lỗi kiến trúc.

Chứng chỉ tự động cũng là chỗ dễ hỏng âm thầm: bài [Traefik + Cloudflare: vì sao cert hết hạn đồng loạt sau 60 ngày](/blog/traefik-cloudflare-https-tu-dong-vps) là một sự cố thật, nơi mọi thứ trông vẫn bình thường cho tới lúc không còn bình thường nữa.
...
<Checklist
  title="Danh sách rà soát hosting"
  items={[
    { text: "Không có bí mật nào nằm trong mã nguồn hay trong file đã commit." },
    { text: "Cùng một image chạy được ở mọi môi trường, chỉ khác biến truyền vào." },
    { text: "Ứng dụng không ghi file lên đĩa cục bộ để dùng lại về sau." },
    { text: "Session và cache nằm ở nơi dùng chung, không nằm trong bộ nhớ tiến trình." },
    { text: "Đã cấu hình ForwardedHeaders và giới hạn proxy tin cậy." },
    { text: "Có endpoint health check tách riêng liveness và readiness." },
    { text: "Base image đã được xác nhận chạy được với mọi thư viện native mà dự án dùng." },
    { text: "Chứng chỉ TLS gia hạn tự động, và có cảnh báo khi sắp hết hạn." },
  ]}
/>
...
```csharp
app.MapPost("/luu", async (HttpContext ctx) =>
{
    await File.WriteAllTextAsync("/tmp/ghi-chu.txt", "xin chào");
    return Results.Ok(Environment.MachineName);
});

app.MapGet("/doc", (HttpContext ctx) =>
    File.Exists("/tmp/ghi-chu.txt")
        ? Results.Ok(new { may = Environment.MachineName, noi_dung = File.ReadAllText("/tmp/ghi-chu.txt") })
        : Results.NotFound(new { may = Environment.MachineName }));
```

Gọi:

```bash
curl -X POST localhost:8080/luu     # -> api1
for i in $(seq 1 6); do curl -s localhost:8080/doc; echo; done
```
...
```text
{"may":"api2"}                                 <- 404
{"may":"api1","noi_dung":"xin chào"}           <- 200
{"may":"api2"}                                 <- 404
{"may":"api1","noi_dung":"xin chào"}           <- 200
```
...
app.MapPost("/luu", async (IBlobStorage blob, CancellationToken ct) =>
{
    await blob.LuuAsync("ghi-chu.txt", "xin chào", ct);
    return Results.Ok();
});
```
...
<FAQSection
  items={[
    {
      question: "Reverse proxy làm những việc gì?",
      answer: "Ba việc chính: kết thúc TLS để chứng chỉ chỉ nằm ở một chỗ và ứng dụng chỉ nói HTTP nội bộ; định tuyến nhiều tên miền về nhiều dịch vụ trên cùng một IP; và cân bằng tải giữa các instance, đồng thời ngừng gửi request tới instance đã chết."
    },
    {
      question: "Vì sao ứng dụng phía sau proxy không thấy IP thật của người dùng?",
      answer: "Vì kết nối tới ứng dụng là do proxy mở, nên ứng dụng thấy IP của proxy. IP gốc nằm trong header X-Forwarded-For. Phải cấu hình ForwardedHeaders để ASP.NET Core đọc header đó, nhưng chỉ tin những proxy đã khai báo — vì header này có thể bị giả mạo nếu tin tất cả."
    },
    {
      question: "Image và container khác nhau thế nào?",
      answer: "Image là bản đóng gói bất biến gồm lớp hệ điều hành, runtime và ứng dụng. Container là một tiến trình đang chạy từ image đó. Quan hệ giống class và instance: một image sinh ra bao nhiêu container cũng được."
    },
    {
      question: "Vì sao chọn base image lại quan trọng đến thế?",
      answer: "Vì nó quyết định thư viện C chuẩn mà mọi thư viện native sẽ liên kết tới. Bản Debian dùng glibc, bản Alpine dùng musl. Rất nhiều thư viện native chỉ phát hành bản dựng sẵn cho glibc, nên chọn Alpine thì build vẫn thành công, container vẫn khởi động, và chỉ chết đúng lúc nạp thư viện đó lúc chạy."
    },
    {
      question: "Điều kiện để mở rộng ngang là gì?",
      answer: "Ứng dụng phải stateless, nghĩa là request đi vào instance nào cũng xử lý được như nhau. Ba thứ phá vỡ điều đó: ghi file lên đĩa cục bộ, giữ session trong bộ nhớ, và cache trong bộ nhớ tiến trình. Cách xử lý lần lượt là object storage, Redis hoặc token, và cache phân tán."
    },
    {
      question: "Liveness và readiness khác nhau ra sao?",
      answer: "Liveness hỏi tiến trình còn sống không; hỏng thì khởi động lại. Readiness hỏi đã sẵn sàng nhận request chưa, ví dụ đã kết nối được database; chưa sẵn sàng thì tạm ngừng gửi request tới nhưng không khởi động lại. Nhầm hai cái này gây vòng lặp khởi động lại liên tục khi database chỉ chậm một chút."
    }
  ]}
/>
...
- [ONNX Runtime chết trên Alpine: lỗi ld-linux-x86-64.so.2 và vì sao libc6-compat không cứu được](/blog/onnx-runtime-alpine-ld-linux-x86-64-so-2) — Container Node.js build xong, chạy lên, rồi chết ngay khi nạp model: Error loading shared library ld-linux-x86-64.so.2.
