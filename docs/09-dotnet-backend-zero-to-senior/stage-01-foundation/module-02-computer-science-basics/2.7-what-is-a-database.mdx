...
description: "Vì sao không lưu dữ liệu vào file: bốn thứ một DBMS làm mà file không làm được. Kèm ACID, ràng buộc nên đặt ở đâu, chọn giữa quan hệ và NoSQL, và vì sao connection là tài nguyên khan hiếm."
sidebar_position: 7
---
...
Chi tiết từng mức: [Transactions và ACID](/docs/database/learn-sql-in-30-days/16-transactions-acid).
...
Thiết kế lược đồ đi sâu ở bài [database design best practices](/docs/database/learn-sql-in-30-days/27-database-design-best-practices).
...
Cái giá: index tốn dung lượng, và **làm chậm mọi lệnh ghi** vì mỗi `INSERT`, `UPDATE`, `DELETE` phải cập nhật thêm cấu trúc đó. Bài [Thêm 4 index làm INSERT chậm 6 lần](/blog/chi-phi-ghi-cua-index) đo con số cụ thể trên PostgreSQL 16.

Và có index chưa chắc đã được dùng: bọc một hàm quanh cột trong `WHERE` là index bị vô hiệu — xem [Sargability](/blog/sql-index-khong-duoc-dung-sargable).

Đây cùng là một ý tưởng với [Big-O ở bài 1.8](/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-01-programming-logic/1.8-data-structures-and-big-o), chỉ khác là áp dụng lên ổ đĩa.
...
1. **Quên giải phóng.** Một `DbContext` không được dispose giữ kết nối mãi. Dùng dependency injection với vòng đời `Scoped` là đúng; giữ `DbContext` ở `Singleton` thì hỏng — chủ đề của [bài về captive dependency](/blog/singleton-scoped-transient-captive-dependency).
...
<Checklist
  title="Danh sách rà soát cơ sở dữ liệu"
  items={[
    { text: "Ràng buộc quan trọng (NOT NULL, UNIQUE, FOREIGN KEY, CHECK) có ở cả database, không chỉ ở code." },
    { text: "Mọi thao tác nhiều bước liên quan tiền bạc đều nằm trong một giao dịch." },
    { text: "Không có lời gọi HTTP hay đọc file nào nằm bên trong một giao dịch đang mở." },
    { text: "DbContext đăng ký ở vòng đời Scoped, không phải Singleton." },
    { text: "Cột dùng để lọc và JOIN đã có index, và số lượng index đã được cân nhắc với chi phí ghi." },
    { text: "Mốc thời gian lưu ở UTC với kiểu timestamptz." },
    { text: "Đã biết max_connections của database và kích thước pool của ứng dụng." },
  ]}
/>
...
```sql
-- Chuẩn bị
CREATE TABLE tai_khoan (id INT PRIMARY KEY, so_du NUMERIC(18,2));
INSERT INTO tai_khoan VALUES (1, 1000);
```

| Bước | Phiên A | Phiên B |
|---|---|---|
| 1 | `BEGIN;` | |
| 2 | `SELECT so_du FROM tai_khoan WHERE id=1;` → 1000 | |
| 3 | | `BEGIN;` |
| 4 | | `SELECT so_du FROM tai_khoan WHERE id=1;` → 1000 |
| 5 | `UPDATE tai_khoan SET so_du = 1000 + 500 WHERE id=1;` | |
| 6 | `COMMIT;` → còn 1500 | |
| 7 | | `UPDATE tai_khoan SET so_du = 1000 + 300 WHERE id=1;` |
| 8 | | `COMMIT;` → còn **1300** |
...
BEGIN;
SELECT so_du FROM tai_khoan WHERE id=1 FOR UPDATE;   -- phiên B phải đợi
UPDATE tai_khoan SET so_du = so_du + 300 WHERE id=1;
COMMIT;
...
UPDATE tai_khoan SET so_du = 1300, version = version + 1
...
**Liên hệ về sau.** Cách 3 chính là cơ chế mà EF Core gọi là concurrency token, cài bằng `IsRowVersion()`. [Bài 13.9](../../stage-04-database-production/module-13-entity-framework-core/13.9-concurrency-control.mdx) trình bày đầy đủ, còn [bài 12.7](../../stage-04-database-production/module-12-sql-deep-dive/12.7-transactions-and-locking.mdx) đi sâu vào các mức cô lập và deadlock.
...
CREATE TABLE tai_khoan (
    id INT PRIMARY KEY,
    so_du NUMERIC(18,2) CHECK (so_du >= 0)
);
INSERT INTO tai_khoan VALUES (1, 1000), (2, 500);

BEGIN;
UPDATE tai_khoan SET so_du = so_du - 800 WHERE id = 1;   -- OK, còn 200
UPDATE tai_khoan SET so_du = so_du - 800 WHERE id = 2;   -- VI PHẠM: -300
COMMIT;
```
...
```text
ERROR:  new row for relation "tai_khoan" violates check constraint "tai_khoan_so_du_check"
```
...
```sql
UPDATE tai_khoan SET so_du = so_du - 800 WHERE id = 1;   -- commit ngay, còn 200
UPDATE tai_khoan SET so_du = so_du - 800 WHERE id = 2;   -- lỗi, không ảnh hưởng câu trên
SELECT * FROM tai_khoan;
-- id | so_du
...
1. **Truy vấn chậm** — thiếu chỉ mục, hoặc lỗi N+1 như [bài 1.4](../module-01-programming-logic/1.4-loops.mdx) đã nêu.
2. **Gọi API bên ngoài khi đang mở giao dịch** — kết nối bị giữ suốt thời gian chờ mạng.
3. **Quên giải phóng `DbContext`** — thường do đăng ký sai vòng đời trong DI, chủ đề của [bài 7.5](../../stage-02-csharp-professional/module-07-dependency-injection/7.5-service-lifetimes.mdx).
...
<FAQSection
  items={[
    {
      question: "Bốn việc DBMS làm mà file không làm được là gì?",
      answer: "Cho nhiều người ghi cùng lúc mà không đè mất thay đổi của nhau; bảo đảm dữ liệu còn nguyên vẹn sau khi mất điện giữa lúc ghi; trả lời câu hỏi phức tạp mà không phải đọc toàn bộ dữ liệu; và từ chối dữ liệu sai ngay tại nguồn bằng ràng buộc."
    },
    {
      question: "Atomicity nghĩa là gì trong thực tế?",
      answer: "Nghĩa là một giao dịch hoặc hoàn tất trọn vẹn, hoặc không để lại dấu vết nào. Chuyển tiền gồm trừ tài khoản A và cộng tài khoản B; nếu hỏng giữa chừng mà không có atomicity thì tiền bốc hơi. Database bảo đảm mất điện trước COMMIT thì cả hai câu lệnh đều bị quay lui."
    },
    {
      question: "Nên đặt ràng buộc ở database hay ở tầng ứng dụng?",
      answer: "Cả hai, nhưng database là nơi nói lời cuối. Lý do: ứng dụng đến rồi đi, còn dữ liệu ở lại. Trong vòng đời hệ thống, dữ liệu sẽ bị ghi bởi ứng dụng chính, script di trú, job nền, truy vấn sửa tay lúc nửa đêm và cả ứng dụng viết lại sau nhiều năm. Chỉ database là chốt chặn mà tất cả đều phải đi qua."
    },
    {
      question: "Khi nào chọn NoSQL thay vì cơ sở dữ liệu quan hệ?",
    ...
    {
      question: "Điều gì xảy ra khi connection pool cạn?",
      answer: "Request mới phải xếp hàng đợi một kết nối được trả lại, rồi hết giờ chờ. Triệu chứng là toàn hệ thống chậm dần mà không có lỗi rõ ràng, nên rất khó chẩn đoán. Hai nguyên nhân phổ biến là quên giải phóng DbContext, và giữ kết nối quá lâu vì gọi API bên ngoài trong lúc đang mở giao dịch."
    },
    {
      question: "Vì sao không nên gọi API bên ngoài trong một giao dịch đang mở?",
      answer: "Vì giao dịch giữ kết nối database và thường giữ cả khoá trên các dòng liên quan. Một lời gọi HTTP mất vài giây sẽ giữ tài nguyên đó suốt thời gian ấy, làm cạn pool và chặn các giao dịch khác. Cách đúng là lấy dữ liệu bên ngoài trước, rồi mới mở giao dịch để ghi."
    }
  ]}
/>
...
- [Học SQL trong 30 ngày](/docs/database/learn-sql-in-30-days) — series đi từ `SELECT` tới tối ưu
- [Entity Framework Core](https://learn.microsoft.com/en-us/ef/core/) — ORM chính thức của .NET
- [Transactions và ACID](/docs/database/learn-sql-in-30-days/16-transactions-acid)
- [Database design best practices](/docs/database/learn-sql-in-30-days/27-database-design-best-practices)
