...
Đây **cùng một nguyên nhân** với sai số `double` ở [bài 1.2](../module-01-programming-logic/1.2-variables-and-data-types.mdx) — chỉ khác là ở đó nó xuất hiện khi cộng số thập phân, còn ở đây nó xuất hiện khi truyền số nguyên lớn.
...
2. **Dùng `Guid` làm khoá công khai.** Vốn đã là chuỗi trong JSON nên không có vấn đề. Đánh đổi là chỉ mục lớn hơn và ghi chậm hơn — [bài 12.2](../../stage-04-database-production/module-12-sql-deep-dive/12.2-crm-database-design.mdx) phân tích đánh đổi này.
...
```ts
function layThongBaoLoi(res: any): string {
...
```ts
const layThongBaoLoi = (p: ProblemDetails) => p.detail ?? p.title;
```
...
Từ .NET 8, `AddProblemDetails` tự động chuyển mọi phản hồi lỗi chưa có thân sang định dạng này. [Bài 8.9](../../stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.9-global-exception-handling.mdx) trình bày cách nối nó với xử lý ngoại lệ toàn cục.
...
{
    public bool DaGui { get; }
    public T? GiaTri { get; }
    public Optional(T? giaTri) { DaGui = true; GiaTri = giaTri; }
}
...
```csharp
if (dto.Phone.DaGui)
    customer.DatSoDienThoai(dto.Phone.GiaTri);   // null nghĩa là xoá
