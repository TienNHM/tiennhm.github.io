---
title: "2.3 — 2. Client - Server Model"
description: "Ai gọi, ai trả lời, và vì sao HTTP không nhớ gì giữa hai request. Kèm nguyên tắc quan trọng nhất của backend: mọi thứ đến từ client đều là dữ liệu do người lạ gửi."
sidebar_position: 3
---
...
sequenceDiagram
    ...
    C->>S: Tôi cần danh sách khách hàng
    S-->>C: 200 OK + JSON
    Note over S: Server KHÔNG thể tự gửi gì<br/>khi chưa được hỏi
    C->>S: Có gì mới không?
    S-->>C: 204 No Content
```
...
Lỗ hổng sinh ra từ việc tin `id` mà client gửi lên có tên riêng: **IDOR**. Bài [Đổi id trên URL ra dữ liệu người khác](/blog/idor-broken-access-control-aspnet-core) mổ xẻ nó và chỉ cách chặn ở một tầng duy nhất. Đây cũng là hạng mục đứng đầu [OWASP Top 10](https://owasp.org/Top10/).
...
<Checklist
  title="Danh sách rà soát client - server"
  items={[
    { text: "Mọi endpoint đều tự kiểm tra quyền, không dựa vào việc giao diện đã ẩn nút." },
    { text: "userId lấy từ token đã xác thực, không lấy từ body hay query string." },
    { text: "Giá tiền và số tiền luôn tính lại ở máy chủ, không nhận từ client." },
    { text: "Mọi kiểm tra ở form phía client đều có bản kiểm tra tương ứng ở máy chủ." },
    { text: "API chưa từng xoá hay đổi tên trường mà không đánh phiên bản mới." },
    { text: "Tính năng thời gian thực dùng SignalR hoặc SSE thay vì polling dày đặc." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao máy chủ không tự gửi dữ liệu xuống client được?",
      answer: "Vì trong HTTP, client luôn là bên mở kết nối và đặt câu hỏi; máy chủ chỉ trả lời. Muốn đẩy dữ liệu theo thời gian thực thì phải dùng polling, Server-Sent Events hoặc WebSocket. Trong .NET, SignalR gói cả ba lại và tự chọn cách tốt nhất mà môi trường cho phép."
    },
    {
      question: "HTTP là stateless nghĩa là gì?",
      answer: "Nghĩa là mỗi request độc lập hoàn toàn với request trước, máy chủ không tự nhớ bạn là ai. Vì thế request nào cần xác thực đều phải tự mang theo bằng chứng, thường là cookie hoặc JWT trong header Authorization. Đổi lại, máy chủ không giữ trạng thái nên thêm bao nhiêu máy chủ cũng được — đó là nền tảng của mở rộng ngang."
    },
    {
      question: "Kiểm tra ở phía client có phải là bảo mật không?",
      answer: "Không. Nó chỉ là trải nghiệm người dùng. Người dùng có thể mở DevTools sửa biến, hoặc bỏ qua giao diện hoàn toàn và gọi thẳng API bằng curl. Mọi kiểm tra quyền và kiểm tra dữ liệu phải được làm lại ở máy chủ, kể cả khi client là do chính bạn viết."
    },
    {
      question: "Những dữ liệu nào từ client không bao giờ được tin?",
      answer: "Giá tiền gửi lên từ giỏ hàng, userId trong body request, kết quả kiểm tra form phía client, và việc một nút đã bị ẩn trên giao diện. Giá phải tính lại từ database, userId phải lấy từ token đã xác thực, kiểm tra phải làm lại ở máy chủ, và mọi endpoint phải tự kiểm tra quyền."
    },
    {
      question: "Thay đổi API nào là phá vỡ tương thích?",
      answer: "Xoá hoặc đổi tên trường đang có, đổi kiểu dữ liệu của trường, thêm tham số bắt buộc, và siết chặt kiểm tra đầu vào. Ngược lại, thêm trường mới vào response, thêm endpoint mới, thêm tham số tuỳ chọn và nới lỏng kiểm tra đều an toàn. Khi buộc phải phá vỡ thì đánh phiên bản mới và giữ bản cũ sống tới khi không còn ai dùng."
    },
    {
      question: "IDOR là gì?",
      answer: "Là lỗ hổng xảy ra khi máy chủ nhận một id từ request rồi đọc hoặc sửa bản ghi đó mà không kiểm tra người gọi có quyền với bản ghi hay không. Đổi số trên URL là xem được dữ liệu người khác. Nó thuộc nhóm Broken Access Control, hạng mục đứng đầu OWASP Top 10."
    }
  ]}
/>
