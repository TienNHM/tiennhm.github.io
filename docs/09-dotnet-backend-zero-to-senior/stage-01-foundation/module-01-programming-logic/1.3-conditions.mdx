...
```csharp
public decimal TinhPhiVanChuyen(string khuVuc)
{
    if (khuVuc == "HN")      return 25_000m;
    else if (khuVuc == "HCM") return 30_000m;
    else if (khuVuc == "DN")  return 28_000m;
    else if (khuVuc == "CT")  return 35_000m;
    else                      return 50_000m;
}
```
...
private const decimal PhiMacDinh = 50_000m;

public decimal TinhPhiVanChuyen(string khuVuc)
    => PhiTheoKhuVuc.TryGetValue(khuVuc, out var phi) ? phi : PhiMacDinh;
```
