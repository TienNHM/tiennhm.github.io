...
description: "if/else, switch expression và pattern matching hiện đại của C#. Kèm guard clause để code phẳng, cách kiểm tra null cho đúng, và vì sao đừng viết bảng giá thành chuỗi if-else."
sidebar_position: 3
---
...
public decimal TinhGiam(Customer? customer, Order? order)
{
    if (customer != null)
    {
...
<Checklist
  title="Danh sách rà soát điều kiện"
  items={[
    { text: "Không có hàm nào lồng if quá hai tầng; phần còn lại đã chuyển thành guard clause." },
    { text: "Kiểm tra null dùng is null / is not null thay vì == null." },
    { text: "switch trên enum không có nhánh _, để trình biên dịch cảnh báo khi enum đổi." },
    { text: "Không có ternary lồng ternary." },
    { text: "Điều kiện dùng && và || chứ không phải & và |." },
    { text: "Không có chuỗi if-else nào chỉ để tra một bảng giá trị." },
  ]}
/>

## Bài tập áp dụng

1. **Làm phẳng.** Lấy đoạn `TinhGiam` bốn tầng ở mục 1.3.1, viết lại bằng guard clause, rồi viết lại lần nữa bằng property pattern. So sánh số dòng và mức dễ đọc của ba phiên bản.

2. **Bắt lỗi vét cạn.** Tạo `enum LeadStatus` bốn giá trị và một `switch expression` không có nhánh `_`. Thêm giá trị thứ năm vào enum và xem trình biên dịch nói gì. Ghi lại mã cảnh báo.

3. **Biến logic thành dữ liệu.** Tìm trong một dự án thật một chuỗi `if-else` từ bốn nhánh trở lên mà các nhánh chỉ khác nhau ở giá trị trả về. Chuyển nó thành `Dictionary`, và viết một câu trả lời: sau khi đổi, thêm một nhánh mới cần sửa mấy dòng?
...
<FAQSection
  items={[
    {
      question: "Guard clause là gì và vì sao nó tốt hơn if lồng nhau?",
      answer: "Guard clause là cách xử lý mọi trường hợp không hợp lệ ngay đầu hàm rồi return sớm, để phần logic chính nằm phẳng ở cuối và không bị thụt lề. Người đọc thấy ngay các điều kiện bị loại trừ, thay vì phải lần theo bốn tầng ngoặc để tìm phần việc thật."
    },
    {
      question: "Vì sao nên dùng is null thay vì == null?",
      answer: "Toán tử == có thể bị nạp chồng, nghĩa là một lớp có thể định nghĩa lại nó và chạy code bất kỳ. Còn is null được trình biên dịch dịch thẳng thành phép so sánh tham chiếu, không thể bị can thiệp. Ngoài ra is null ngắn hơn và đọc đúng nghĩa. Dạng phủ định là is not null."
    },
    {
      question: "switch expression khác switch statement ở chỗ nào?",
      answer: "switch expression trả về một giá trị và được kiểm tra vét cạn. Khi switch trên enum mà thiếu nhánh, trình biên dịch cảnh báo — nhờ đó khi ai đó thêm giá trị mới vào enum, mọi chỗ cần cập nhật đều được chỉ ra. switch statement thì chỉ thực thi lệnh và không có bảo đảm này."
    },
    {
      question: "Bỏ nhánh _ trong switch expression có rủi ro gì?",
      answer: "Nếu giá trị chạy vào không khớp nhánh nào, chương trình ném SwitchExpressionException lúc chạy. Với enum nội bộ do bạn kiểm soát thì đánh đổi này xứng đáng: nổ rõ ràng vẫn tốt hơn im lặng trả về giá trị mặc định sai. Với dữ liệu từ bên ngoài thì nên giữ nhánh _ và xử lý tường minh."
    },
    {
      question: "Khi nào một chuỗi if-else nên trở thành dữ liệu?",
      answer: "Khi các nhánh chỉ khác nhau ở giá trị chứ không khác nhau ở hành vi — ví dụ bảng giảm giá theo hạng khách. Lúc đó nó là một bảng tra cứu bị viết nhầm thành code. Chuyển sang Dictionary, rồi xa hơn là đưa xuống database hay file cấu hình, để đổi chính sách không phải deploy lại."
    },
    {
      question: "&& và & khác nhau thế nào trong điều kiện?",
      answer: "&& đoản mạch: vế trái sai thì không chạy vế phải. & luôn chạy cả hai vế. Vì vậy customer != null & customer.IsActive vẫn ném NullReferenceException, còn dùng && thì an toàn. Trong điều kiện hầu như luôn dùng && và ||."
    }
  ]}
/>
