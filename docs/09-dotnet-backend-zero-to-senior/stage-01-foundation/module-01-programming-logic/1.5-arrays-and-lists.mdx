...
```mermaid
flowchart TD
    A[Cần lưu nhiều phần tử] --> B{Thao tác chính là gì?}
    B -->|Duyệt tuần tự, số lượng cố định| C[array]
    B -->|Thêm/xoá ở cuối, duyệt| D[List]
    B -->|Tra theo khoá| E[Dictionary]
    B -->|Kiểm tra đã tồn tại chưa| F[HashSet]
    B -->|Vào trước ra trước| G[Queue]
    B -->|Vào sau ra trước| H[Stack]
```
