---
title: "1.6 — 6. Tư Duy Giải Bài Toán"
...
Còn tối ưu thì chỉ làm sau khi **đo**. Trực giác của con người về chỗ chậm gần như luôn sai — chi tiết ở bài [1.8 Big-O](1.7-data-structures-and-big-o.mdx).
...
```csharp
public decimal CalculateAverage(List<Order> orders)
    => orders.Sum(o => o.Total) / orders.Count;
```

| Ca biên | Đầu vào cụ thể | Kết quả | Đúng? |
|---|---|---|---|
| Rỗng | `new List<Order>()` | `DivideByZeroException` | **Sai** |
...
```csharp
public decimal CalculateAverage(IReadOnlyList<Order>? orders)
{
...
Một triệu dòng chỉ cần 20 lần kiểm tra. Đây chính là sức mạnh của O(log n) mà [bài 1.8](1.7-data-structures-and-big-o.mdx) sẽ gọi tên.
...
- **Trong lịch sử Git:** `git bisect` tìm commit gây lỗi giữa hàng nghìn commit — [bài 3.10](../module-03-git-developer-workflow/3.9-debugging-workflow.mdx).
...
- **Trong cấu trúc dữ liệu:** chỉ số B-tree của database tra một dòng trong mười triệu bằng ba tới bốn lần đọc trang — [bài 12.5](../../stage-04-database-production/module-12-sql-deep-dive/12.4-indexing.mdx).
...
- **Bài trước:** [1.5 — 5. Mảng và Danh Sách (Arrays & Lists)](1.5-arrays-and-lists.mdx)
- **Bài tiếp theo:** [1.7 — 7. Cấu Trúc Dữ Liệu Cơ Bản và Big-O](1.7-data-structures-and-big-o.mdx)
