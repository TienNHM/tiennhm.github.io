...
1. **Đo bẫy `Contains`.** Dựng hai danh sách 20.000 số nguyên. Đối chiếu chúng bằng `List.Contains` trong vòng lặp, đo bằng `Stopwatch`. Làm lại với `HashSet`. Ghi lại tỉ lệ chênh lệch.

2. **Khoá sai.** Tạo `class CustomerKey` thường (không phải `record`) với hai thuộc tính, dùng làm khoá `Dictionary`. Thêm một phần tử, rồi thử `TryGetValue` bằng một object **mới** có cùng giá trị. Giải thích vì sao không tìm thấy, rồi sửa bằng `record`.

3. **Đặt trước sức chứa.** Thêm 1.000.000 phần tử vào một `List<int>` không đặt capacity và một `List<int>` có đặt. So sánh thời gian và số lần cấp phát.
