...
description: "Hàm, tham số, ref/out và local function. Kèm ba tiêu chí phân biệt hàm tốt với hàm xấu: một mức trừu tượng, tên nói đúng việc, và tách hàm thuần khỏi hàm có tác dụng phụ."
sidebar_position: 5
---
...
<Checklist
  title="Danh sách rà soát hàm"
  items={[
    { text: "Mọi hàm gọi tên được bằng một động từ, và tên nói đúng việc nó làm." },
    { text: "Không có hàm nào trộn nhiều mức trừu tượng trong cùng một thân." },
    { text: "Tham số bool ở chỗ gọi đều có tham số đặt tên, hoặc đã đổi sang enum." },
    { text: "Không dùng out cho nhiều giá trị trả về; đã thay bằng tuple có tên hoặc record." },
    { text: "Hàm phụ thuộc thời gian nhận DateTimeOffset làm tham số, không gọi UtcNow bên trong." },
    { text: "Ngoại lệ chỉ dùng cho tình huống bất thường, không dùng cho luồng chạy thông thường." },
    { text: "Local function không dùng biến của hàm cha đều được khai static." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Chỉ gọi một lần thì có nên tách thành hàm riêng không?",
      answer: "Có, nếu việc tách đặt được tên cho một ý tưởng. Lý do tách hàm không phải là dùng lại, mà là đặt tên. Một điều kiện ba dòng khó hiểu, khi trở thành IsEligibleForLoyaltyDiscount(order), biến thành một khái niệm nghiệp vụ mà người đọc không cần giải mã."
    },
    {
      question: "Vì sao nên tránh out cho nhiều giá trị trả về?",
      answer: "Vì chỗ gọi phải khai báo biến trước, đọc rối, và không ghép chuỗi được với LINQ. Tuple có tên hoặc record cho cùng kết quả mà đọc rõ hơn. out chỉ còn một chỗ dùng chuẩn mực là mẫu TryXxx, nơi giá trị trả về bool mới là thông tin chính."
    },
    {
      question: "Một hàm ở một mức trừu tượng nghĩa là gì?",
      answer: "Nghĩa là mọi câu lệnh trong thân hàm nói chuyện ở cùng một tầng. Nếu một dòng nói 'gửi email cho khách' còn dòng kế bên nói 'mở kết nối SQL', hàm đang trộn hai mức. Cách sửa là đẩy chi tiết cấp thấp xuống các hàm con, giữ hàm cha như một bản tóm tắt đọc được."
    },
    {
      question: "Hàm thuần là gì và vì sao đáng theo đuổi?",
      answer: "Là hàm mà cùng đầu vào luôn cho cùng đầu ra và không đụng tới trạng thái bên ngoài. Nó test được mà không cần mock, chạy song song không lo tranh chấp, và đọc hiểu được mà không cần nhìn chỗ khác. Không phải hàm nào cũng thuần được, nhưng nên gom tác dụng phụ về một chỗ và giữ phần tính toán ở dạng thuần."
    },
    {
      question: "Khi nào ném ngoại lệ, khi nào trả về bool theo mẫu TryXxx?",
      answer: "Hỏi xem thất bại đó có bình thường không. Người dùng gõ sai định dạng ngày là bình thường nên dùng TryParse. Database biến mất giữa chừng là bất thường nên ném ngoại lệ. Thêm một lý do kỹ thuật: ném ngoại lệ tốn kém vì phải dựng stack trace, nên dùng nó cho luồng thông thường trong vòng lặp là lỗi hiệu năng thật sự."
    },
    {
      question: "Vì sao nên khai static cho local function?",
      answer: "Vì static ngăn nó bắt biến của hàm cha. Khi local function bắt biến, trình biên dịch phải tạo một closure và cấp phát bộ nhớ trên heap. Thêm static là để trình biên dịch báo lỗi nếu bạn vô tình dùng biến bên ngoài, tức là biến việc cấp phát ngoài ý muốn thành lỗi biên dịch."
    }
  ]}
/>
