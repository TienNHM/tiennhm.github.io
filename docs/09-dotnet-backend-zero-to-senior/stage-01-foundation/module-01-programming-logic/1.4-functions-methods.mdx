---
title: "1.4 — 4. Hàm (Functions / Methods)"
...
```mermaid
flowchart LR
    A[Tên hàm] --> B[Bắt đầu bằng động từ]
    A --> C[Nói CÁI GÌ, không nói LÀM THẾ NÀO]
    A --> D[Hàm trả bool bắt đầu bằng Is/Has/Can/Try]
    A --> E[PascalCase, tiếng Anh]
```
...
Analyze(orders, out int c, out decimal total, out decimal avg);
...
```csharp
public bool IsExpired(Subscription sub, DateTime asOf)
    => sub.ExpiresAt < asOf;              // mọi đầu vào đều tường minh
```
...
public void IsExpired_OnFeb29_UsesGivenInstant()
{
    ...
    Assert.False(IsExpired(sub, new DateTime(2024, 2, 29, 11, 59, 59)));
    Assert.True (IsExpired(sub, new DateTime(2024, 2, 29, 12, 0,  1)));
}
```
...
- **Bài trước:** [1.3 — 3. Vòng Lặp (Loops)](1.3-loops.mdx)
- **Bài tiếp theo:** [1.5 — 5. Mảng và Danh Sách (Arrays & Lists)](1.5-arrays-and-lists.mdx)
