---
title: "1.3 — 3. Vòng Lặp (Loops)"
description: "for, while, foreach và yield return. Kèm ba lỗi kinh điển: sửa collection khi đang duyệt, lệch một đơn vị, và gọi database bên trong vòng lặp — cái cuối là nguồn gốc của vấn đề N+1."
sidebar_position: 3
---
...
Bài [Vì sao EF Core bắn 201 query cho 1 màn hình danh sách?](/blog/ef-core-n-plus-1-query) đo cụ thể chi phí của lỗi này và chỉ cách phát hiện bằng log. Phần lý thuyết bên phía SQL nằm ở bài [hiệu năng truy vấn](/docs/database/learn-sql-in-30-days/20-query-performance).
...
<Checklist
  title="Danh sách rà soát vòng lặp"
  items={[
    { text: "Không có lời gọi database, HTTP hay đọc file nào nằm bên trong thân vòng lặp." },
    { text: "Không sửa collection trong lúc foreach qua chính nó." },
    { text: "Điều kiện dừng của for dùng < chứ không phải <=." },
    { text: "Vòng lặp chỉ để duyệt và lọc thì đã cân nhắc chuyển sang LINQ." },
    { text: "Không duyệt cùng một IEnumerable nhiều lần mà chưa ToList()." },
    { text: "Hàm dùng yield return có phần kiểm tra tham số tách ra hàm bọc ngoài." },
  ]}
/>
...
Tạo một `List<int>` mười phần tử, dùng `foreach` duyệt và `Remove` các phần tử chẵn. Chạy, ghi lại thông báo lỗi đầy đủ. Sau đó sửa bằng cả ba cách ở mục 1.4.3 và so sánh.
...
**Cách phòng lâu dài.** EF Core có thể được cấu hình để **báo lỗi** khi phát hiện truy vấn tải lười trong vòng lặp. Bài [Vì sao EF Core bắn 201 query cho 1 màn hình danh sách?](/blog/ef-core-n-plus-1-query) đo chi tiết chi phí, còn [Module 13](../../stage-04-database-production/module-13-entity-framework-core/) trình bày `Include` và các chiến lược tải dữ liệu để tránh lỗi này một cách hệ thống.

</details>

#### Bài 3 — Đo bộ nhớ của `yield return`

Viết hai hàm đọc một file văn bản lớn: một trả `List<string>`, một dùng `yield return`. Chạy mỗi hàm và in `GC.GetTotalMemory(true)` trước và sau. Giải thích khác biệt.
...
```csharp
static List<string> DocHet(string path) => File.ReadAllLines(path).ToList();

static IEnumerable<string> DocDan(string path)
{
    using var reader = new StreamReader(path);
    string? line;
    while ((line = reader.ReadLine()) is not null)
...
static void Do(string ten, Func<IEnumerable<string>> lay)
{
    var truoc = GC.GetTotalMemory(true);
    int dem = 0;
    foreach (var line in lay()) dem++;
    var sau = GC.GetTotalMemory(false);
    Console.WriteLine($"{ten,-8} {dem} dòng, tăng {(sau - truoc) / 1024 / 1024} MB");
}

Do("List", () => DocHet("lon.txt"));
Do("yield", () => DocDan("lon.txt"));
```
...
<FAQSection
  items={[
    {
      question: "Vì sao sửa collection khi đang foreach lại ném InvalidOperationException?",
      answer: "foreach làm việc qua một enumerator, và enumerator giữ một số phiên bản nội bộ của collection. Mọi thao tác thêm hay xoá làm số đó thay đổi, và lần gọi MoveNext() kế tiếp phát hiện ra rồi ném lỗi. Cách sửa: dùng RemoveAll, tạo danh sách mới bằng Where().ToList(), hoặc duyệt ngược bằng for."
    },
    {
      question: "Vì sao khi xoá phần tử tại chỗ phải duyệt ngược?",
      answer: "Vì xoá phần tử thứ i làm mọi phần tử phía sau dịch lên một vị trí. Nếu đi xuôi, phần tử vừa dịch vào vị trí i sẽ bị bỏ qua ở vòng lặp tiếp theo. Duyệt ngược từ cuối về đầu thì việc dịch không ảnh hưởng tới các chỉ số chưa xét."
    },
    {
      question: "Vấn đề N+1 là gì?",
      answer: "Là khi bạn chạy một query để lấy danh sách N bản ghi, rồi chạy thêm một query cho mỗi bản ghi trong vòng lặp — tổng cộng N+1 query. Với 200 đơn hàng, một màn hình danh sách bắn 201 câu truy vấn. Cách sửa là lấy toàn bộ dữ liệu phụ thuộc trong một query rồi tra cứu trong bộ nhớ, hoặc dùng Include của EF Core."
    },
    {
      question: "yield return khác gì với trả về một List?",
      answer: "Hàm có yield return không chạy gì cho tới khi ai đó bắt đầu duyệt kết quả, và mỗi lần chỉ giữ một phần tử trong bộ nhớ thay vì cả tập dữ liệu. Nhờ đó đọc được file rất lớn, hoặc dừng sớm mà không phí công. Đổi lại, ngoại lệ xảy ra lúc duyệt chứ không phải lúc gọi hàm."
    },
    {
      question: "Duyệt một IEnumerable hai lần có vấn đề gì?",
      answer: "LINQ trên IEnumerable được đánh giá lười, nên mỗi lần duyệt là một lần chạy lại toàn bộ chuỗi phép biến đổi từ đầu. Gọi Count() rồi First() trên cùng một biến là chạy hai lần. Nếu cần dùng nhiều lần, gọi ToList() để vật chất hoá kết quả một lần duy nhất."
    },
    {
      question: "Khi nào nên dùng for thay vì foreach?",
      answer: "Khi thật sự cần chỉ số — ví dụ để đánh số hiển thị, hoặc để sửa và xoá phần tử theo vị trí. Mọi trường hợp còn lại nên mặc định dùng foreach vì nó không có chỗ cho lỗi lệch một đơn vị."
    }
  ]}
/>
