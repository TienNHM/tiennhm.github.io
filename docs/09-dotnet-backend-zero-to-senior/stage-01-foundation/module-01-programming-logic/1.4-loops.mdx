...
1. **Tự gây ra lỗi.** Tạo một `List<int>` mười phần tử, `foreach` qua nó và `Remove` phần tử chẵn. Chạy và ghi lại thông báo lỗi đầy đủ. Sau đó sửa bằng cả ba cách ở mục 1.4.3 và so sánh.
...
3. **Đo bộ nhớ của `yield`.** Viết hai hàm đọc một file lớn: một trả `List<string>`, một dùng `yield return`. Chạy mỗi hàm và in `GC.GetTotalMemory(true)` trước và sau. Giải thích khác biệt.
