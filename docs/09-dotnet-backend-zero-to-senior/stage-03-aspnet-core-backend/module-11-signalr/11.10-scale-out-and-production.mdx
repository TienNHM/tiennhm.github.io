---
title: "11.10 — 10. Scale-out và Production"
description: "Vì sao cần backplane, Redis backplane không phải hàng đợi, sticky session, Azure SignalR Service, giám sát và graceful shutdown."
sidebar_position: 10
---
...
<Checklist
  title="Danh sách rà soát production"
  items={[
    { text: "Có backplane trước khi chạy quá một instance." },
    { text: "ChannelPrefix được đặt nếu nhiều ứng dụng dùng chung Redis." },
    { text: "AbortOnConnectFail = false để ứng dụng khởi động được khi Redis chưa sẵn sàng." },
    { text: "Redis có Sentinel hoặc Cluster, không phải một node đơn." },
    { text: "Hiểu rằng backplane là pub/sub và tin nhắn có thể mất." },
    { text: "Thông báo quan trọng được ghi database trước khi push." },
    { text: "Sticky session được cấu hình, hoặc đã bỏ negotiate có chủ đích." },
    { text: "Nginx có header Upgrade, Connection và proxy_read_timeout đủ dài." },
    { text: "Giám sát số kết nối, tỷ lệ kết nối lại và phân bố transport." },
    { text: "Health check backplane trả Degraded, không phải Unhealthy." },
    { text: "ShutdownTimeout đủ dài và có thông báo trước cho client." },
    { text: "Rolling deployment không làm toàn bộ client kết nối lại cùng lúc." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Điều gì hỏng khi scale ngang không có backplane?",
      answer: "Người gửi ở một pod và người nhận ở pod khác thì pod gửi không tìm thấy kết nối của người nhận, nên tin nhắn biến mất mà không có lỗi hay log nào. Với 2 pod thì khoảng một nửa tin nhắn mất, và nó chạy hoàn hảo trên máy dev một instance."
    },
    {
      question: "Redis backplane có đảm bảo tin nhắn tới nơi không?",
      answer: "Không. Nó là pub/sub chứ không phải hàng đợi, nên instance mất kết nối Redis trong vài giây sẽ mất hoàn toàn những tin phát trong khoảng đó, không có gửi lại và không có xác nhận. Đó là lý do phải ghi database trước rồi mới push."
    },
    {
      question: "Vì sao backplane nhân lưu lượng?",
      answer: "Mỗi tin gửi tới một nhóm được publish một lần rồi mọi instance đều nhận và tự lọc, kể cả instance không giữ kết nối nào của nhóm đó. Với 10 instance thì một tin thành 10 lượt nhận, 9 trong đó vô ích. Ở lưu lượng cao, Redis thành nút cổ chai."
    },
    {
      question: "Khi nào cần sticky session?",
      answer: "Với SSE và long polling thì luôn cần vì mọi request phải về cùng instance. Với WebSocket thì cần nếu còn bước negotiate, vì negotiate và kết nối phải cùng instance. Bỏ negotiate và chỉ dùng WebSocket thì không cần, đổi lại mất khả năng dự phòng khi WebSocket bị chặn."
    },
    {
      question: "Azure SignalR Service khác Redis backplane thế nào?",
      answer: "Nó thay đổi kiến trúc chứ không chỉ là backplane: client kết nối tới dịch vụ chứ không tới ứng dụng của bạn. Nhờ đó không cần sticky session, và deploy không làm đứt kết nối của client. Đánh đổi là chi phí theo kết nối và vendor lock-in; dưới khoảng 10.000 kết nối thì Redis rẻ hơn nhiều."
    },
    {
      question: "Health check cho backplane nên trả trạng thái gì?",
      answer: "Degraded chứ không phải Unhealthy. Redis chết thì real-time hỏng nhưng API vẫn phục vụ được, nên đặt Unhealthy sẽ rút toàn bộ pod khỏi load balancer vì một tính năng phụ và tự gây downtime."
    }
  ]}
/>
