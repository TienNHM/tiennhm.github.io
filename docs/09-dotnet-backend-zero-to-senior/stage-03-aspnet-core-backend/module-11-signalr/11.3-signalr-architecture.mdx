...
```csharp
// SAI — Context da khong con hop le khi callback chay
...
Muốn gửi từ ngoài ngữ cảnh một lời gọi thì dùng `IHubContext<T>` ([bài 11.8](11.8-ihubcontext-server-push-outside-hub.mdx)).
...
```csharp
// DUNG — tao scope rieng cho moi lan goi
...
await Clients.User(userId).Receive(dto);              // MOI thiet bi cua nguoi do
...
public class NotificationHub(IPresenceTracker presence) : Hub<INotificationClient>
{
    public override async Task OnConnectedAsync()
    {
        ...
        if (userId is null)
        {
            Context.Abort();                 // khong xac thuc duoc
            return;
        }
...
2. Nó có thể **không chạy** nếu tiến trình server bị kill — trạng thái presence phải tự hết hạn, đừng chỉ dựa vào sự kiện này ([bài 11.10](11.10-presence-online-offline-status.mdx)).
...
`EnableDetailedErrors` chỉ bật ở Development — ở production nó gửi **stack trace tới client** ([bài 8.9](../module-08-aspnet-core-fundamentals/8.9-global-exception-handling.mdx)).

`MaximumReceiveMessageSize` mặc định 32KB. SignalR không dành cho truyền file; dùng HTTP upload thường ([bài 9.6](../module-09-web-api-professional/9.6-file-upload-and-download.mdx)).
