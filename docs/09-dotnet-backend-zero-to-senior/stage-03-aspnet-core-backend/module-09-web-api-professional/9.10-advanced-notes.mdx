---
title: "9.10 — Mở rộng và đào sâu"
description: "GraphQL, gRPC, webhook, HATEOAS và OpenAPI sinh client — mở rộng sau khi đã vững REST."
sidebar_position: 10
---
...
<Checklist
  title="Danh sách rà soát API nâng cao"
  items={[
    { text: "Mọi endpoint khai báo Produces đầy đủ cho OpenAPI." },
    { text: "Client do đội viết được sinh từ OpenAPI, không viết tay." },
    { text: "Bước sinh client nằm trong CI để bắt breaking change." },
    { text: "Webhook có ký HMAC kèm timestamp trong chữ ký." },
    { text: "Webhook gửi qua outbox và có retry với backoff." },
    { text: "Nếu dùng GraphQL, có giới hạn độ sâu và độ phức tạp truy vấn." },
    { text: "Đã cân nhắc tham số chọn trường trước khi chọn GraphQL." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Lợi ích lớn nhất của việc sinh client từ OpenAPI là gì?",
      answer: "Breaking change trở thành lỗi biên dịch ở phía client thay vì lỗi lúc chạy. Đưa bước sinh client vào CI thì mọi thay đổi API đều được kiểm tra trước khi triển khai."
    },
    {
      question: "Vì sao thiếu Produces làm mất lợi ích của việc sinh client?",
      answer: "Vì công cụ sinh không biết kiểu trả về nên cho ra kiểu any, và khi đó trình biên dịch không kiểm tra được gì."
    },
    {
      question: "Vì sao timestamp phải nằm trong chữ ký webhook?",
      answer: "Không có nó, kẻ tấn công chặn được một request hợp lệ có thể gửi lại nó mãi mãi và chữ ký vẫn đúng. Timestamp cho phép bên nhận từ chối request quá cũ."
    },
    {
      question: "GraphQL kéo theo ba vấn đề gì mà REST không có?",
      answer: "N plus 1 query vì mỗi trường lồng nhau có thể sinh một truy vấn riêng, truy vấn tuỳ ý có thể làm sập database nên phải giới hạn độ sâu và độ phức tạp, và cache HTTP không dùng được vì mọi truy vấn là POST tới cùng một URL."
    },
    {
      question: "Lựa chọn trung gian thay cho GraphQL là gì?",
      answer: "Giữ REST và thêm tham số chọn trường trên query string. Bạn có phần lớn lợi ích về việc tránh over-fetching mà giữ được cache HTTP và không thêm hạ tầng nào."
    },
    {
      question: "Vì sao HATEOAS hiếm khi đáng làm?",
      answer: "Vì gần như không client nào tận dụng. Frontend đã biết cấu trúc API từ tài liệu hoặc từ client sinh tự động, nên phần link nhúng chỉ làm payload to thêm."
    }
  ]}
/>
