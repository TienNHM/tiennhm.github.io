---
title: "9.6 — 6. Rate Limiting (.NET 7+)"
...
{
    o.AddFixedWindowLimiter("cu-dinh", opt =>
    {
        opt.PermitLimit = 100;
...
```csharp
o.AddSlidingWindowLimiter("truot", opt =>
{
    opt.PermitLimit = 100;
    opt.Window = TimeSpan.FromMinutes(1);
...
```csharp
o.AddTokenBucketLimiter("gao", opt =>
{
...
```csharp
o.AddConcurrencyLimiter("dong-thoi", opt =>
{
...
```csharp
async Task<HttpResponseMessage> GoiXau(string url, CancellationToken ct)
{
    while (true)
    {
...
```csharp
async Task<HttpResponseMessage> GoiTot(string url, CancellationToken ct)
{
    for (int lan = 0; lan < 5; lan++)
    {
        ...
        var cho = res.Headers.RetryAfter?.Delta
                  ?? TimeSpan.FromSeconds(Math.Pow(2, lan));         // lùi theo cấp số nhân
        cho += TimeSpan.FromMilliseconds(Random.Shared.Next(0, 1000));  // thêm nhiễu

        _logger.LogWarning("Bị giới hạn, chờ {Giay}s", cho.TotalSeconds);
