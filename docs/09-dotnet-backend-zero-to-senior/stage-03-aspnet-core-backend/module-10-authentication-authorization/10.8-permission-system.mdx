...
description: "Quyền chi tiết: vì sao bitmask hết chỗ ở quyền thứ 65, mô hình role-permission, cache quyền, và vì sao không nên nhét quyền vào token."
sidebar_position: 8
---
...
Và vấn đề lớn hơn kích thước: **gỡ quyền không có hiệu lực** cho tới khi token hết hạn ([bài 10.2](10.2-authentication-vs-authorization.mdx)).
...
Kết hợp với `PermissionPolicyProvider` ở [bài 10.6](10.6-policy-based-authorization.mdx), bạn có một hệ thống mà **gõ sai tên quyền là lỗi biên dịch**.
...
<Checklist
  title="Danh sách rà soát hệ thống quyền"
  items={[
    { text: "Quyền là chuỗi có cấu trúc, không phải bitmask enum." },
    { text: "Tên quyền dùng hằng số, gõ sai là lỗi biên dịch." },
    { text: "Mô hình hỗ trợ cả cấp thêm lẫn gỡ bỏ quyền ở mức người dùng." },
    { text: "Quyền bị gỡ thắng quyền được cấp từ vai trò." },
    { text: "Quyền không nằm trong token nếu số lượng lớn." },
    { text: "Quyền được cache với TTL làm trần rủi ro." },
    { text: "Cache bị xoá trong MỌI đường thay đổi quyền, kể cả sửa quyền của vai trò." },
    { text: "Nhiều instance thì dùng cache phân tán, không phải IMemoryCache." },
    { text: "Có endpoint trả quyền của người dùng cho client ẩn nút." },
    { text: "Server vẫn kiểm tra quyền cho mọi request, không tin client." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao bitmask enum không mở rộng được?",
    ...
    {
      question: "Nên đặt tên quyền thế nào?",
      answer: "Theo quy ước tài nguyên chấm hành động, ví dụ customers.delete hay deals.approve, và khai bằng hằng số trong các lớp lồng nhau. Nhờ đó gõ sai là lỗi biên dịch, và thêm quyền chỉ cần thêm một hằng số cùng một dòng trong bảng."
    },
    {
      question: "Vì sao cần cột IsGranted ở bảng quyền người dùng?",
      answer: "Để hỗ trợ cả hai chiều: cấp thêm quyền ngoài vai trò và gỡ bỏ một quyền mà vai trò có. Nhu cầu kiểu chị An là Manager nhưng không được xoá khách hàng không mô hình hoá được bằng vai trò. Thứ tự áp dụng là quyền từ vai trò, cộng quyền cấp riêng, rồi trừ quyền bị gỡ; quyền bị gỡ thắng."
    },
    {
      question: "Có nên nhét danh sách quyền vào token không?",
      answer: "Chỉ khi số quyền nhỏ, dưới khoảng 10. Với hàng trăm quyền thì token phình lên vài KB và vượt giới hạn header của nhiều proxy, gây lỗi 431. Quan trọng hơn là gỡ quyền không có hiệu lực cho tới khi token hết hạn."
    },
    {
      question: "Cache quyền cần lưu ý gì?",
      answer: "Phải xoá cache trong mọi đường thay đổi quyền, gồm gán và gỡ vai trò, sửa quyền của vai trò ảnh hưởng mọi người dùng vai trò đó, cấp và gỡ quyền riêng, và khoá tài khoản. TTL ngắn như 5 phút đóng vai trò trần rủi ro khi bạn quên một chỗ. Với nhiều instance thì IMemoryCache chỉ xoá trên một pod nên cần cache phân tán."
    },
    {
      question: "Client ẩn nút theo quyền có phải bảo mật không?",
      answer: "Không, đó chỉ là trải nghiệm người dùng để họ không bấm vào thứ sẽ báo lỗi. Client có thể bị sửa và API gọi trực tiếp được, nên server vẫn phải kiểm tra quyền cho mọi request."
    }
  ]}
/>
