...
description: "Ai xác thực và ai phân quyền, vấn đề thu hồi token, token exchange thay vì chuyển tiếp token, và cách chia sẻ thông tin người dùng giữa service."
sidebar_position: 7
---
...
Gateway **không thể** làm việc này — nó không có bảng `Leads`, và nếu ta cho nó truy cập thì đã vi phạm database-per-service ([bài 18.3](18.3-service-decomposition.mdx)).
...
```csharp
// Moi service VAN validate token — phong thu nhieu lop
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        ...
        options.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            ClockSkew = TimeSpan.FromSeconds(30)      // mac dinh 5 PHUT — qua dai
        };
    });
```

`ClockSkew` mặc định là **5 phút**, nghĩa là token đã hết hạn vẫn được chấp nhận thêm 5 phút nữa ([bài 10.3](../../stage-03-aspnet-core-backend/module-10-authentication-authorization/10.3-jwt-authentication.mdx)). Với hệ thống có đồng bộ thời gian qua NTP, 30 giây là quá đủ.
...
```
10:00  Nhan vien nghi viec, admin vo hieu hoa tai khoan
10:00  Token cu VAN HOP LE — khong ai huy duoc
10:15  Token het han
...
```csharp
// Danh sach chan — kiem tra jti trong Redis
...
Cache 5 phút giữ hiệu năng gần như token tự chứa, nhưng thay đổi quyền có hiệu lực trong vòng 5 phút thay vì phải chờ cấp token mới ([bài 14.4](../../stage-04-database-production/module-14-caching-background-jobs/14.4-idistributedcache-and-redis.mdx)).
...
var exchanged = await _identityClient.ExchangeTokenAsync(new TokenExchangeRequest
{
    ...
    Audience = "billing-api",              // chi dung duoc cho Billing
...
```csharp
// SAI — client tu khai tenant, doi so la doc du lieu tenant khac
var tenantId = Request.Query["tenantId"];

// DUNG — lay tu claim da duoc ky
var tenantId = User.FindFirst("tenant_id")?.Value
    ?? throw new UnauthorizedAccessException("Thiếu tenant_id");
```

Mỗi service tự áp dụng lọc theo tenant qua global query filter ([bài 13.11](../../stage-04-database-production/module-13-entity-framework-core/13.11-global-query-filters.mdx)). **Đừng tin** rằng gateway đã lọc — phòng thủ nhiều lớp, và rò rỉ dữ liệu giữa các tenant là loại sự cố khó khắc phục nhất về mặt uy tín.

### 18.7.6 — Rà lại code của bạn

<Checklist
  title="Danh sách rà soát identity trong hệ phân tán"
  items={[
    { text: "Gateway xác thực token, service phân quyền theo tài nguyên." },
    { text: "Mỗi service vẫn tự xác thực token, không tin mạng nội bộ." },
    { text: "ClockSkew đặt ngắn, không để mặc định 5 phút." },
    { text: "Access token có thời hạn ngắn kèm refresh token." },
    { text: "Đã quyết định có cần thu hồi tức thì hay không, và cài đặt tương ứng." },
    { text: "Token không chứa danh sách quyền chi tiết." },
    { text: "Quyền chi tiết tra cứu tại service và cache ngắn hạn." },
    { text: "Đã đánh giá rủi ro của việc chuyển tiếp token giữa service." },
    { text: "Job nền dùng danh tính riêng của service, không mượn token người dùng." },
    { text: "tenant_id lấy từ claim đã ký, không lấy từ tham số client gửi." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao gateway xác thực còn service phân quyền?",
      answer: "Vì xác thực chỉ cần kiểm tra chữ ký, hạn và audience nên làm một lần ở biên là hợp lý. Phân quyền theo tài nguyên cần dữ liệu mà chỉ service mới có, ví dụ ai sở hữu lead nào, và gateway không được phép truy cập database của service."
    },
    {
      question: "Vì sao service vẫn phải xác thực token dù đã có gateway?",
      answer: "Vì nếu ai đó vào được mạng nội bộ, họ có thể gọi thẳng service và bỏ qua gateway. Service tin tưởng hoàn toàn vào vị trí mạng là một lớp phòng thủ duy nhất, và nó thất bại hoàn toàn khi bị xuyên qua."
    },
    {
      question: "Vấn đề thu hồi JWT là gì và xử lý thế nào?",
      answer: "JWT tự chứa nên không huỷ được trước khi hết hạn, khiến một tài khoản vừa bị vô hiệu hoá vẫn truy cập được cho tới lúc token hết hạn. Xử lý bằng thời hạn ngắn kèm refresh token, thêm danh sách chặn jti trong Redis nếu cần thu hồi tức thì, hoặc dùng reference token."
    },
    {
      question: "Vì sao không nên nhồi mọi quyền vào claim?",
      answer: "Vì token phình to có thể vượt giới hạn header của reverse proxy và gây lỗi 431, mọi request phải truyền cả khối đó, và quyền chỉ cập nhật khi cấp token mới. Giữ token gọn và tra cứu quyền chi tiết tại service với cache ngắn hạn."
    },
    {
      question: "Rủi ro của việc chuyển tiếp token người dùng giữa các service là gì?",
      answer: "Nếu một service bị xâm nhập, kẻ tấn công có token của mọi người dùng đang truy cập và dùng được chúng để gọi mọi service khác, kể cả những service mà service bị xâm nhập không bao giờ cần gọi. Token exchange thu hẹp phạm vi đó."
    },
    {
      question: "Vì sao tenant_id không được lấy từ tham số client gửi?",
      answer: "Vì client có thể đổi tham số đó để đọc dữ liệu của tenant khác. Nó phải lấy từ claim đã được ký, và mỗi service vẫn tự áp lọc theo tenant thay vì tin rằng gateway đã lọc."
    }
  ]}
/>
