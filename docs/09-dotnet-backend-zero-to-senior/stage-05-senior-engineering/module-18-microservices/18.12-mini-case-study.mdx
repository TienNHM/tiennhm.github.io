---
title: "18.12 — Mini case study"
description: "Tách Notification Service khỏi monolith CRM: kế hoạch Strangler Fig năm bước, ba sự cố gặp phải và cách xử lý từng cái."
sidebar_position: 12
---

import { SummaryBox, Checklist, FAQSection } from '@site/src/components/SEO';

# 18.12 — Mini case study
...
Gio cao diem (9h va 14h):
  - Module Notification gui ~50.000 email/gio (nhac lich, canh bao deal)
  - CPU cua ca ung dung leo len 85%
  - API nghiep vu cham theo: p99 tu 300ms len 2,1 giay
  - Phai scale ca 4 instance len 12 -> ton tien cho phan KHONG can scale
```
...
**Bước 1 — Gateway.** Đặt YARP trước monolith, mọi route vẫn về monolith. Tự nó không đổi gì nhưng tạo **điểm chuyển đổi** cho các bước sau ([bài 18.4](18.4-api-gateway-yarp-dotnet.mdx)).
...
Modules/Notification/
  ...
  Notification.Contracts/        <- chi project nay duoc module khac dung
```

Bước này phát hiện ra 14 chỗ code khác gọi thẳng vào nội bộ Notification — con số mà không ai ngờ. Sửa chúng **trong monolith** rẻ hơn rất nhiều so với sửa sau khi đã tách ra mạng ([bài 18.2](18.2-khi-nao-khong-nen-microservices.mdx)).
...
await _bus.Publish(new SendNotificationCommand(...), ct);   // co the that bai
```
...
Đây chính là dual write problem ([bài 17.3](../module-17-distributed-systems/17.3-messaging-fears-lost-duplicates.mdx)).

**Sửa:** đưa message vào outbox trong cùng transaction.

```csharp
_db.OutboxMessages.Add(OutboxMessage.From(new SendNotificationCommand(...)));
await _db.SaveChangesAsync(ct);        // MOT transaction cho ca hai
```

**Bài học:** outbox phải có **trước** khi bắt đầu tách, không phải thêm vào khi đã mất dữ liệu ([bài 17.4](../module-17-distributed-systems/17.4-outbox-pattern-applied.mdx)).
...
```csharp
// Moi thong bao = mot loi goi HTTP toi monolith
...
**Bài học:** integration event phải **tự chứa** ([bài 17.2](../module-17-distributed-systems/17.2-event-driven-architecture-when-to-use.mdx)). Nếu consumer phải gọi ngược về producer để hiểu message, bạn chưa thật sự tách — chỉ thêm một chặng mạng.
...
<Checklist
  title="Danh sách rà soát khi tách service đầu tiên"
  items={[
    { text: "Module được chọn ít phụ thuộc và hỏng thì ít nghiêm trọng." },
    { text: "Đã tách thành module trong monolith trước khi tách ra tiến trình." },
    { text: "Outbox đã có sẵn trước khi bắt đầu tách." },
    { text: "Có cơ chế chống trùng dùng chung trong giai đoạn chạy song song." },
    { text: "Integration event tự chứa, consumer không gọi ngược về producer." },
    { text: "Mỗi bước đảo ngược được bằng đổi cấu hình." },
    { text: "Lưu lượng chuyển dần theo phần trăm, không chuyển một lần." },
    { text: "Code cũ được xoá sau khi ổn định, không để lại phòng khi cần." },
    { text: "Đã ghi lại thời gian thực tế để ước lượng lần sau." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao service tách đầu tiên nên chọn chỗ hỏng thì ít nghiêm trọng?",
      answer: "Vì đây là lần đầu đội làm việc này và chắc chắn sẽ có sai sót. Tách một module mà email trễ vài phút không làm hỏng dữ liệu thì rủi ro chấp nhận được, còn tách Billing hay Order trước là đặt cược vào lần đầu."
    },
    {
      question: "Vì sao phải tách thành module trong monolith trước?",
      answer: "Vì bước đó phơi bày mọi chỗ code khác đang gọi thẳng vào nội bộ module. Sửa những chỗ đó trong monolith rẻ hơn rất nhiều so với sửa sau khi đã tách ra thành lời gọi mạng."
    },
    {
      question: "Sự cố mất thông báo xảy ra vì đâu?",
      answer: "Dual write. Monolith ghi database rồi publish message trong hai bước riêng, và tiến trình bị dừng giữa hai bước trong một đợt deploy. Về phía monolith mọi thứ đã thành công nên không có dấu vết nào trong log."
    },
    {
      question: "Vì sao cần chống trùng trong giai đoạn chạy song song?",
      answer: "Vì cơ chế chia lưu lượng có thể sai mà bạn không biết, như trong ca này khi monolith vẫn gửi 100 phần trăm. Một bảng chống trùng dùng chung với unique constraint đảm bảo mỗi thông báo chỉ gửi một lần bất kể ai gửi."
    },
    {
      question: "Vì sao độ trễ tăng khi service mới gọi ngược về monolith?",
      answer: "Vì mỗi thông báo thành một lời gọi HTTP thêm, và chúng đánh vào chính monolith mà ta đang cố giảm tải. Đưa dữ liệu cần thiết vào chính message giải quyết cả hai vấn đề cùng lúc."
    },
    {
      question: "Bài học chung từ sự cố thứ ba là gì?",
      answer: "Integration event phải tự chứa. Nếu consumer phải gọi ngược về producer để hiểu message thì bạn chưa thật sự tách, chỉ thêm một chặng mạng vào cùng một sự phụ thuộc."
    }
  ]}
/>
...
- **Bài trước:** [18.11 — Mở rộng và đào sâu](18.11-advanced-notes.mdx)
- **Bài tiếp theo:** [18.13 — Ví dụ thực tế nhanh](18.13-quick-real-world-example.mdx)
