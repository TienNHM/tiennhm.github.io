...
description: "Code chạy được đầu-cuối: entity, interceptor, dispatcher an toàn nhiều instance, consumer idempotent, và ba bài test chứng minh nó đúng."
sidebar_position: 8
---
...
public sealed class OutboxMessageConfiguration : IEntityTypeConfiguration<OutboxMessage>
{
    public void Configure(EntityTypeBuilder<OutboxMessage> b)
    {
        ...
        // Filtered index: chi chua hang CHUA xu ly
...
public sealed class ConvertLeadHandler(AppDbContext db) : IRequestHandler<ConvertLeadCommand, Result>
{
    public async Task<Result> Handle(ConvertLeadCommand request, CancellationToken ct)
    {
        ...
        await db.SaveChangesAsync(ct);       // MOT transaction cho ca ba thay doi
    ...
    ILogger<OutboxDispatcher> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                // Con viec thi lam tiep ngay, het viec thi nghi
                if (processed == 0)
    ...
    private async Task<int> DispatchBatchAsync(CancellationToken ct)
    {
        ...
        // READPAST: bo qua hang instance khac dang khoa -> khong publish trung
...
- **`catch (OperationCanceledException)` riêng.** Khi shutdown, `Task.Delay` ném exception này; nếu bị bắt chung vào nhánh log lỗi, mỗi lần dừng container sẽ sinh một dòng log lỗi giả ([bài 15.3](../../stage-04-database-production/module-15-docker-deployment/15.3-dockerfile-for-aspnet-core.mdx)).
    ...
    : IConsumer<LeadConvertedIntegrationEvent>
{
    public async Task Consume(ConsumeContext<LeadConvertedIntegrationEvent> context)
    {
        ...
        {
            return;                                  // da xu ly roi -> bo qua, ACK binh thuong
        }
...
public async Task Outbox_ShouldSurvive_CrashAfterCommit()
{
    ...
    Assert.Single(pending);        // message van con, se duoc publish khi dispatcher chay lai
}

// 2. Message trung -> chi xu ly MOT lan
[Fact]
public async Task Consumer_ShouldBeIdempotent_WhenMessageDeliveredTwice()
{
    ...
    await _consumer.Consume(CreateContext(evt));        // CUNG EventId

    var count = await _billingDb.Subscriptions.CountAsync(s => s.CustomerId == customerId);
    Assert.Equal(1, count);
}

// 3. Hai dispatcher song song -> khong publish trung
[Fact]
...
Test 3 **phải chạy trên database thật** (Testcontainers) — provider InMemory không có khoá nên `READPAST` không có ý nghĩa gì và test sẽ xanh một cách vô nghĩa ([bài 16.10](../module-16-clean-architecture/16.10-testing-strategy-overview.mdx)).

### 17.8.7 — Giám sát

```csharp
// Chi so quan trong nhat: tuoi cua message chua xu ly CU NHAT
...
<Checklist
  title="Danh sách rà soát luồng outbox đầu-cuối"
  items={[
    { text: "Use case ghi outbox và thay đổi nghiệp vụ trong cùng một SaveChangesAsync." },
    { text: "Dispatcher dùng READPAST hoặc SKIP LOCKED." },
    { text: "Dispatcher không nghỉ khi vừa xử lý xong một batch đầy." },
    { text: "Một message lỗi không chặn các message còn lại trong batch." },
    { text: "OperationCanceledException khi shutdown được xử lý riêng." },
    { text: "Consumer ghi inbox và thay đổi nghiệp vụ trong cùng transaction." },
    { text: "Chống trùng dựa vào primary key, không dựa vào câu if." },
    { text: "Có test cho crash sau commit, message trùng, và hai dispatcher song song." },
    { text: "Test chạy trên database thật, không dùng provider InMemory." },
    { text: "Giám sát tuổi của message chưa xử lý cũ nhất, không chỉ đếm số lượng." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao use case chỉ nên gọi SaveChangesAsync một lần?",
      answer: "Vì một lần gọi là một transaction, nên thay đổi nghiệp vụ và hàng outbox được commit cùng nhau hoặc không gì cả. Gọi hai lần tạo ra khoảng giữa mà tiến trình có thể chết, và đó chính là dual write problem."
    },
    {
      question: "Vì sao dispatcher không nên nghỉ khi vừa xử lý xong một batch đầy?",
      answer: "Vì batch đầy nghĩa là có thể còn message chờ. Nếu tồn 10.000 message mà mỗi 5 giây chỉ xử lý 100 thì hàng đợi không bao giờ được rút hết. Chỉ nghỉ khi batch trả về rỗng."
    },
    {
      question: "Vì sao try catch phải nằm trong vòng lặp message chứ không bao ngoài?",
      answer: "Để một message lỗi không chặn các message còn lại trong batch. Nếu bao ngoài, một poison message sẽ làm cả batch dừng và lặp lại mãi ở cùng vị trí."
    },
    {
      question: "Vì sao OperationCanceledException cần xử lý riêng?",
      answer: "Vì khi container dừng, Task.Delay ném exception này một cách bình thường. Bắt chung vào nhánh log lỗi sẽ sinh một dòng lỗi giả mỗi lần shutdown, làm nhiễu log và gây báo động sai."
    },
    {
      question: "Vì sao commit transaction ở cuối consumer lại quan trọng?",
      answer: "Vì nếu phần nghiệp vụ thất bại thì transaction rollback và hàng inbox cũng biến mất, nên lần retry sau xử lý lại bình thường. Nếu inbox commit riêng trước, message bị đánh dấu đã xử lý trong khi thực ra chưa, và nó bị bỏ qua vĩnh viễn."
    },
    {
      question: "Vì sao đo tuổi message chưa xử lý tốt hơn đếm số lượng?",
      answer: "Vì mười nghìn message vừa tạo trong một giây là bình thường, còn một message kẹt mười phút là sự cố. Đếm số lượng không phân biệt được hai tình huống đó, đo tuổi thì phân biệt được ngay."
    }
  ]}
/>
