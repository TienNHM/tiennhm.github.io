---
title: "17.8 — 8. MassTransit / NServiceBus / EasyNetQ"
description: "Ba thư viện messaging trên .NET: abstraction, giá cả, mức khoá vào framework, và cách giữ code nghiệp vụ không phụ thuộc vào lựa chọn đó."
sidebar_position: 8
---
...
Cấu hình này đã có ba thứ cần cho production: outbox chống mất tin ([bài 17.4](17.3-outbox-pattern-applied.mdx)), retry hai tầng ([bài 17.7](17.6-retry-and-backoff.mdx)), và prefetch để phân bổ tải đều giữa consumer ([bài 17.5](17.4-kafka-vs-rabbitmq-pragmatic.mdx)).
...
<Checklist
  title="Danh sách rà soát khi chọn thư viện messaging"
  items={[
    { text: "Đã đọc giấy phép của đúng phiên bản định dùng, không dựa vào thông tin cũ." },
    { text: "Đã ước lượng chi phí thoát nếu phải bỏ thư viện này." },
    { text: "Chỉ dùng một thư viện messaging trong dự án." },
    { text: "Application layer chỉ phụ thuộc interface của chính mình." },
    { text: "Có fake publisher để test không cần broker." },
    { text: "Đã cấu hình prefetch, retry hai tầng và outbox nếu dùng MassTransit." },
    { text: "Nếu dùng saga, phần phụ thuộc được cô lập trong một project riêng." },
    { text: "Đã cân nhắc client gốc nếu nhu cầu thật sự đơn giản." },
  ]}
/>
...
| Logic xử lý message | **Có** | Qua `IEventHandler<T>` |
...
<FAQSection
  items={[
    {
      question: "Ba thư viện này khác nhau ở điểm nào rõ nhất?",
      answer: "Ở mức trừu tượng và phạm vi tính năng. MassTransit và NServiceBus có abstraction dày với outbox, saga, retry hai tầng và hỗ trợ nhiều broker. EasyNetQ mỏng, chỉ RabbitMQ, không có saga hay outbox, nhưng dùng được ngay trong vài dòng."
    },
    {
      question: "Vì sao phải kiểm tra giấy phép trước khi chọn thư viện nền tảng?",
      answer: "Vì mô hình cấp phép của dự án mã nguồn mở có thể thay đổi giữa các phiên bản. Khi đó bạn phải chọn giữa trả tiền, ở lại phiên bản cũ không còn cập nhật, hoặc viết lại lớp hạ tầng. Nên đọc giấy phép của đúng phiên bản định dùng và ước lượng trước chi phí thoát."
    },
    {
      question: "Vì sao Application layer không nên phụ thuộc trực tiếp IPublishEndpoint?",
      answer: "Vì khi đó mọi handler đều là một chỗ phải sửa nếu đổi thư viện. Định nghĩa interface IEventPublisher của riêng mình và cài đặt nó ở tầng hạ tầng thì đổi thư viện chỉ phải viết lại một file."
    },
    {
      question: "Ngoài khả năng đổi thư viện, tách abstraction còn lợi gì?",
      answer: "Test không cần dựng broker vì có thể dùng fake publisher thu message vào danh sách, và Application không bị ràng buộc vào vòng đời DI của thư viện, thứ hay gây lỗi khó chẩn đoán về scoped service."
    },
    {
      question: "Giới hạn của việc bọc thư viện sau interface là gì?",
      answer: "Không nên bọc tính năng nâng cao. Saga state machine không thể đưa sau một interface chung mà vẫn giữ giá trị, nên nếu dùng saga thì chấp nhận phụ thuộc và cô lập nó trong một project riêng."
    },
    {
      question: "Khi nào dùng client gốc là đủ, không cần thư viện nào?",
      answer: "Khi chỉ có một broker, chỉ publish vài loại event, không cần saga hay scheduling, và bạn đã tự viết outbox. Chọn thư viện khi cần saga, cần retry và DLQ có sẵn, hoặc muốn đổi broker mà không sửa code nghiệp vụ."
    }
  ]}
/>
