---
title: "17.2 — 2. Hai nỗi sợ cốt lõi: mất tin & gửi trùng"
description: "Dual write problem, vì sao exactly-once không tồn tại ở tầng vận chuyển, và cách làm consumer an toàn khi message tới hai lần."
sidebar_position: 2
---
...
<Checklist
  title="Danh sách rà soát chống mất tin và trùng tin"
  items={[
    { text: "Không có chỗ nào ghi database rồi publish broker trong hai bước riêng rẽ." },
    { text: "Đã dùng outbox thay vì try/catch quanh publish." },
    { text: "Mỗi consumer đều an toàn khi nhận cùng message hai lần." },
    { text: "Chống trùng dựa vào unique constraint, không dựa vào câu if kiểm tra trước." },
    { text: "Ghi inbox và thay đổi nghiệp vụ nằm trong cùng một transaction." },
    { text: "Đã kiểm thử bằng cách xử lý cùng một message hai lần liên tiếp." },
    { text: "Bảng inbox có job dọn dữ liệu cũ, xoá theo lô." },
    { text: "Không dựa vào exactly-once của broker khi consumer ghi ra database ngoài." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Dual write problem là gì?",
      answer: "Là việc ghi vào hai hệ thống lưu trữ không chia sẻ transaction, ví dụ database và broker. Giữa hai thao tác luôn có một khoảng mà tiến trình có thể chết, nên một bên đã đổi còn bên kia thì chưa. Đổi thứ tự chỉ đổi hướng mất nhất quán chứ không sửa được."
    },
    {
      question: "Vì sao try/catch không sửa được dual write?",
      answer: "Vì SaveChanges đã commit nên không rollback được, và nếu ghi log để retry thì tiến trình vẫn có thể chết trước khi retry chạy. Vấn đề gốc là tiến trình chính là thứ có thể chết, nên không đoạn code nào bên trong nó giải quyết được."
    },
    {
      question: "Vì sao exactly-once delivery không tồn tại?",
      answer: "Vì broker không phân biệt được consumer chết trước khi xử lý với consumer xử lý xong nhưng ACK bị mất trên đường về. Nó buộc phải chọn ACK trước khi xử lý, tức có thể mất, hoặc ACK sau khi xử lý, tức có thể trùng. Không có lựa chọn thứ ba."
    },
    {
      question: "Exactly-once semantics của Kafka có mâu thuẫn với điều đó không?",
      answer: "Không, vì phạm vi của nó hẹp, chỉ áp dụng cho luồng Kafka sang Kafka với transaction và idempotent producer. Ngay khi consumer ghi ra một database ngoài thì đảm bảo đó không còn áp dụng và bạn vẫn cần idempotency."
    },
    {
      question: "Vì sao kiểm tra tồn tại trước rồi mới ghi là sai?",
      answer: "Vì giữa lúc kiểm tra và lúc ghi, một consumer khác có thể kiểm tra và cũng thấy chưa xử lý, nên cả hai cùng xử lý. Cách chắc chắn là để database quyết định bằng unique constraint rồi bắt lỗi vi phạm, vì đó là một thao tác nguyên tử."
    },
    {
      question: "Vì sao inbox và thay đổi nghiệp vụ phải cùng transaction?",
      answer: "Vì nếu ghi inbox commit riêng mà phần nghiệp vụ thất bại, message bị đánh dấu đã xử lý trong khi thực ra chưa, và lần retry sau sẽ bỏ qua nó. Khi đó bạn mất dữ liệu một cách im lặng."
    }
  ]}
/>
