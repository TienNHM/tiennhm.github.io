---
title: "19.10 — Mở rộng và đào sâu"
description: "GC tuning, Span và ArrayPool, Native AOT, object pooling — những thứ nằm ngoài phạm vi module và điều kiện để chúng đáng làm."
sidebar_position: 10
---
...
<Checklist
  title="Danh sách rà soát trước khi tối ưu sâu"
  items={[
    { text: "Đã sửa hết N+1 được phát hiện bằng test đếm truy vấn." },
    { text: "Đã thêm index cho mọi truy vấn chậm đã đo." },
    { text: "Không còn sync-over-async trong đường xử lý request." },
    { text: "Đã dùng projection cho truy vấn chỉ đọc." },
    { text: "Đã cache dữ liệu tổng hợp nặng." },
    { text: "Các lời gọi độc lập đã chạy song song." },
    { text: "Có số liệu profiler chứng minh hot path cần tối ưu sâu." },
    { text: "Nếu chạy container nhỏ, đã cân nhắc Workstation GC." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao Server GC có thể gây vấn đề trong container nhỏ?",
      answer: "Vì nó cấp phát heap theo số core nhìn thấy được, nên một container 512 MB chạy trên máy nhiều core có thể dùng bộ nhớ cao hơn dự kiến. Với container nhỏ và tải vừa phải, Workstation GC thường giảm bộ nhớ đáng kể mà thông lượng không giảm nhiều."
    },
    {
      question: "Large Object Heap gây vấn đề gì?",
      answer: "Object trên 85.000 byte vào thẳng LOH và LOH không được nén mặc định, nên cấp phát mảng lớn liên tục gây phân mảnh. Biểu hiện là OutOfMemoryException dù tổng bộ nhớ vẫn còn trống."
    },
    {
      question: "Hai cảnh báo khi dùng ArrayPool là gì?",
      answer: "Rent có thể trả về mảng lớn hơn kích thước yêu cầu nên phải dùng độ dài thật chứ không dùng buffer.Length. Và quên gọi Return biến tối ưu thành rò rỉ bộ nhớ."
    },
    {
      question: "Rào cản lớn nhất của Native AOT là gì?",
      answer: "Hạn chế về reflection. Nhiều thư viện phổ biến dựa vào reflection và không chạy được dưới AOT, trong đó EF Core bị hạn chế đáng kể. Minimal API đơn giản thì khả thi, ứng dụng CRM đầy đủ thì thường không."
    },
    {
      question: "Khi nào object pooling đáng dùng?",
      answer: "Chỉ khi object đắt để tạo và được dùng rất thường xuyên, và đã có số liệu profiler chứng minh. Với object nhỏ ngắn hạn thì Gen 0 GC đã rất rẻ, nên pooling chỉ thêm độ phức tạp và rủi ro trạng thái sót lại."
    },
    {
      question: "Vì sao các chủ đề trong bài này là bước thứ bảy trở đi?",
      answer: "Vì sáu bước trước gồm sửa N+1, thêm index, bỏ sync-over-async, projection, cache và chạy song song thường cho cải thiện hàng chục tới hàng trăm lần. Tối ưu vi mô khi hệ thống còn nghẽn ở những chỗ đó là tối ưu 1 phần trăm trong khi 90 phần trăm vấn đề vẫn nguyên."
    }
  ]}
/>
