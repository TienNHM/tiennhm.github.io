...
description: "God Service, Anemic Domain Model, Domain phụ thuộc EF Core, controller điều phối nghiệp vụ, và kiến trúc lớp hơn bài toán — dấu hiệu, hậu quả, cách gỡ."
sidebar_position: 11
---
...
**Cách gỡ — không viết lại:** tách dần theo use case. Mỗi lần chạm vào một phương thức, kéo nó ra thành một handler riêng ([bài 16.7](16.7-mediatr-use-case-pipeline.mdx)), rồi để `CrmService` gọi handler đó. Sau vài tháng `CrmService` rỗng và xoá được.

```csharp
// Buoc trung gian — CrmService uy quyen, khong con logic
...
```csharp
// ANTI-PATTERN — entity chi la tui chua du lieu
public class Lead
{
...
// Quy tac nam o day — va o ba cho khac nua
public class LeadService
{
    public async Task ConvertAsync(Guid id)
    {
        ...
        if (lead.Value <= 0) throw new Exception("Gia tri khong hop le");
        lead.Status = "Won";
...
```csharp
// SUA — quy tac song trong entity, khong the lach
...
Vi phạm trực tiếp dependency rule ([bài 16.4](16.4-uncle-bob.mdx)). Hậu quả cụ thể:
...
```csharp
// ANTI-PATTERN — controller lam viec cua use case
[HttpPost("{id}/convert")]
...
Transaction nằm trong pipeline behavior ([bài 16.7](16.7-mediatr-use-case-pipeline.mdx)), email đi qua domain event và chạy **sau khi** commit ([bài 16.9](16.9-in-process.mdx)).

### 16.11.5 — MediatR cho mọi thứ

```csharp
// ANTI-PATTERN — mot lop query chi de goi mot dong EF
...
```csharp
// Doc thuan — goi thang, ngan gon hon, va khong mat gi
[HttpGet("countries")]
...
Crm.Api/
Crm.Api.Contracts/
...cho mot ung dung 5 man hinh CRUD, doi 2 nguoi
```
...
<Checklist
  title="Danh sách rà soát anti-pattern"
  items={[
    { text: "Không có lớp service nào nhận quá 5–6 dependency." },
    { text: "Entity có phương thức nghiệp vụ, không chỉ property get set." },
    { text: "Setter của entity là private, không thể đổi trạng thái từ bên ngoài." },
    { text: "Assembly Domain không tham chiếu EF Core hay ASP.NET Core." },
    { text: "Có kiến trúc test chặn vi phạm ranh giới ngay trong CI." },
    { text: "Controller không gọi nhiều hơn một use case." },
    { text: "Mọi thao tác ghi nhiều bảng đều nằm trong một transaction." },
    { text: "Tác dụng phụ như gửi email chạy sau khi commit, không trong cùng luồng." },
    { text: "Truy vấn đọc đơn giản không bị bọc trong MediatR một cách máy móc." },
    { text: "Số project trong solution tương xứng với độ phức tạp thật." },
    { text: "Mỗi ranh giới đều trả lời được nó giải quyết vấn đề cụ thể gì." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao Anemic Domain Model nguy hiểm hơn God Service?",
      answer: "Vì God Service dễ thấy, còn anemic model trông giống kiến trúc tốt. Bạn có đủ Domain, Application, Infrastructure nhưng quy tắc nghiệp vụ nằm trong các service, nên entity cho phép mọi đoạn code phá invariant. Bạn trả toàn bộ chi phí của kiến trúc phân tầng mà không nhận được lợi ích bảo vệ quy tắc."
    },
    {
      question: "Anemic Domain Model có bao giờ chấp nhận được không?",
      answer: "Có, với CRUD thuần không có quy tắc nghiệp vụ, ví dụ bảng danh mục chỉ có mã và tên. Nó chỉ sai khi bạn đã trả chi phí của kiến trúc phân tầng mà vẫn để quy tắc ở ngoài entity."
    },
    {
      question: "Cách gỡ một God Service ba nghìn dòng mà không viết lại?",
      answer: "Dùng Strangler Fig. Mỗi lần chạm vào một phương thức thì kéo nó ra thành một handler riêng, rồi để service cũ uỷ quyền cho handler đó. Sau vài tháng service cũ rỗng và xoá được, không cần một đợt viết lại lớn."
    },
    {
      question: "Vấn đề nghiêm trọng nhất khi controller điều phối nghiệp vụ là gì?",
      answer: "Không có transaction. Nếu thêm customer thành công nhưng cập nhật lead thất bại thì dữ liệu mâu thuẫn vĩnh viễn, và lần convert sau sẽ tạo customer trùng. Hai vấn đề còn lại là không tái sử dụng được cho background job và không test được nếu không dựng HTTP."
    },
    {
      question: "Khi nào nên dùng MediatR và khi nào gọi thẳng?",
      answer: "Dùng MediatR khi cần pipeline gồm validation, transaction, logging hoặc authorization chạy quanh use case, tức là hầu hết thao tác ghi. Truy vấn đọc đơn giản không cần gì trong số đó thì gọi thẳng, vì ba file cho một dòng EF Core là chi phí thuần."
    },
    {
      question: "Làm sao chặn vi phạm ranh giới một cách tự động?",
      answer: "Viết kiến trúc test bằng NetArchTest.Rules khẳng định assembly Domain không phụ thuộc Microsoft.EntityFrameworkCore hay Microsoft.AspNetCore. Test chạy trong CI và chặn PR vi phạm, hiệu quả hơn nhắc nhau trong code review."
    }
  ]}
/>
