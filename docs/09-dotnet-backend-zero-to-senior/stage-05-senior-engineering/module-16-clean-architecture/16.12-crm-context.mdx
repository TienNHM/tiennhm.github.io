---
title: "16.12 — Liên hệ CRM"
description: "Bốn bounded context của CRM, ranh giới aggregate cụ thể, và ba quyết định kiến trúc thường gây tranh cãi nhất trong dự án thật."
sidebar_position: 12
---
...
<Checklist
  title="Danh sách rà soát kiến trúc CRM"
  items={[
    { text: "Mỗi bounded context có mô hình riêng, không dùng chung entity." },
    { text: "Các context liên kết qua ID, không qua tham chiếu entity." },
    { text: "Aggregate chỉ gộp những thứ có invariant chung thật sự." },
    { text: "Lead và Activity là hai aggregate riêng." },
    { text: "Lead và Customer là hai entity riêng với vòng đời riêng." },
    { text: "Repository riêng cho từng aggregate root, không dùng IRepository chung." },
    { text: "Interface repository chỉ phơi ra thao tác hợp lệ về nghiệp vụ." },
    { text: "Truy vấn đọc dùng projection, không đi qua repository." },
    { text: "Domain event được dịch sang integration event trước khi ra ngoài." },
  ]}
/>
...
Đây chính xác là thứ mà `private set` và `IReadOnlyList` ở [bài 16.5](16.5-ddd-tactics-backend-pragmatic.mdx) ngăn chặn — nhưng `IRepository<OrderItem>` **mở lại** cửa hậu đó, vì nó cho phép nạp và lưu entity con mà không qua root.
...
<FAQSection
  items={[
    {
      question: "Vì sao bốn context có bốn mô hình Customer khác nhau lại đúng?",
      answer: "Vì mỗi context quan tâm khía cạnh khác nhau của cùng một người. Sales cần ngân sách và người ra quyết định, Billing cần mã số thuế và hạn mức. Ép thành một lớp dùng chung tạo entity khổng lồ mà không context nào dùng hết."
    },
    {
      question: "Quy tắc xác định ranh giới aggregate là gì?",
      answer: "Aggregate là ranh giới của tính nhất quán tức thì. Câu hỏi kiểm tra là nếu hai thứ này lệch nhau trong năm giây thì có ai bị thiệt hại không. Nếu không thì chúng thuộc hai aggregate khác nhau."
    },
    {
      question: "Vì sao Lead và Activity không nên chung aggregate?",
      answer: "Vì không có invariant nào nối chúng: một lead có 0 hay 500 activity đều hợp lệ. Gộp lại buộc phải nạp toàn bộ activity chỉ để thêm một dòng, và làm tăng tranh chấp đồng thời qua concurrency token của lead."
    },
    {
      question: "Vì sao Lead và Customer nên là hai entity riêng?",
      answer: "Vì chúng có vòng đời và quy tắc khác nhau. Lead xoá được, gộp được, đánh dấu spam được. Customer có ràng buộc pháp lý và không xoá được khi còn hoá đơn. Gộp lại khiến mọi phương thức phải bắt đầu bằng nếu là lead thì."
    },
    {
      question: "Vì sao không nên dùng IRepository chung?",
      answer: "Vì nó cho truy cập mọi entity kể cả những cái không phải aggregate root, phá vỡ ranh giới aggregate. Và nó thường chỉ là lớp mỏng trên DbContext, vốn đã là repository và unit of work rồi."
    },
    {
      question: "Vì sao truy vấn đọc không nên đi qua repository?",
      answer: "Vì repository phục vụ đường ghi, nơi cần aggregate đầy đủ để áp quy tắc. Đường đọc chỉ cần dữ liệu, và bắt nó qua repository buộc phải nạp entity đầy đủ rồi vứt đi phần lớn."
    }
  ]}
/>
