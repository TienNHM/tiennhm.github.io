...
```csharp
// Exception cho loi NGHIEP VU — chu ky phuong thuc noi doi
...
```csharp
// Result — moi ket qua co the co deu nam trong chu ky
...
```csharp
// Dieu kien "lead dang hoat dong cua tenant" lap o nhieu cho
...
```csharp
// 1. Owned entity — luu thanh nhieu cot
...
// 2. Value converter — luu thanh MOT cot
b.Property(l => l.Email)
 .HasConversion(e => e.Value, v => new EmailAddress(v))
 .HasMaxLength(320);

// 3. Complex type (EF Core 8+) — khong bi theo doi nhu entity
...
// Trinh bien dich chan loi nay — voi Guid thuan thi khong
