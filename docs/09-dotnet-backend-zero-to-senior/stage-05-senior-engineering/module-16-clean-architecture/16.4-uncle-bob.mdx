...
Interface cuối đáng chú ý: `DateTime.UtcNow` cũng là **phụ thuộc vào thế giới bên ngoài**, và nó khiến test không lặp lại được ([bài 7.2](../../stage-02-csharp-professional/module-07-dependency-injection/7.2-di-problem-scope.mdx)). .NET 8 có `TimeProvider` sẵn, nên không cần tự định nghĩa nữa.
...
Tổ chức theo layer, đúng mẫu ở [bài 7.7](../../stage-02-csharp-professional/module-07-dependency-injection/7.7-program-cs-and-webapplicationbuilder.mdx):
...
Bạn mất projection, `Include`, `AsNoTracking`, phân trang hiệu quả — toàn bộ nội dung [bài 13.6](../../stage-04-database-production/module-13-entity-framework-core/13.6-query-patterns.mdx) và [13.8](../../stage-04-database-production/module-13-entity-framework-core/13.8-performance.mdx).
...
`ExecuteUpdateAsync` không diễn đạt được qua một repository thuần khiết, nên cập nhật 50.000 dòng phải nạp entity — chậm gấp hàng chục lần ([bài 13.8](../../stage-04-database-production/module-13-entity-framework-core/13.8-performance.mdx)).
...
Đường ghi cần trừu tượng vì nó chứa quy tắc; đường đọc cần tự do vì nó chỉ cần nhanh. Đây cũng là kết luận ở [bài 13.10](../../stage-04-database-production/module-13-entity-framework-core/13.10-unit-of-work-and-repository-pattern.mdx).
...
Và nhớ lý do gốc ở [bài 16.2](16.2-vi-sao-crm-to-ra-la-luc-kien-truc-tra-gia.mdx): mục tiêu là **chi phí thay đổi thấp**. Nếu thuần khiết làm chi phí tăng, nó đang đi ngược mục tiêu — dù nó "đúng chuẩn".
