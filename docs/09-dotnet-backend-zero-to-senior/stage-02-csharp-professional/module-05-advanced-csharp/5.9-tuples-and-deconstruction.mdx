...
Deconstruction ghép rất tốt với pattern matching ở [bài 4.9](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.9-pattern-matching):
...
Dòng thứ ba đáng chú ý: `ValueTuple` cài đặt sẵn `Equals` và `GetHashCode` theo giá trị, nên nó dùng làm khoá `Dictionary` được ngay — không gặp vấn đề đã nêu ở [bài 1.6](/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-01-programming-logic/1.6-arrays-and-lists).
...
**Gợi ý.** Bài này nối tiếp bài tập ở [bài 1.5](../../stage-01-foundation/module-01-programming-logic/1.5-functions-methods.mdx). Điểm mới ở đây là tìm ra **ngoại lệ chính đáng** cho quy tắc "đừng dùng `out`".
...
Thuộc tính `[NotNullWhen(true)]` nói với trình biên dịch rằng khi hàm trả `true` thì `kh` chắc chắn khác `null` — nhờ vậy bạn dùng `kh` ngay sau `if` mà không bị cảnh báo. Đây chính là kỹ thuật ở [bài 5.7](5.7-nullable-reference-types.mdx).
...
Cách này cho phép đặt tên kiểu, thêm phương thức kiểm tra, và tránh nhầm với một tuple `(int, int)` khác mang ý nghĩa khác — đúng tinh thần value object ở [bài 4.8](../module-04-csharp-core/4.8-records-and-value-objects.mdx).

**Lưu ý về tuple làm khoá.** Cả `ValueTuple` lẫn `record struct` đều đã có `Equals` và `GetHashCode` theo giá trị sẵn, nên chúng hoạt động đúng làm khoá ngay mà không cần viết thêm gì — khác hẳn với `class` thường, như [bài 1.6](../../stage-01-foundation/module-01-programming-logic/1.6-arrays-and-lists.mdx) đã chứng minh.
