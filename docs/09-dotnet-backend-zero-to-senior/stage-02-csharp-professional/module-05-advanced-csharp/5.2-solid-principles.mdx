...
Ba nguyên tắc cuối đã được đào ở Module 4: [Liskov ở bài 4.4](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.4-inheritance-and-polymorphism), còn [Interface Segregation và Dependency Inversion ở bài 4.5](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.5-interface). Bài này tập trung vào hai cái đầu và vào phần phê phán.
...
Đây là chỗ nối với cảnh báo ở [bài 4.6 về trừu tượng hoá quá sớm](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.6-abstraction).

### 5.2.4 — Ba nguyên tắc còn lại, tóm tắt

**L — Liskov:** lớp con phải dùng thay lớp cha được ở mọi chỗ, mà code gọi không cần biết. Ba dấu hiệu vi phạm: lớp con ném ngoại lệ cho phương thức cha hỗ trợ, lớp con override rồi không làm gì, và code gọi phải kiểm tra kiểu. Chi tiết kèm ví dụ hình vuông–hình chữ nhật ở [bài 4.4](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.4-inheritance-and-polymorphism).
...
Một `IFooService` chỉ có `FooService` cài đặt, không vượt ranh giới hạ tầng nào, không cần thay bằng bản giả — đó là nghi lễ. Ba câu hỏi quyết định nằm ở [bài 4.5](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.5-interface).
...
**Gợi ý.** Bài này mở rộng bài tập ở [bài 4.10](../module-04-csharp-core/4.10-common-mistakes.mdx). Điểm mới ở đây là yêu cầu bạn tìm được **một phản ví dụ**: một class lớn nhưng không nên tách.
...
```csharp
public decimal TinhPhi(LoaiThanhToan loai, decimal soTien) => loai switch
{
    LoaiThanhToan.TheTinDung => soTien * 0.025m,
    LoaiThanhToan.ChuyenKhoan => 11_000m,
    LoaiThanhToan.ViDienTu => soTien * 0.015m,
    LoaiThanhToan.TienMat => 0m,
...
```csharp
public interface IPhiThanhToan
{
    LoaiThanhToan Loai { get; }
    decimal Tinh(decimal soTien);
}

public class PhiTheTinDung : IPhiThanhToan
{
    public LoaiThanhToan Loai => LoaiThanhToan.TheTinDung;
    public decimal Tinh(decimal soTien) => soTien * 0.025m;
}
// ... ba lớp nữa

public class BoTinhPhi
{
    private readonly IReadOnlyDictionary<LoaiThanhToan, IPhiThanhToan> _map;

    public BoTinhPhi(IEnumerable<IPhiThanhToan> cacChienLuoc) =>
        _map = cacChienLuoc.ToDictionary(x => x.Loai);

    public decimal Tinh(LoaiThanhToan loai, decimal soTien) =>
        _map.TryGetValue(loai, out var c)
            ? c.Tinh(soTien)
            : throw new ArgumentOutOfRangeException(nameof(loai));
}
```
...
**Điểm dễ bỏ sót về đăng ký.** Với mẫu chiến lược, đừng quên rằng thêm một lớp thôi chưa đủ — phải đăng ký nó vào DI. Quên đăng ký thì `TryGetValue` trả `false` và ném ngoại lệ **lúc chạy**, thay vì lỗi biên dịch như `switch` vét cạn ở [bài 4.9](../module-04-csharp-core/4.9-pattern-matching.mdx). Có thể giảm rủi ro bằng cách quét assembly tự động:

```csharp
services.Scan(scan => scan.FromAssemblyOf<IPhiThanhToan>()
    .AddClasses(c => c.AssignableTo<IPhiThanhToan>())
...
public class LeadService : ILeadService
{
    ...
    public Task<Lead> TaoAsync(Lead lead, CancellationToken ct)
        => _repo.ThemAsync(lead, ct);       // chuyển tiếp, không thêm gì
}
```
...
public record CreateLeadRequest(string Ten, string Email, decimal GiaTri);
...
{
    public async Task<Result<LeadDto>> XuLyAsync(CreateLeadRequest req, CancellationToken ct)
    {
        var lead = Lead.Tao(req.Ten, new Email(req.Email), req.GiaTri);
        if (lead.IsFailure) return Result<LeadDto>.That(lead.Loi!);

        await repo.ThemAsync(lead.Value, ct);
        return Result<LeadDto>.Ok(LeadDto.Tu(lead.Value));
    }
}
```
...
Thêm một tính năng là thêm một file, ở một chỗ. Đây là kiến trúc phân lát dọc, chủ đề của [bài 16.5](../../stage-05-senior-engineering/module-16-clean-architecture/16.5-vertical-slice-architecture.mdx).
