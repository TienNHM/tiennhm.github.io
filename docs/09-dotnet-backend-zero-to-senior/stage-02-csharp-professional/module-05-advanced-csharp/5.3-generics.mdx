...
1. **Đo giá của boxing.** Thêm một triệu số nguyên vào `ArrayList` và vào `List<int>`, đo thời gian và bộ nhớ bằng `Stopwatch` cùng `GC.GetTotalMemory`. Ghi lại tỉ lệ.

2. **Tự chứng minh covariance.** Viết `class Animal`, `class Dog : Animal`. Thử gán `List<Dog>` cho `List<Animal>` và ghi lỗi biên dịch. Làm lại với `IEnumerable<Dog>` và giải thích vì sao nó hợp lệ.

3. **`static` theo kiểu.** Cài `Counter<T>` ở mục 5.3.5, gán giá trị khác nhau cho ba kiểu đóng và in ra. Giải thích kết quả bằng lời của bạn.
