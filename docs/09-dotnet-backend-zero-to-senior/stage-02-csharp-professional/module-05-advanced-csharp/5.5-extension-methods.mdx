...
Hai chi tiết đáng chú ý: **trả về chính đối tượng** để nối chuỗi được, và đặt lớp extension **trong cùng project với thứ nó đăng ký** — nhờ vậy `Program.cs` không cần biết `SqlOrderRepository` tồn tại, đúng tinh thần `internal` ở [bài 4.7](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.6-access-modifiers).
...
Extension không truy cập được thành viên `private`, nên logic nghiệp vụ nằm ở đó sẽ chỉ dùng được dữ liệu công khai — đẩy bạn về phía [anemic model](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.2-encapsulation).
