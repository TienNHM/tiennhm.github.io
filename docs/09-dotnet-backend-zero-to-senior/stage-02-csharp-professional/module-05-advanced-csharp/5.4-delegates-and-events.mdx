---
title: "5.4 — 4. Delegates và Events"
description: "Delegate là con trỏ hàm có kiểu; event là delegate bị giới hạn quyền. Kèm rò rỉ bộ nhớ do quên gỡ đăng ký, và vì sao event không hợp với xử lý bất đồng bộ."
sidebar_position: 4
---
...
<Checklist
  title="Danh sách rà soát delegate và event"
  items={[
    { text: "Không có delegate công khai nào đáng lẽ phải là event." },
    { text: "Mọi lời gọi event đều dùng ?.Invoke thay vì kiểm tra null rồi gọi." },
    { text: "Mỗi += đều có một -= tương ứng ở đâu đó." },
    { text: "Lambda dùng làm handler được giữ trong một biến để còn gỡ được." },
    { text: "Không có handler async nào gắn vào event (tránh async void)." },
    { text: "Đối tượng ngắn hạn đăng ký vào đối tượng sống lâu đều cài IDisposable." },
    { text: "Trong ASP.NET Core, cân nhắc domain event bất đồng bộ thay cho event .NET cổ điển." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Delegate là gì và liên quan gì tới lambda?",
      answer: "Delegate là con trỏ hàm có kiểu: một biến giữ được một phương thức và gọi nó sau. Func, Action, Predicate đều là delegate dựng sẵn. Mọi biểu thức lambda bạn truyền cho LINQ đều được biên dịch thành một delegate, ví dụ orders.Where với điều kiện chính là truyền một Func nhận Order trả bool."
    },
    {
      question: "event khác gì với một delegate công khai?",
      answer: "event giới hạn quyền của bên ngoài: chỉ được cộng bằng và trừ bằng để đăng ký hoặc gỡ, không gán đè được và không tự kích hoạt được. Với delegate công khai, người dùng lớp bạn có thể gán null xoá sạch handler của người khác, hoặc tự phát một sự kiện giả."
    },
    {
      question: "Vì sao quên gỡ đăng ký event lại gây rò rỉ bộ nhớ?",
      answer: "Vì khi đăng ký một phương thức instance làm handler, delegate tạo ra giữ tham chiếu tới this. Nếu đối tượng phát sự kiện sống lâu — ví dụ một singleton — thì nó giữ tham chiếu tới mọi đối tượng ngắn hạn đã đăng ký, và GC không thu hồi được cái nào. Bộ nhớ tăng đều và chỉ lộ ra sau vài giờ chạy."
    },
    {
      question: "Vì sao gỡ đăng ký bằng một lambda viết lại y hệt không có tác dụng?",
      answer: "Vì mỗi biểu thức lambda tạo ra một đối tượng delegate mới, khác với cái đã đăng ký dù nội dung giống hệt. Muốn gỡ được thì phải giữ lambda đó trong một biến lúc đăng ký và dùng chính biến đó khi gỡ."
    },
    {
      question: "Multicast delegate có gì cần lưu ý?",
      answer: "Hai điều. Một handler ném ngoại lệ thì các handler sau không chạy và không có cơ chế thử tiếp. Và với Func, chỉ giá trị trả về của handler cuối cùng được giữ lại, kết quả của những cái trước bị vứt đi. Vì vậy multicast hợp với thông báo chứ không hợp với tính toán."
    },
    {
      question: "Vì sao event không hợp với bất đồng bộ?",
      answer: "Vì handler async gắn vào event trở thành async void: không await được nên bên phát không biết nó xong chưa, và ngoại lệ bên trong không bắt được ở chỗ gọi mà làm sập tiến trình. Trong backend hiện đại nên thay bằng cơ chế domain event trả Task, như cách MediatR làm."
    }
  ]}
/>
...
- [Events overview](https://learn.microsoft.com/en-us/dotnet/csharp/events-overview) — quy ước `EventHandler<T>`
