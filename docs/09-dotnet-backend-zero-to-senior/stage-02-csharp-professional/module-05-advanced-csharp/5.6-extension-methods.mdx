...
description: "Extension method thêm phương thức cho kiểu bạn không sở hữu. Kèm quy tắc độ ưu tiên khi trùng tên, vì sao gọi được trên null, và ba dấu hiệu bạn đang lạm dụng."
sidebar_position: 6
---
...
Hai chi tiết đáng chú ý: **trả về chính đối tượng** để nối chuỗi được, và đặt lớp extension **trong cùng project với thứ nó đăng ký** — nhờ vậy `Program.cs` không cần biết `SqlOrderRepository` tồn tại, đúng tinh thần `internal` ở [bài 4.7](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.7-access-modifiers).
...
Extension không truy cập được thành viên `private`, nên logic nghiệp vụ nằm ở đó sẽ chỉ dùng được dữ liệu công khai — đẩy bạn về phía [anemic model](/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.3-encapsulation).
...
<Checklist
  title="Danh sách rà soát extension method"
  items={[
    { text: "Chỉ viết extension cho kiểu bạn KHÔNG sở hữu." },
    { text: "Extension là hàm thuần, không gọi database hay HTTP bên trong." },
    { text: "Extension nhận null đều khai tham số nullable và xử lý tường minh." },
    { text: "Không đặt extension trong namespace System hay namespace gốc của dự án." },
    { text: "Tên extension không trùng với tên có khả năng xuất hiện trong chính kiểu đó." },
    { text: "Extension đăng ký DI trả về chính đối tượng để nối chuỗi." },
    { text: "Lớp extension đăng ký DI nằm cùng project với thứ nó đăng ký." },
  ]}
/>
...
Đây là cách **dạy trình biên dịch** hiểu hàm của bạn, chủ đề chính của [bài 5.7](5.7-nullable-reference-types.mdx).
...
1. **Mỗi dự án tự khai báo cách đăng ký của nó.** Thêm một repository vào `Crm.Infrastructure` chỉ cần sửa file trong chính dự án đó — `Program.cs` và các dự án khác không đụng tới. Đây cũng là cách để dự án hạ tầng **không lộ** các lớp cài đặt ra ngoài, đúng như [bài 4.7](../module-04-csharp-core/4.7-access-modifiers.mdx) đã nêu: chúng có thể là `internal`, chỉ extension method là `public`.

2. **`Program.cs` thành bản đồ hệ thống.** Đọc 18 dòng là biết ứng dụng gồm những phần nào. Đọc 94 dòng đăng ký thì không.

3. **Giảm xung đột Git.** Với `Program.cs` dài, mọi người thêm dịch vụ đều sửa cùng một file, nên xung đột liên tục — đúng vấn đề ở [bài 3.8](../../stage-01-foundation/module-03-git-developer-workflow/3.8-git-conflicts.mdx). Tách ra thì mỗi người sửa file của tầng mình.
...
Lưu ý ở đây thứ tự **vẫn quan trọng** như [bài 8.3](../../stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.3-request-pipeline-and-middleware.mdx) trình bày — gom vào extension không làm thay đổi điều đó, chỉ đưa nó vào một chỗ duy nhất để dễ rà soát và ghi chú lý do.
...
<FAQSection
  items={[
    {
      question: "Extension method thực chất là gì?",
      answer: "Là một phương thức static trong một class static, với tham số đầu tiên có từ khoá this. Trình biên dịch dịch lời gọi kiểu input.IsValidEmail thành StringExtensions.IsValidEmail(input). Nó chỉ là đường cú pháp, và điều đó giải thích mọi hành vi còn lại của nó."
    },
    {
      question: "Khi extension trùng tên với phương thức của class thì sao?",
      answer: "Phương thức của class luôn thắng; extension chỉ được xét khi không tìm thấy phương thức phù hợp trên kiểu đó. Hệ quả cần cảnh giác: nếu tác giả thư viện thêm phương thức trùng tên ở phiên bản sau, extension của bạn âm thầm ngừng được gọi — code vẫn biên dịch nhưng hành vi đổi."
    },
    {
      question: "Vì sao gọi extension trên biến null lại không ném ngoại lệ?",
      answer: "Vì đó thực chất là một lời gọi static với đối số null, không có lời gọi trên instance nào cả. Điều này hữu ích để viết hàm kiểm tra null gọn gàng, nhưng cũng nguy hiểm vì người đọc tưởng đã chắc chắn khác null. Nếu extension có thể nhận null thì nên khai tham số là kiểu nullable và xử lý tường minh."
    },
    {
      question: "Vì sao LINQ cần using System.Linq?",
      answer: "Vì toàn bộ LINQ là extension method nằm trong lớp System.Linq.Enumerable. Không có using thì chúng không nằm trong phạm vi và trình biên dịch báo không tìm thấy phương thức. Đây cũng là lý do LINQ hoạt động với mọi kiểu cài đặt IEnumerable, kể cả kiểu bạn tự viết."
    },
    {
      question: "Vì sao không nên viết extension cho kiểu mình sở hữu?",
      answer: "Vì extension không truy cập được thành viên private, nên logic đặt ở đó chỉ dùng được dữ liệu công khai, đẩy bạn về phía anemic model. Kiểu của chính bạn thì thêm phương thức vào thẳng class, nơi nó truy cập được trạng thái nội bộ và giữ được bất biến."
    },
    {
      question: "Nên đặt extension ở namespace nào?",
      answer: "Namespace của chính module đó, ví dụ MyApp.Orders.Extensions, để người dùng chọn using một cách có ý thức. Đặt vào namespace System hay namespace gốc khiến mọi file trong solution tự động thấy chúng, kể cả nơi không cần, và người đọc không đoán được phương thức đến từ đâu."
    }
  ]}
/>
