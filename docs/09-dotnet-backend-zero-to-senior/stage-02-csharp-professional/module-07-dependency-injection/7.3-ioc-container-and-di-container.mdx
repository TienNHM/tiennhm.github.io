...
1. **Đăng ký trùng.** Đăng ký `IEmailService` hai lần với hai implementation khác nhau. Gọi `GetRequiredService<IEmailService>()` và `GetServices<IEmailService>()`, ghi lại kết quả của cả hai.
...
3. **Nhiều constructor.** Viết một lớp có hai constructor, một nhận `ILogger<T>` và một nhận `ILogger<T>` cùng `IEmailService`, trong đó chỉ đăng ký `ILogger`. Đoán xem container chọn cái nào, rồi kiểm chứng.
