---
title: "7.7 — 6. Options Pattern"
description: "Bind cấu hình vào POCO type-safe: IOptions, IOptionsSnapshot, IOptionsMonitor khác nhau ra sao, ValidateOnStart, named options và secret."
sidebar_position: 7
---
...
Tiêm `IOptionsSnapshot<T>` vào một Singleton là **captive dependency** ([bài 7.5](7.4-service-lifetimes.mdx)) — nó bị giam và không bao giờ đọc lại nữa, im lặng.
...
Đây cũng là câu trả lời cho vấn đề ở [bài 7.7](7.6-program-cs-and-webapplicationbuilder.mdx): cần một service để cấu hình một service khác mà **không** gọi `BuildServiceProvider()`. `IConfigureOptions<T>` làm điều tương tự cho việc thiết lập giá trị:
...
<Checklist
  title="Danh sách rà soát Options Pattern"
  items={[
    { text: "Không còn service nào tiêm IConfiguration để đọc giá trị lẻ." },
    { text: "Mọi AddOptions đều có ValidateOnStart()." },
    { text: "Mỗi lớp settings có const SectionName nằm trong chính nó." },
    { text: "Không tiêm IOptionsSnapshot vào service Singleton." },
    { text: "Cấu hình cần reload trong Singleton dùng IOptionsMonitor.CurrentValue." },
    { text: "Callback OnChange có chống trùng và IDisposable của nó được giữ lại." },
    { text: "Luật liên quan nhiều thuộc tính dùng Validate hoặc IValidateOptions." },
...
**Gợi ý.** `IOptionsSnapshot<T>` được đăng ký là **`Scoped`**. Tiêm một `Scoped` vào một `Singleton` chính là captive dependency ở [bài 7.5](7.4-service-lifetimes.mdx).
...
<FAQSection
  items={[
    {
      question: "Options pattern giải quyết vấn đề gì của IConfiguration?",
      answer: "Bốn vấn đề: gõ sai khoá chỉ nhận null chứ không có lỗi biên dịch, mọi giá trị đều là string nên phải tự ép kiểu, không biết cấu hình đủ hay chưa cho tới khi code chạy tới, và unit test phải dựng IConfiguration giả. Options bind cấu hình vào một lớp POCO có kiểu và kiểm tra được ngay lúc khởi động."
    },
    {
      question: "ValidateOnStart quan trọng ở chỗ nào?",
      answer: "Nó chạy validation lúc khởi động thay vì lúc IOptions được resolve lần đầu. Không có nó thì ứng dụng khởi động bình thường với cấu hình thiếu, và bạn chỉ phát hiện khi có người gọi đúng chức năng đó, thường là trên production."
    },
    {
      question: "Ba interface IOptions khác nhau thế nào?",
      answer: "IOptions là Singleton, đọc một lần và không reload. IOptionsSnapshot là Scoped, đọc lại mỗi request nên không tiêm được vào Singleton. IOptionsMonitor là Singleton nhưng CurrentValue luôn là bản mới nhất. Chọn theo cấu hình có đổi khi đang chạy không, và lifetime của service dùng nó."
    },
    {
      question: "Tiêm IOptionsSnapshot vào Singleton thì sao?",
      answer: "Đó là captive dependency: snapshot bị giam trong Singleton và không bao giờ đọc lại cấu hình nữa. Nó xảy ra im lặng, không có lỗi nào, chỉ là cấu hình trông như không đổi. Với Singleton thì dùng IOptionsMonitor."
    },
    {
      question: "Khi luật kiểm tra cần tới service khác thì làm sao?",
      answer: "Dùng IValidateOptions<T>, vì nó được resolve từ container nên tiêm được dependency. Đây cũng là câu trả lời cho nhu cầu dùng một service để cấu hình một service khác mà không phải gọi BuildServiceProvider thủ công; IConfigureOptions<T> làm điều tương tự cho việc thiết lập giá trị."
    },
    {
      question: "Secret nên để ở đâu?",
      answer: "Không để trong appsettings.json vì file đó nằm trong Git. Development dùng dotnet user-secrets, production dùng biến môi trường với hai dấu gạch dưới thay cho dấu hai chấm, Azure Key Vault hoặc secret của hệ thống điều phối. Điểm hay là code không đổi vì mọi nguồn đều bind vào cùng một lớp settings."
    }
  ]}
/>
