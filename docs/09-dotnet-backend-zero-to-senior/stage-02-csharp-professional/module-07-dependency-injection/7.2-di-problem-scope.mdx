...
description: "Vì sao tự new dependency trong constructor làm code không test được, không đổi được implementation và rò rỉ tài nguyên — và DI sửa chính xác điều gì."
sidebar_position: 2
---
...
<Checklist
  title="Danh sách rà soát phụ thuộc"
  items={[
    { text: "Không có lời gọi new nào tới service có I/O bên trong lớp nghiệp vụ." },
    { text: "Không có connection string hay host name nào nằm trong mã nguồn." },
    { text: "Mọi phụ thuộc đều xuất hiện trong constructor, không ẩn đâu đó." },
    { text: "Lớp nghiệp vụ phụ thuộc vào interface, và không tự chọn implementation." },
    { text: "Không tiêm entity hay DTO vào container." },
    { text: "Mọi chỗ dùng DateTime.Now trong logic có thể test đã chuyển sang TimeProvider." },
    { text: "Constructor nào quá 5 tham số đều được xem lại xem lớp có làm quá nhiều việc không." },
  ]}
/>
...
3. **Bọc thêm hành vi mà không sửa gì.** Thêm thử lại, ghi log, giới hạn tần suất bằng decorator như [bài 7.6](7.6-registering-services.mdx) trình bày.
...
**Gợi ý.** Bài này lặp lại ý ở [bài 1.5](../../stage-01-foundation/module-01-programming-logic/1.5-functions-methods.mdx) và [bài 4.5](../module-04-csharp-core/4.5-interface.mdx), nhưng ở đây trọng tâm là **tiêm qua DI** thay vì truyền tham số.
...
**Vì sao tiêm qua DI tốt hơn truyền tham số.** Với hàm nhỏ thì truyền `DateTime` làm tham số là đủ, như [bài 1.5](../../stage-01-foundation/module-01-programming-logic/1.5-functions-methods.mdx) đã nêu. Nhưng khi chuỗi gọi sâu — controller gọi service gọi repository — truyền tay qua mọi tầng là không khả thi. DI giải quyết đúng vấn đề đó: khai báo một lần ở constructor, container lo phần còn lại.
...
<FAQSection
  items={[
    {
      question: "Vấn đề lớn nhất của việc new dependency trong constructor là gì?",
      answer: "Không viết được unit test. Không có cách nào chen vào giữa để thay thế dependency, nên test cần database thật, SMTP thật và sẽ gửi email tới địa chỉ trong dữ liệu test. Đây là lý do nhiều codebase cũ bị coi là không test được: không phải thiếu thời gian mà là kiến trúc chặn lại."
    },
    {
      question: "Thêm interface có đủ để giải quyết vấn đề không?",
      answer: "Không. Nếu kiểu của field là interface nhưng constructor vẫn gọi new một implementation cụ thể thì lớp vẫn tự quyết định implementation nào được dùng, và cả năm vấn đề vẫn còn nguyên. Thứ thật sự thay đổi là ai gọi new, chứ không phải có interface hay không."
    },
    {
      question: "Phụ thuộc ẩn nghĩa là gì?",
      answer: "Là khi constructor không cho biết lớp thật sự cần những gì. Nhìn new CustomerService bạn nghĩ nó không cần gì, nhưng thực tế nó cần SQL Server, cần SMTP và cần quyền ghi. Constructor nói dối, và bạn chỉ biết sự thật khi chạy. Với DI thì đọc constructor là biết ngay."
    },
    {
      question: "Constructor có 9 tham số có phải lỗi của DI không?",
      answer: "Không, đó là DI đang cho bạn thấy lớp này làm quá nhiều việc. Trước khi có DI thì vấn đề đó vẫn tồn tại nhưng bị giấu đi. Cách chữa là tách lớp, không phải giấu bớt phụ thuộc."
    },
    {
      question: "Khi nào không nên dùng DI?",
      answer: "Với kiểu chỉ chứa dữ liệu như entity và DTO, với kiểu chỉ có một implementation và sẽ mãi như vậy như StringBuilder hay List, với hàm thuần không có tác dụng phụ, và với script hay tool nhỏ dùng một lần. Tiêu chí thực dụng là có I/O, có thời gian, có ngẫu nhiên hoặc có thể thay thế thì tiêm; ngoài ra cứ new."
    },
    {
      question: "Vì sao DateTime.Now là vấn đề?",
      answer: "Vì nó khiến test không lặp lại được: cùng một bài test cho kết quả khác nhau tuỳ vào lúc bạn chạy nó. Đây là dạng phụ thuộc vào thời gian, và .NET 8 có TimeProvider để tiêm nó vào như một dependency bình thường."
    }
  ]}
/>
