...
description: "Keyed services: đăng ký nhiều implementation cùng interface theo khoá, FromKeyedServices, IKeyedServiceProvider, và khi nào strategy pattern tốt hơn."
sidebar_position: 9
---
...
| Tình huống | Dùng gì |
|---|---|
| Cần **chạy hết** các implementation (validator, handler) | `IEnumerable<T>`, không khoá — [bài 7.6](7.6-registering-services.mdx) |
| Implementation chọn theo **môi trường**, cố định lúc khởi động | `if (env.IsDevelopment())` trong `Program.cs` — đơn giản hơn nhiều |
| Mỗi implementation tự biết nó xử lý được gì | Strategy pattern: `IEnumerable<IPaymentStrategy>` với `CanHandle(type)` |
...
<Checklist
  title="Danh sách rà soát keyed services"
  items={[
    { text: "Khoá dùng enum hoặc const, không phải chuỗi rải rác trong code." },
    { text: "Không dùng keyed services cho trường hợp cần chạy hết mọi implementation." },
    { text: "Biết rằng IEnumerable<T> không nhìn thấy service có khoá." },
    { text: "Chọn theo dữ liệu lúc chạy được gói sau một interface hẹp, không lộ IServiceProvider." },
    { text: "Lựa chọn theo môi trường dùng if trong Program.cs thay vì keyed services." },
    { text: "Logic chọn phức tạp hơn một khoá thì dùng strategy pattern với CanHandle." },
    { text: "KeyedService.AnyKey chỉ dùng có chủ đích, không để che lỗi gõ sai khoá." },
  ]}
/>
...
{
    public Task GuiAsync(string kenh, string noiDung, CancellationToken ct)
        => factory(kenh).GuiAsync(noiDung, ct);
}
```
...
```csharp
public enum Kenh { Email, Sms, Zalo }

builder.Services.AddKeyedScoped<INotificationSender, EmailSender>(Kenh.Email);
builder.Services.AddKeyedScoped<INotificationSender, SmsSender>(Kenh.Sms);
builder.Services.AddKeyedScoped<INotificationSender, ZaloSender>(Kenh.Zalo);

// Dùng — chọn lúc chạy
public class NotifyService(IServiceProvider sp)
{
    public Task GuiAsync(Kenh kenh, string noiDung, CancellationToken ct)
        => sp.GetRequiredKeyedService<INotificationSender>(kenh).GuiAsync(noiDung, ct);
}

// Hoặc chọn lúc biên dịch, khi biết trước cần kênh nào
public class WelcomeService([FromKeyedServices(Kenh.Email)] INotificationSender sender) { }
```
...
**Dùng `enum` làm khoá thay vì chuỗi** là cải thiện đáng kể: nó loại bỏ lỗi gõ sai, và cho phép `switch` vét cạn ở những chỗ khác trong hệ thống như [bài 4.9](../module-04-csharp-core/4.9-pattern-matching.mdx) trình bày.

**Hạn chế của keyed services.** Cách dùng `IServiceProvider` ở trên đưa Service Locator quay lại — đúng thứ mà [bài 7.10](7.10-anti-patterns.mdx) cảnh báo. Chữ ký `NotifyService(IServiceProvider sp)` không cho biết lớp này cần gì.
...
{
    public async Task<Result> XuLyAsync(Order o, CancellationToken ct)
    {
        foreach (var v in validators)                    // chỉ chạy validator KHÔNG khoá
        {
            var kq = await v.KiemTraAsync(o, ct);
            if (!kq.HopLe) return Result.That(kq.Loi);
        }
        // ...
    }
}
...
```csharp
builder.Services.AddScoped<EmailSender>();
builder.Services.AddKeyedScoped<INotificationSender>(Kenh.Email, (sp, _) => sp.GetRequiredService<EmailSender>());
...
1. **Khoá so sánh bằng `Equals`.** Dùng `enum`, `string`, hoặc `record` đều được. Dùng `class` thường thì gặp đúng vấn đề ở [bài 1.6](../../stage-01-foundation/module-01-programming-logic/1.6-arrays-and-lists.mdx).
...
```csharp
public enum PhuongThuc { TheTinDung, ChuyenKhoan, ViDienTu }

builder.Services.AddKeyedScoped<IPaymentGateway, TheTinDungGateway>(PhuongThuc.TheTinDung);
builder.Services.AddKeyedScoped<IPaymentGateway, ChuyenKhoanGateway>(PhuongThuc.ChuyenKhoan);
builder.Services.AddKeyedScoped<IPaymentGateway, ViDienTuGateway>(PhuongThuc.ViDienTu);

public class PaymentService(IPaymentGatewayResolver resolver)
{
    public Task<Result> ThanhToanAsync(Order o, PhuongThuc pt, CancellationToken ct)
        => resolver.Lay(pt).ThanhToanAsync(o, ct);
}
```
...
public interface IPaymentGateway
{
    ...
    Task<Result> ThanhToanAsync(Order o, CancellationToken ct);
}

public class TheTinDungGateway : IPaymentGateway
{
    public bool CoTheXuLy(Order o) => o.PhuongThuc == PhuongThuc.TheTinDung && o.Total <= 50_000_000m;
    // ...
}

builder.Services.AddScoped<IPaymentGateway, TheTinDungGateway>();
builder.Services.AddScoped<IPaymentGateway, ChuyenKhoanGateway>();
builder.Services.AddScoped<IPaymentGateway, ViDienTuGateway>();

public class PaymentService(IEnumerable<IPaymentGateway> gateways)
{
    public Task<Result> ThanhToanAsync(Order o, CancellationToken ct)
    {
        var gw = gateways.FirstOrDefault(g => g.CoTheXuLy(o))
                 ?? throw new InvalidOperationException("Không có cổng thanh toán phù hợp");
        return gw.ThanhToanAsync(o, ct);
    }
}
```
...
<FAQSection
  items={[
    {
      question: "Keyed services giải quyết vấn đề gì?",
      answer: "Trước .NET 8, container .NET không có cách chính thức để nói cho tôi interface này nhưng là implementation cụ thể kia, nên mọi người phải tự chế Func factory hoặc đổi sang Autofac. Keyed services cho phép đăng ký nhiều implementation cùng interface theo khoá và lấy ra bằng FromKeyedServices."
    },
    {
      question: "Nên dùng string hay enum làm khoá?",
      answer: "Enum, vì khoá có thể là bất kỳ object nào có Equals và GetHashCode đúng. Enum cho bạn kiểm tra lúc biên dịch và đổi tên an toàn, còn chuỗi thì gõ sai chỉ phát hiện lúc chạy."
    },
    {
      question: "Chọn implementation theo dữ liệu lúc chạy thì làm sao?",
      answer: "Attribute cần hằng số lúc biên dịch nên phải dùng GetRequiredKeyedService từ IServiceProvider. Về hình thức đó là service locator, nhưng khác ở chỗ lớp này chỉ resolve một kiểu duy nhất đã biết và việc chọn khoá là nghiệp vụ thật của nó. Nên gói lại sau một interface hẹp để phần còn lại của hệ thống không thấy IServiceProvider."
    },
    {
      question: "Vì sao IEnumerable<T> không thấy service có khoá?",
      answer: "Vì service có khoá và không khoá nằm ở hai không gian tách biệt trong container. Muốn lấy cả tập có khoá thì phải dùng GetKeyedServices với một khoá cụ thể, nghĩa là nhiều service cùng chung một khoá."
    },
    {
      question: "Khi nào không nên dùng keyed services?",
      answer: "Ba trường hợp. Khi cần chạy hết mọi implementation như validator hay handler thì dùng IEnumerable không khoá. Khi implementation chọn theo môi trường và cố định lúc khởi động thì một câu if trong Program.cs đơn giản hơn. Và khi logic chọn phức tạp hơn một khoá thì dùng strategy pattern."
    },
    {
      question: "Strategy pattern hơn keyed services ở điểm nào?",
      answer: "Implementation tự khai báo nó xử lý được gì qua phương thức CanHandle, thay vì nơi gọi phải biết khoá. Thêm một trường hợp mới chỉ cần thêm một lớp và một dòng đăng ký, không đụng vào lớp điều phối. Với logic chọn phức tạp thì mẫu này rõ ràng hơn."
    }
  ]}
/>
