---
title: "7.6 — 5. Program.cs và WebApplicationBuilder"
...
```mermaid
flowchart LR
    A[CreateBuilder] --> B["Giai đoạn 1<br/>builder.Services.Add..."]
    B --> C["builder.Build()<br/>đóng băng container"]
    C --> D["Giai đoạn 2<br/>app.Use... app.Map..."]
    D --> E["app.Run()"]
```
...
Analyzer `ASP0000` cảnh báo chuyện này. Cần cấu hình phụ thuộc vào một service khác thì dùng options pattern với `IConfigureOptions<T>` ([bài 7.8](7.7-options-pattern.mdx)) — nó được resolve **sau** khi container dựng xong.
...
`CreateScope()` là bắt buộc: `app.Services` là **root provider**, và lấy thẳng một service `Scoped` từ đó sẽ ném exception khi `ValidateScopes` bật — hoặc tệ hơn, giam nó nếu không bật ([bài 7.5](7.4-service-lifetimes.mdx)).
...
Lưu ý `CreateScope()` — không lấy trực tiếp từ `app.Services`, vì `DbContext` là `Scoped` và lấy từ phạm vi gốc sẽ ném lỗi hoặc gây rò rỉ như [bài 7.5](7.4-service-lifetimes.mdx) trình bày.
...
2. **Gom vào một extension** như [bài 5.6](../module-05-advanced-csharp/5.5-extension-methods.mdx) trình bày, để thứ tự chỉ tồn tại ở đúng một chỗ.
...
Bài kiểm thử này chạy qua **toàn bộ** đường ống nên nó bắt được lỗi thứ tự — thứ mà unit test không bao giờ bắt được. Đây chính là lý do kiểm thử tích hợp tồn tại, và [bài 9.9](../../stage-03-aspnet-core-backend/module-09-web-api-professional/9.8-api-testing.mdx) trình bày đầy đủ.
...
- **Bài trước:** [7.5 — 4. Registering Services](7.5-registering-services.mdx)
- **Bài tiếp theo:** [7.7 — 6. Options Pattern](7.7-options-pattern.mdx)
