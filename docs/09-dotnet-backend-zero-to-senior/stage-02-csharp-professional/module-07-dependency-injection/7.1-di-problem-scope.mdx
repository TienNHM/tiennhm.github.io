---
title: "7.1 — Vấn đề DI giải quyết"
...
```mermaid
flowchart TB
    subgraph Truoc["Trước: lớp tự tạo"]
        A[CustomerService] -->|new| B[SmtpEmailService]
        A -->|new| C[SqlCustomerRepository]
    end
    subgraph Sau["Sau: container cung cấp"]
        D[CustomerService] -->|phụ thuộc| E[IEmailService]
        D -->|phụ thuộc| F[ICustomerRepository]
        G[Composition Root] -."tạo và tiêm".-> D
        G -."chọn".-> E
        G -."chọn".-> F
    end
```
