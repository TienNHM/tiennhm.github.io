...
- **`BackgroundService`** qua `IServiceScopeFactory` ([bài 7.5](7.4-service-lifetimes.mdx)).
- **Factory hẹp resolve đúng một kiểu đã biết**, như ở [bài 7.9](7.8-keyed-services.mdx).
...
Chỗ này thường xuất hiện khi ai đó thêm tính năng gấp. Nó mang lại đầy đủ năm vấn đề ở [bài 7.2](7.1-di-problem-scope.mdx), chỉ khác là bây giờ chúng nằm lẫn giữa code đã làm đúng.
...
Như vậy ngay cả khi một lập trình viên quên lọc, dữ liệu vẫn không rò rỉ. [Bài 13.11](../../stage-04-database-production/module-13-entity-framework-core/13.10-global-query-filters.mdx) trình bày đầy đủ.
...
3. **Kiểm tra lúc khởi động mất tác dụng.** `ValidateOnBuild` ở [bài 7.6](7.5-registering-services.mdx) dựa trên constructor để dựng đồ thị phụ thuộc. Với Service Locator, đồ thị đó không tồn tại.
...
**Gợi ý.** Số phụ thuộc lớn là **triệu chứng**, không phải bệnh. Bệnh là lớp đang làm quá nhiều việc. Hãy hỏi lại câu hỏi ở [bài 5.2](../module-05-advanced-csharp/5.1-solid-principles.mdx): **ai yêu cầu thay đổi lớp này?**
...
**Lưu ý về `ILogger`.** Nó xuất hiện ở mọi lớp và thường không tính vào số phụ thuộc "thật", vì nó là mối quan tâm xuyên suốt chứ không phải một trách nhiệm nghiệp vụ. Nếu muốn bỏ nó khỏi constructor hoàn toàn, có thể chuyển việc ghi log sang decorator như [bài 7.6](7.5-registering-services.mdx) trình bày.
...
- **Bài trước:** [7.8 — 7. Keyed Services (.NET 8)](7.8-keyed-services.mdx)
- **Bài tiếp theo:** [7.10 — Mở rộng và đào sâu](7.10-advanced-notes.mdx)
