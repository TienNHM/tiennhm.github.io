...
**`DbContext`**: `AddDbContext<T>()` đăng ký `Scoped` sẵn. Đừng đổi lifetime. Cần dùng ngoài request thì dùng `AddDbContextFactory` ([bài 6.6](../module-06-async-programming/6.5-task-parallel-library.mdx)).
