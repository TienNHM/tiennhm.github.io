...
Tiêm `IOptionsSnapshot<T>` vào một Singleton là **captive dependency** ([bài 7.5](7.5-service-lifetimes.mdx)) — nó bị giam và không bao giờ đọc lại nữa, im lặng.
...
Đây cũng là câu trả lời cho vấn đề ở [bài 7.7](7.7-program-cs-and-webapplicationbuilder.mdx): cần một service để cấu hình một service khác mà **không** gọi `BuildServiceProvider()`. `IConfigureOptions<T>` làm điều tương tự cho việc thiết lập giá trị:
...
**Gợi ý.** `IOptionsSnapshot<T>` được đăng ký là **`Scoped`**. Tiêm một `Scoped` vào một `Singleton` chính là captive dependency ở [bài 7.5](7.5-service-lifetimes.mdx).
