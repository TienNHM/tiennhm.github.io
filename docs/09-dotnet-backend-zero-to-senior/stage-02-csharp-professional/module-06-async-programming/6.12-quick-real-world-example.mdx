---
title: "6.12 — Ví dụ thực tế nhanh"
description: "Một dòng .Result làm API treo toàn bộ dưới tải: cơ chế thread pool starvation và ba lệnh tìm ra nó trong dự án."
sidebar_position: 12
---
...
Nó bắt được vấn đề **trước khi** người dùng phàn nàn — và bắt được cả những nguyên nhân khác cũng gây starvation, không riêng `.Result` ([bài 15.9](../../stage-04-database-production/module-15-docker-deployment/15.9-production-monitoring.mdx)).

### 6.12.8 — Rà lại code của bạn

<Checklist
  title="Danh sách rà soát thread pool starvation"
  items={[
    { text: "Không có .Result, .Wait hay GetAwaiter().GetResult() trong đường xử lý request." },
    { text: "Không có async void nào ngoài event handler giao diện." },
    { text: "Không bọc code async trong Task.Run." },
    { text: "Async đi suốt chuỗi, không có mắt xích đồng bộ ở giữa." },
    { text: "Có phân tích tĩnh chặn sync-over-async ở mức lỗi." },
    { text: "Giám sát ThreadPool Queue Length, không chỉ CPU." },
    { text: "Có cảnh báo khi queue length vượt ngưỡng." },
    { text: "Hiểu rằng CPU thấp kèm độ trễ cao nghĩa là đang chờ." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Tổ hợp số liệu nào xác định thread pool starvation?",
      answer: "ThreadPool Queue Length cao trong khi CPU thấp. Nó nghĩa là có nhiều việc đang xếp hàng nhưng thread không bận tính toán, tức chúng đang bị chiếm trong lúc chờ thứ gì đó."
    },
    {
      question: "Vì sao sự cố này khó chẩn đoán?",
      answer: "Vì endpoint gây lỗi không phải endpoint biểu hiện triệu chứng. Một endpoint hiếm dùng chiếm hết thread, và mọi endpoint khác chậm theo, nên người báo lỗi chỉ tới sai chỗ."
    },
    {
      question: "Vì sao thread pool tăng chậm?",
      answer: "Vì tạo thread tốn kém, mỗi thread khoảng một megabyte stack, và tăng ồ ạt thường hại nhiều hơn lợi. .NET cố ý tăng dần để tránh phản ứng thái quá với đỉnh tải ngắn, nhưng đặc tính đó khiến starvation thành vòng xoáy."
    },
    {
      question: "Async đi suốt chuỗi nghĩa là gì?",
      answer: "Mọi mắt xích từ controller xuống tới lời gọi I/O đều phải async. Một chỗ dùng .Result ở giữa vẫn chiếm thread, kể cả khi hàm bao ngoài đã khai báo async."
    },
    {
      question: "Tăng ThreadPool.SetMinThreads có sửa được vấn đề không?",
      answer: "Không, nó chỉ che triệu chứng và mua thêm thời gian. Mỗi thread tốn khoảng một megabyte, và vấn đề sẽ quay lại khi tải tăng thêm. Dùng để sống sót qua sự cố rồi sửa đúng ngay sau đó."
    },
    {
      question: "Vì sao Task.Run bọc code async là sai?",
      answer: "Vì code async vốn không cần thread riêng trong lúc chờ I/O. Bọc nó trong Task.Run đẩy việc sang một thread pool thread khác, nên chiếm hai thread thay vì không chiếm cái nào."
    }
  ]}
/>
...
- **Bài trước:** [6.11 — Mini case study](6.11-mini-case-study.mdx)
- **Bài tiếp theo:** [6.13 — Review and Assessment](6.13-review-and-assessment.mdx)
