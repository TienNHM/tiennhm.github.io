...
bombardier -c 200 -n 200 http://localhost:5000/bat-dong-bo
```
...
```csharp
var kq = GoiApiAsync().Result;           // 1. .Result
GoiApiAsync().Wait();                    // 2. .Wait()
var kq = GoiApiAsync().GetAwaiter().GetResult();   // 3. tưởng là "an toàn hơn" — không phải
```
...
app.MapGet("/v1", () => { var kq = LayDuLieuDongBo(); return Results.Ok(kq); });

// 2. Task.Run — mượn MỘT luồng khác rồi cũng giam nó
app.MapGet("/v2", async () => { var kq = await Task.Run(LayDuLieuDongBo); return Results.Ok(kq); });

// 3. Async thật — không giam luồng nào
app.MapGet("/v3", async (CancellationToken ct) => { var kq = await LayDuLieuAsync(ct); return Results.Ok(kq); });
```
...
var thumbnail = await Task.Run(() => TaoThumbnail(anh), ct);
```
...
```csharp
public ValueTask<Customer?> LayAsync(int id, CancellationToken ct)
{
    if (_cache.TryGetValue(id, out var kh))
        return new ValueTask<Customer?>(kh);          // không cấp phát

    return new ValueTask<Customer?>(LayTuDbAsync(id, ct));   // chỉ cấp phát khi cần
}
```
