...
description: "System.Threading.Channels: hàng đợi trong tiến trình không cần lock, backpressure với bounded channel, và BackgroundService làm consumer đúng cách."
sidebar_position: 9
---
...
<Checklist
  title="Danh sách rà soát Channels"
  items={[
    { text: "Channel dùng CreateBounded, không phải CreateUnbounded." },
    { text: "FullMode được chọn có chủ đích, không để mặc định vì tiện." },
    { text: "Consumer kế thừa BackgroundService và ghi đè ExecuteAsync." },
    { text: "Mỗi job được xử lý trong try/catch riêng, exception không thoát ra ExecuteAsync." },
    { text: "Mỗi job tạo scope DI riêng qua IServiceScopeFactory." },
    { text: "StopAsync gọi Writer.Complete() trước khi chờ." },
    { text: "ShutdownTimeout đủ dài cho job dài nhất." },
    { text: "SingleReader/SingleWriter khớp với số consumer/producer thực tế." },
    { text: "Đã trả lời được: mất một item khi restart thì có sao không?" },
  ]}
/>
...
public class EmailConsumer : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken ct)
    {
        await foreach (var job in _q.DocAsync(ct))     // KHÔNG BAO GIỜ kết thúc
...
public class EmailConsumer(EmailQueue q) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await foreach (var job in q.DocAsync(stoppingToken))
            await XuLyAsync(job, stoppingToken);
    }
}
```
...
Nó vẫn **giữ tham chiếu** tới task, nên khi ứng dụng tắt, `StopAsync` huỷ token và chờ task kết thúc gọn gàng. Đây chính là fire-and-forget **làm đúng cách** — khác với `_ = MethodAsync()` ở [bài 6.7](6.7-async-patterns.mdx), nơi không ai giữ task.
...
protected override async Task ExecuteAsync(CancellationToken stoppingToken)
{
    await foreach (var job in _q.DocAsync(stoppingToken))
        await XuLyAsync(job, stoppingToken);      // ném -> thoát vòng lặp -> consumer chết
}
```
...
```csharp
await q.XepAsync(new Job("LOI"), ct);                    // gây ngoại lệ
for (int i = 1; i <= 10; i++)
    await q.XepAsync(new Job($"hop-le-{i}"), ct);
```

**Kết quả:** công việc `LOI` ném ngoại lệ, consumer thoát vòng lặp. Mười công việc hợp lệ nằm mãi trong hàng đợi. Ứng dụng **vẫn phục vụ HTTP bình thường**, health check vẫn xanh, không có dấu hiệu nào cho thấy có vấn đề.
...
{
    _logger.LogError(ex, "Job {JobId} thất bại lần {Lan}", job.Id, job.SoLanThu);

    if (job.SoLanThu < 3)
        await _q.XepAsync(job with { SoLanThu = job.SoLanThu + 1 }, stoppingToken);
    else
        await _deadLetter.LuuAsync(job, ex, stoppingToken);
}
```
...
<FAQSection
  items={[
    {
      question: "Channel giải quyết vấn đề gì?",
      answer: "Nó tách producer khỏi consumer khi một bên sinh việc nhanh hơn bên kia xử lý được. Producer trả lời ngay, consumer xử lý theo nhịp của nó, và khi hàng đợi đầy thì producer bị làm chậm lại thay vì hệ thống sụp. Đây là hàng đợi trong bộ nhớ, không cần lock và hỗ trợ async."
    },
    {
      question: "Nên chọn bounded hay unbounded?",
      answer: "Gần như luôn là bounded. Unbounded không chặn producer nên nếu producer bền bỉ nhanh hơn consumer thì hàng đợi lớn dần cho tới khi tiến trình hết bộ nhớ, và nó chết đúng vào lúc tải cao nhất. Bounded với FullMode Wait tạo ra backpressure nên hệ thống suy giảm dần thay vì sập đột ngột."
    },
    {
      question: "Vì sao không được viết consumer trong IHostedService.StartAsync?",
      answer: "Vì host gọi StartAsync của từng hosted service tuần tự và chờ từng cái xong trước khi mở cổng HTTP. Một vòng lặp await foreach chạy mãi sẽ làm ứng dụng treo ở bước khởi động, không có log lỗi, chỉ là không bao giờ sẵn sàng. Đúng là kế thừa BackgroundService và ghi đè ExecuteAsync."
    },
    {
      question: "Vì sao mỗi job cần scope DI riêng?",
      answer: "Vì BackgroundService là singleton, còn DbContext và phần lớn service nghiệp vụ là scoped. Lấy thẳng chúng vào constructor là lỗi vòng đời và sẽ dùng chung một instance cho mọi job. Cách đúng là tiêm IServiceScopeFactory rồi tạo scope mới cho từng job."
    },
    {
      question: "Vì sao phải try/catch quanh từng job?",
      answer: "Vì exception thoát ra khỏi ExecuteAsync sẽ giết consumer vĩnh viễn. Hàng đợi vẫn nhận việc nhưng không còn ai xử lý, và ứng dụng vẫn chạy bình thường nên không ai phát hiện ra. Một job hỏng không được phép làm chết consumer."
    },
    {
      question: "Khi nào không nên dùng Channel?",
      answer: "Khi không được phép mất việc. Channel sống trong bộ nhớ của một tiến trình nên restart hay crash là mất sạch, không chia sẻ được giữa các instance, và không có retry hay dead-letter. Câu hỏi quyết định là mất một item khi server restart thì có sao không; nếu có sao thì dùng Hangfire, RabbitMQ hoặc Kafka."
    }
  ]}
/>
