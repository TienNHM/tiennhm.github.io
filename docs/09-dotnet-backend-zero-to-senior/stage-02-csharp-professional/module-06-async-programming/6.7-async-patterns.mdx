...
description: "IAsyncEnumerable để stream, async factory thay cho constructor async, IAsyncDisposable, khử trùng lặp request và fire-and-forget làm đúng cách."
sidebar_position: 7
---
...
<Checklist
  title="Danh sách rà soát Async Patterns"
  items={[
    { text: "Mọi async iterator tự viết đều có [EnumeratorCancellation] trên CancellationToken." },
    { text: "Endpoint export dữ liệu lớn dùng IAsyncEnumerable thay vì ToListAsync." },
    { text: "Không có .Result hay .Wait() nào trong constructor." },
    { text: "Lớp cần khởi tạo bất đồng bộ có constructor private và static CreateAsync." },
    { text: "Lớp có dọn dẹp I/O cài IAsyncDisposable và được dùng với await using." },
    { text: "Cache khử trùng lặp lưu Task<T> và luôn gỡ entry trong finally." },
    { text: "Không có fire-and-forget nào cho việc quan trọng — dùng hàng đợi." },
    { text: "Mọi fire-and-forget còn lại đều có try/catch ghi log bên trong." },
  ]}
/>
...
**Gợi ý.** Bài này là phiên bản bất đồng bộ của bài `yield return` ở [bài 1.4](../../stage-01-foundation/module-01-programming-logic/1.4-loops.mdx), nơi đã đo được 122 MB so với 4 MB.
...
```csharp
public async Task<List<Branch>> LayChiNhanhAsync(CancellationToken ct)
{
    if (_cache.TryGetValue("branches", out List<Branch>? ds))
        return ds!;

    // 50 request cùng tới đây khi cache vừa hết hạn
    ds = await _db.Branches.ToListAsync(ct);          // 50 truy vấn database
    _cache.Set("branches", ds, TimeSpan.FromSeconds(5));
    return ds;
}
```
...
```csharp
await Task.WhenAll(Enumerable.Range(1, 50).Select(_ => svc.LayChiNhanhAsync(ct)));
...
```csharp
public Task<List<Branch>> LayChiNhanhAsync(CancellationToken ct)
{
    return _cache.GetOrCreate("branches", entry =>
    {
        entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(5);
        return LayTuDbAsync(ct);          // trả về Task, KHÔNG await
    })!;
}

private async Task<List<Branch>> LayTuDbAsync(CancellationToken ct)
...
```csharp
app.MapPost("/dang-ky", async (DangKyRequest req, CrmDbContext db, IEmailSender mail, CancellationToken ct) =>
{
    var kh = Customer.Tao(req.Ten, new Email(req.Email));
    db.Customers.Add(kh);
    await db.SaveChangesAsync(ct);

    _ = mail.GuiChaoMungAsync(kh.Email);     // fire-and-forget
    return Results.Ok();
});
```
...
2. **Ngoại lệ bị nuốt hoàn toàn.** Nếu `GuiChaoMungAsync` ném, không có khối `catch` nào, không có log. Từ .NET 4.5, ngoại lệ không được quan sát **không** làm sập tiến trình nữa — nghĩa là nó thất bại **hoàn toàn im lặng**.
...
public sealed class EmailQueue
{
    ...
    public ValueTask XepAsync(EmailJob job, CancellationToken ct) => _ch.Writer.WriteAsync(job, ct);
    public IAsyncEnumerable<EmailJob> DocAsync(CancellationToken ct) => _ch.Reader.ReadAllAsync(ct);
    public void HoanTat() => _ch.Writer.Complete();
}
    ...
    : BackgroundService
{
    ...
    {
        await foreach (var job in q.DocAsync(stoppingToken))
        {
            try
            {
                using var scope = sf.CreateScope();
                var mail = scope.ServiceProvider.GetRequiredService<IEmailSender>();
                await mail.GuiChaoMungAsync(job.Email, stoppingToken);
            }
...
_ = q.XepAsync(new EmailJob(kh.Email), ct);
```
...
<FAQSection
  items={[
    {
      question: "IAsyncEnumerable giúp gì so với trả về List?",
      answer: "Nó trả dữ liệu dần nên bộ nhớ đi từ tỷ lệ với số bản ghi xuống còn hằng số, và byte đầu tiên ra sớm hơn nhiều. Nhưng không nên dùng khi kết quả nhỏ, khi cần Count hoặc duyệt hai lần, hoặc khi người tiêu thụ chậm vì connection database phải mở suốt quá trình duyệt."
    },
    {
      question: "[EnumeratorCancellation] để làm gì?",
      answer: "Nó cho phép CancellationToken truyền vào async iterator thật sự có tác dụng. Thiếu thuộc tính này thì token bị bỏ qua im lặng: không có lỗi biên dịch, chỉ là việc huỷ không bao giờ hoạt động."
    },
    {
      question: "Vì sao không viết được constructor async?",
      answer: "Vì constructor phải trả về đối tượng đã dựng xong. Gọi .Result trong constructor là deadlock cho sẵn. Mẫu chuẩn là constructor private cộng static factory CreateAsync, trong đó private constructor là phần quan trọng vì nó khiến không thể tạo đối tượng ở trạng thái chưa khởi tạo."
    },
    {
      question: "IAsyncDisposable giải quyết vấn đề gì?",
      answer: "IDisposable bắt bạn phải block trong Dispose nếu việc dọn dẹp có I/O, đúng thứ bạn đang cố tránh. IAsyncDisposable cho phép await trong quá trình giải phóng, và được dùng với await using."
    },
    {
      question: "Cache stampede là gì và cách chống?",
      answer: "Là khi nhiều request cùng lúc thấy cache miss và cùng đi database. Cách chống là cache Task<T> thay vì T: request đầu tiên đặt Task đang chạy vào cache, những request sau tìm thấy chính Task đó và cùng await nó. Phải nhớ gỡ entry trong khối finally, nếu không một lần lỗi sẽ cache Task lỗi vĩnh viễn."
    },
    {
      question: "Fire-and-forget sai ở chỗ nào?",
      answer: "Exception biến mất không dấu vết, và quan trọng hơn là công việc bị cắt giữa chừng khi request kết thúc hoặc ứng dụng shutdown, lúc đó HttpContext và mọi service scoped đã bị giải phóng. Câu trả lời đúng gần như luôn là đẩy vào hàng đợi cho một BackgroundService xử lý, vì nó có scope DI riêng, có retry và sống sót qua restart."
    }
  ]}
/>
