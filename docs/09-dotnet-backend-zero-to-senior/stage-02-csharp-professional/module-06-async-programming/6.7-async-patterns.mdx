...
2. **Tái hiện stampede.** Viết một service có cache 5 giây, bắn 50 request đồng thời bằng `Task.WhenAll` ngay sau khi cache hết hạn, đếm số truy vấn database. Sửa bằng cách cache `Task<T>` và đếm lại.
