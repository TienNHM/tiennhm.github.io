...
2. **Làm mất phần tử.** Dùng `Parallel.ForEachAsync` với 1000 phần tử cùng `Add` vào một `List<T>`. Chạy 10 lần và ghi lại số phần tử cuối cùng của mỗi lần. So sánh với `ConcurrentBag`.
