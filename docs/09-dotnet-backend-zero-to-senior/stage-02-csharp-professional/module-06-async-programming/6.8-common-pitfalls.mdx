...
Cách chữa là `async` lên tới tận trên cùng. Cơ chế deadlock đã mổ xẻ ở [bài 6.4](6.4-configureawait.mdx) và trong bài [Gọi `.Result` khi nào thì deadlock](/blog/deadlock-result-wait-csharp).
...
Quy tắc nhận biết: **nếu tham số nhận `Action`, `Action<T>` hay một delegate trả `void`, thì lambda `async` bạn truyền vào là `async void`.** Hậu quả đã nói ở [bài 6.3](6.3-task-and-async-await.mdx): vòng lặp kết thúc trước khi việc xong, và exception làm sập tiến trình.
...
**Gợi ý.** Bài này bổ sung cho bài 2 ở [bài 6.2](6.2-why-async-matters.mdx). Điểm mới: trong ASP.NET Core, luồng mà `Task.Run` mượn đến **từ đâu**?
...
**Nguyên tắc chung, lặp lại từ [bài 4.7](../module-04-csharp-core/4.7-access-modifiers.mdx).** Cái gì công cụ kiểm được thì để công cụ kiểm; sức chú ý của người review là tài nguyên khan hiếm, hãy dành nó cho bốn bẫy còn lại.
