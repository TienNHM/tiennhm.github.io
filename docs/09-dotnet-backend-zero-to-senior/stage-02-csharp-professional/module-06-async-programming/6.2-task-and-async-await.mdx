---
title: "6.2 — 1. Task và async/await"
description: "Task là lời hứa về một kết quả tương lai, không phải một thread. Kèm async void là gì và vì sao nó làm sập tiến trình, cùng tiêu chí chọn ValueTask."
sidebar_position: 2
---
...
<Checklist
  title="Danh sách rà soát Task và async/await"
  items={[
    { text: "Không có async void nào trong toàn bộ code backend." },
    { text: "Mọi phương thức async đều có hậu tố Async trong tên." },
    { text: "CancellationToken là tham số cuối của mọi phương thức async có I/O." },
    { text: "Không giả định thread không đổi sau một await." },
    { text: "Không dùng ThreadLocal hay ThreadStatic qua ranh giới await." },
    { text: "Chỉ dùng ValueTask ở nơi đã đo và thấy cấp phát là nút cổ chai." },
    { text: "Không bỏ await trong phương thức có using hoặc try/catch." },
  ]}
/>
...
```csharp
public async Task<int> XuLyAsync(int id, CancellationToken ct)
{
    var kh = await LayKhachHangAsync(id, ct);      // await thứ nhất
    var don = await LayDonHangAsync(kh.Id, ct);    // await thứ hai
    return don.Count;
}
```
...
3. **`MoveNext` là thứ bạn thấy trong dấu vết ngăn xếp.** Khi đọc log lỗi và gặp `at Crm.Services.OrderService.<XuLyAsync>d__0.MoveNext()`, giờ bạn biết đó là gì: máy trạng thái của phương thức `XuLyAsync`, chứ không phải một lỗi lạ.
...
```csharp
public async void XuLyAsync() { }    // gõ thiếu, trình biên dịch chấp nhận
public async Task XuLyAsync() { }    // đúng
```
...
{
    public bool DaGiaiPhong;
    public void Dispose() => DaGiaiPhong = true;
    public string Doc() => DaGiaiPhong ? throw new ObjectDisposedException(nameof(Res)) : "ok";
}

static Task<string> BayUsing()
{
    using var r = new Res();
    return Task.Delay(30).ContinueWith(_ => r.Doc());   // KHÔNG await
}

try { Console.WriteLine(await BayUsing()); }
...
6. ContinueWith chạy, gọi r.Doc() -> r ĐÃ bị giải phóng -> ném
```
...
```csharp
static async Task<string> DungCach()
{
    using var r = new Res();
    await Task.Delay(30);
    return r.Doc();          // using chỉ kết thúc SAU dòng này
}
```
...
public Task<Customer?> LayAsync(int id, CancellationToken ct)
    => _repo.LayTheoIdAsync(id, ct);
```
...
<FAQSection
  items={[
    {
      question: "await có tạo thread mới không?",
      answer: "Không. Với I/O thật thì không có thread nào đang chờ cả: lệnh được gửi xuống hệ điều hành, phương thức trả về ngay và thread quay lại pool, rồi khi dữ liệu về thì một thread bất kỳ từ pool chạy tiếp phần còn lại. Task là lời hứa về một kết quả, không phải một thread."
    },
    {
      question: "async biến phương thức thành gì?",
      answer: "Thành một máy trạng thái do trình biên dịch sinh ra, với mỗi await là một điểm có thể tạm dừng rồi tiếp tục. Hai hệ quả: biến cục bộ sống qua await được nâng thành field nên nằm trên heap, và code sau await có thể chạy trên thread khác."
    },
    {
      question: "Vì sao async void nguy hiểm?",
      answer: "Ba lý do. Không await được nên người gọi không biết nó xong chưa. Ngoại lệ không có Task nào để lưu nên đi thẳng lên SynchronizationContext và làm sập tiến trình, kể cả khi lời gọi nằm trong try catch. Và không kiểm thử được vì bài test kết thúc trước khi công việc xong."
    },
    {
      question: "Khi nào được phép dùng async void?",
      answer: "Chỉ với event handler của UI framework, nơi chữ ký do framework quy định, và ngay cả khi đó vẫn phải tự bắt mọi ngoại lệ bên trong. Trong backend thì không có trường hợp nào cần async void; thấy nó là thấy lỗi."
    },
    {
      question: "ValueTask dùng khi nào và có hạn chế gì?",
      answer: "Chỉ khi phần lớn lời gọi trả về ngay, điển hình là hàm có cache, vì nó tránh được việc cấp phát trên heap ở đường nhanh. Ba hạn chế: chỉ await được một lần, không await đồng thời từ nhiều chỗ, và không gọi Result khi chưa hoàn thành. Mặc định vẫn nên dùng Task."
    },
    {
      question: "Khi nào KHÔNG được bỏ async await để trả thẳng Task?",
      answer: "Ba trường hợp. Khi có using, vì bỏ await thì đối tượng bị giải phóng trước lúc truy vấn xong và bạn nhận ObjectDisposedException ở một chỗ hoàn toàn khác. Khi có try catch, vì ngoại lệ sẽ không bị bắt ở đây nữa. Và khi cần dùng kết quả trước khi trả về."
    }
  ]}
/>
