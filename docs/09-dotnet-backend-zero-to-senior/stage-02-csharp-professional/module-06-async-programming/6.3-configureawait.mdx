---
title: "6.3 — 2. ConfigureAwait"
description: "ConfigureAwait(false) nói với CLR rằng continuation không cần quay về SynchronizationContext gốc. Vì sao nó cứu deadlock trên ASP.NET Framework và vì sao ASP.NET Core không cần."
sidebar_position: 3
---
...
sequenceDiagram
    ...
    T->>T: gọi .Result — block và giữ context
    T->>IO: gửi request
    IO-->>C: phản hồi về, đăng ký continuation
    C--xT: cần dùng thread này để chạy tiếp
    Note over T,C: Thread chờ Task<br/>Task chờ thread<br/>DEADLOCK
```
...
:::tip Đọc thêm
Bài [Gọi `.Result` khi nào thì deadlock, khi nào thì không?](/blog/deadlock-result-wait-csharp) đi sâu vào việc vì sao cùng một dòng code treo trong WPF/ASP.NET Framework nhưng chạy bình thường trong Console/ASP.NET Core.
:::
...
.NET 8 thêm một overload rõ nghĩa hơn cho `Task` (không phải `Task<T>`):
...
await task.ConfigureAwait(ConfigureAwaitOptions.SuppressThrowing);     // không ném lại exception
await task.ConfigureAwait(ConfigureAwaitOptions.ForceYielding);        // luôn yield, không bao giờ đồng bộ
```
...
<Checklist
  title="Danh sách rà soát ConfigureAwait"
  items={[
    { text: "Không có .Result hay .Wait() nào trong đường chạy request." },
    { text: "Trong thư viện dùng lại: ConfigureAwait(false) ở MỌI await, không nửa vời." },
    { text: "Trong ASP.NET Core: không rải ConfigureAwait(false) vô ích khắp nơi." },
    { text: "Trong WPF/WinForms: không ConfigureAwait(false) trước code chạm vào control." },
    { text: "Hiểu rằng ConfigureAwait(false) chỉ giấu triệu chứng của .Result, không chữa nó." },
    { text: "Đã cân nhắc ConfigureAwaitOptions.SuppressThrowing thay cho try/catch rỗng." },
  ]}
/>
...
{
    var result = GetDataAsync().Result;   // TREO VĨNH VIỄN
...
2. Gọi GetDataAsync(), chạy tới await GetStringAsync
3. await ghi nhớ SynchronizationContext hiện tại = context của luồng UI
4. GetDataAsync trả về một Task chưa hoàn tất
5. .Result CHẶN luồng UI, chờ Task đó
6. HTTP hoàn tất -> phần còn lại của GetDataAsync cần chạy tiếp
...
| Khi có tải | — | Thread pool starvation, như [bài 6.2](6.1-why-async-matters.mdx) |
...
{
    try { MessageBox.Show(await GetDataAsync()); }
    catch (Exception ex) { MessageBox.Show(ex.Message); }
}
```

Đây là một trong những trường hợp hiếm hoi `async void` được chấp nhận, như [bài 6.3](6.2-task-and-async-await.mdx) đã nêu — và vẫn phải bọc `try/catch`.
...
{
    await _repo.SaveAsync().ConfigureAwait(false);   // rời context
    await Task.Delay(100);                           // vẫn ở thread pool
    txtStatus.Text = "Đã lưu";                       // NÉM — không phải luồng UI
}
```
...
**Điều quan trọng nhất của bài này.** Vấn đề không phải "dùng hay không dùng `ConfigureAwait(false)`" — cả hai lựa chọn đều đúng trong ngữ cảnh của nó. Vấn đề là **không có quyết định nào được ghi lại**, nên mỗi người làm một kiểu. Hãy chọn một, ghi vào tài liệu của dự án, và **để công cụ ép buộc** thay vì trông chờ vào code review — đúng nguyên tắc ở [bài 4.7](../module-04-csharp-core/4.6-access-modifiers.mdx): biến quy ước thành ràng buộc kỹ thuật.
...
<FAQSection
  items={[
    {
      question: "SynchronizationContext là gì?",
      answer: "Là cơ chế trả lời câu hỏi phần code sau await nên chạy ở đâu. Mặc định runtime chụp lại SynchronizationContext.Current tại thời điểm await và đăng ký continuation vào đó, nên continuation quay về đúng thread cũ. ASP.NET Framework, WinForms và WPF đều có context; ASP.NET Core và Console thì context là null."
    },
    {
      question: "Deadlock khi gọi .Result xảy ra như thế nào?",
      answer: "Thread request bị .Result block và đang giữ context. Khi I/O trả về, continuation được đăng ký vào chính context đó, nhưng context chỉ cho một thao tác tại một thời điểm và slot đang bị .Result chiếm. Continuation không chạy nên Task không hoàn thành, mà .Result thì chờ Task. Hai bên chờ nhau vĩnh viễn, không có exception và không có log."
    },
    {
      question: "ASP.NET Core có cần ConfigureAwait(false) không?",
      answer: "Không, vì ASP.NET Core đã bỏ hẳn SynchronizationContext nên không có context nào để quay về. Rải ConfigureAwait(false) khắp controller và service chỉ làm code khó đọc mà không được gì. Thư viện dùng lại thì vẫn cần, vì thư viện không biết ai sẽ gọi mình."
    },
    {
      question: "ConfigureAwait áp dụng cho cả phương thức hay chỉ một await?",
      answer: "Chỉ đúng một await. Nhưng sau await đầu tiên có ConfigureAwait(false) thì bạn đã ra khỏi context rồi, nên những await sau dù không ghi gì cũng không còn context để quay về. Vì vậy trong thư viện phải dùng nhất quán ở mọi await, nếu không sẽ có bug chỉ xuất hiện khi thao tác hoàn thành đồng bộ."
    },
    {
      question: "ConfigureAwait(false) có phải cách chữa deadlock đúng không?",
      answer: "Không, nó chỉ giấu triệu chứng. Nguyên nhân thật là .Result hoặc .Wait() chặn thread. Cách chữa đúng là async all the way, tức để phương thức gọi cũng là async Task và dùng await thay vì .Result."
    },
    {
      question: "ConfigureAwaitOptions trong .NET 8 có gì mới?",
      answer: "Nó là overload rõ nghĩa hơn cho Task, với None tương đương ConfigureAwait(false), ContinueOnCapturedContext tương đương true, SuppressThrowing để chờ xong mà không ném lại exception, và ForceYielding để luôn yield chứ không bao giờ chạy đồng bộ."
    }
  ]}
/>
...
- **Bài trước:** [6.2 — 1. Task và async/await](6.2-task-and-async-await.mdx)
- **Bài tiếp theo:** [6.4 — 3. CancellationToken](6.4-cancellationtoken.mdx)
