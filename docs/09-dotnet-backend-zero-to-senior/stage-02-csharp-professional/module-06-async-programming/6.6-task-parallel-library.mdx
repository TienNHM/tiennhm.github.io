...
description: "Task.WhenAll, Task.WhenAny và Parallel.ForEachAsync: chạy song song đúng cách, chặn số lượng đồng thời, và không nuốt mất exception."
sidebar_position: 6
---
...
`AddDbContextFactory<AppDbContext>()` trong `Program.cs` là thứ cho bạn `_factory`. Xem thêm ở [Module 13 — Entity Framework Core](../../stage-04-database-production/module-13-entity-framework-core/).
...
`Parallel.ForEach` nhận `Action<T>`, nên lambda `async` ở đây trở thành **`async void`** (xem [bài 6.3](6.3-task-and-async-await.mdx)): vòng lặp "xong" ngay lập tức trong khi công việc còn đang chạy, và mọi exception làm sập tiến trình.
...
<Checklist
  title="Danh sách rà soát Task Parallel Library"
  items={[
    { text: "Các lời gọi I/O độc lập trong cùng một request đã được gom vào Task.WhenAll." },
    { text: "Không có Task.WhenAll nào trên cùng một DbContext instance." },
    { text: "Không có Task.WhenAll không giới hạn trên danh sách có kích thước tuỳ ý." },
    { text: "Danh sách lớn dùng Parallel.ForEachAsync với MaxDegreeOfParallelism." },
    { text: "Mọi SemaphoreSlim.WaitAsync đều có Release trong khối finally." },
    { text: "Không có lambda async nào truyền vào Parallel.ForEach." },
    { text: "Timeout dùng WaitAsync hoặc CancelAfter, không dùng WhenAny với Task.Delay." },
    { text: "Nơi cần log đủ lỗi đã gán WhenAll vào biến để đọc AggregateException." },
  ]}
/>
...
```csharp
var kh  = await _crm.LayKhachHangAsync(id, ct);      // 120 ms
var don = await _order.LayDonGanDayAsync(id, ct);    // 200 ms
var tk  = await _report.LayThongKeAsync(id, ct);     //  80 ms
// tổng: ~400 ms
```
...
```csharp
var tKh  = _crm.LayKhachHangAsync(id, ct);           // cả ba khởi động ngay
var tDon = _order.LayDonGanDayAsync(id, ct);
var tTk  = _report.LayThongKeAsync(id, ct);

await Task.WhenAll(tKh, tDon, tTk);

var kh = tKh.Result; var don = tDon.Result; var tk = tTk.Result;
...
```csharp
var (kh, don, tk) = await (
    _crm.LayKhachHangAsync(id, ct),
    _order.LayDonGanDayAsync(id, ct),
    _report.LayThongKeAsync(id, ct)
...
```csharp
var tasks = danhSach.Select(async item =>
{
    using var scope = _scopeFactory.CreateScope();
    var db = scope.ServiceProvider.GetRequiredService<CrmDbContext>();
    await db.Orders.AddAsync(ChuyenDoi(item), ct);
...
await Parallel.ForEachAsync(
    ...
    async (item, token) =>
    {
        using var scope = _scopeFactory.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<CrmDbContext>();
        await db.Orders.AddAsync(ChuyenDoi(item), token);
...
await db.Orders.AddRangeAsync(danhSach.Select(ChuyenDoi), ct);
await db.SaveChangesAsync(ct);     // MỘT giao dịch, một kết nối
```

Cách này thường nhanh hơn hẳn mọi phương án song song, vì nó tránh được toàn bộ chi phí kết nối và chỉ có một giao dịch. [Bài 13.8](../../stage-04-database-production/module-13-entity-framework-core/13.8-performance.mdx) trình bày các kỹ thuật ghi hàng loạt đầy đủ hơn.
...
```csharp
var ketQua = await Task.WhenAll(danhSach.Select(async item =>
{
    try
    {
        await XuLyAsync(item, ct);
        return (item, ThanhCong: true, Loi: (Exception?)null);
    }
    catch (Exception ex)
    {
        return (item, ThanhCong: false, Loi: ex);
    }
}));

var that = ketQua.Where(r => !r.ThanhCong).ToList();
if (that.Count > 0)
{
    _logger.LogWarning("{Số} trong {Tổng} mục thất bại", that.Count, ketQua.Length);
    foreach (var r in that) _logger.LogError(r.Loi, "Mục {Item} thất bại", r.item);
}
```
...
<FAQSection
  items={[
    {
      question: "Task.WhenAll giúp tiết kiệm bao nhiêu thời gian?",
      answer: "Tổng thời gian giảm từ tổng các việc xuống còn việc chậm nhất. Ba lời gọi 200ms, 150ms và 300ms chạy tuần tự mất 650ms nhưng chạy song song chỉ mất 300ms. Điều kiện là ba lời gọi phải độc lập, không cái nào cần kết quả của cái trước."
    },
    {
      question: "Vì sao await Task.WhenAll làm mất exception?",
      answer: "Vì await chỉ ném lại exception của task đầu tiên bị lỗi, dù nhiều task cùng lỗi. Muốn lấy đủ thì phải gán WhenAll vào một biến rồi đọc thuộc tính Exception của nó, đó là một AggregateException chứa đầy đủ InnerExceptions."
    },
    {
      question: "Vì sao không được Task.WhenAll trên cùng một DbContext?",
      answer: "Vì DbContext không thread-safe và chỉ cho phép một thao tác tại một thời điểm. Chạy song song sẽ nhận InvalidOperationException báo rằng một thao tác thứ hai bắt đầu trước khi thao tác trước hoàn thành. Muốn song song thì mỗi task phải có một DbContext riêng lấy từ IDbContextFactory."
    },
    {
      question: "Viết timeout bằng WhenAny với Task.Delay có vấn đề gì?",
      answer: "Hai vấn đề. Nếu việc chính xong trước thì timer của Task.Delay vẫn chạy đủ thời gian, gây rò rỉ trong vòng lặp nóng. Và việc chính vẫn tiếp tục chạy sau khi bạn đã bỏ qua nó, nên tài nguyên vẫn bị chiếm. Cách đúng là dùng WaitAsync từ .NET 6, hoặc tốt hơn là CancellationTokenSource với CancelAfter để huỷ thật sự."
    },
    {
      question: "Khi nào dùng Parallel.ForEachAsync thay cho Task.WhenAll?",
      answer: "Khi danh sách có kích thước tuỳ ý hoặc lớn. Task.WhenAll không giới hạn số lượng chạy đồng thời nên với 1000 phần tử nó mở 1000 kết nối cùng lúc và làm cạn connection pool. Parallel.ForEachAsync cho phép đặt MaxDegreeOfParallelism để chặn con số đó."
    },
    {
      question: "Vì sao không truyền lambda async vào Parallel.ForEach?",
      answer: "Vì Parallel.ForEach nhận Action nên lambda async trở thành async void. Vòng lặp kết thúc ngay lập tức trong khi công việc còn đang chạy, và mọi exception bên trong làm sập tiến trình. Parallel.ForEach dành cho CPU-bound; cho I/O thì dùng Parallel.ForEachAsync."
    }
  ]}
/>
