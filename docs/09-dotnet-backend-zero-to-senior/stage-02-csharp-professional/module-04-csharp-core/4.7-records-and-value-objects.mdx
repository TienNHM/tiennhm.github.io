...
public async Task<int> CreateCustomerAsync(
    string name, string email, string phone, string taxCode, CancellationToken ct)
```

Chỗ gọi:

```csharp
await CreateCustomerAsync("Nguyễn Văn An", "0912345678", "an@company.com", "0101234567", ct);
...
public async Task<int> CreateCustomerAsync(
    string name, Email email, PhoneNumber phone, string taxCode, CancellationToken ct)
```
