---
title: "4.6 — 6. Access Modifiers"
description: "Sáu mức truy cập của C# và mức mặc định của từng ngữ cảnh. Kèm vì sao protected internal khác private protected, và cách mở cửa cho test mà không phơi ra cho cả thế giới."
sidebar_position: 6
---
...
<Checklist
  title="Danh sách rà soát mức truy cập"
  items={[
    { text: "Không có class nào public chỉ vì quên nghĩ; mỗi public là một quyết định." },
    { text: "Lớp cài đặt của interface được khai internal khi không cần dùng ngoài assembly." },
    { text: "Không có field nào public; chỉ có property." },
    { text: "Test truy cập internal qua InternalsVisibleTo, không bằng cách đổi thành public." },
    { text: "Property nghiệp vụ dùng { get; private set; } thay vì { get; set; }." },
    { text: "Không dùng protected internal hay private protected nếu không có lý do cụ thể." },
    { text: "Thư viện dùng chung có API public tối thiểu, vì mỗi public là một cam kết." },
  ]}
/>
...
{
    [Fact]
    public void NhanDienDuocLoiTrungKhoaCuaPostgres()
    {
        var ex = TaoDbUpdateException(sqlState: "23505");
        Assert.True(SqlErrorMapper.LaLoiTrungKhoa(ex));   // lớp internal
    }
}
```
...
<FAQSection
  items={[
    {
      question: "Mức truy cập mặc định trong từng ngữ cảnh là gì?",
      answer: "Kiểu ở cấp namespace mặc định là internal. Thành viên của class và struct mặc định là private. Thành viên của interface và enum luôn public. Vì vậy một class không có từ khoá nào sẽ không dùng được từ project khác — đó là câu trả lời cho phần lớn thắc mắc vì sao không thấy class này."
    },
    {
      question: "protected internal khác private protected ở đâu?",
      answer: "protected internal là phép HOẶC: cùng assembly hoặc là lớp con ở bất kỳ assembly nào, nên nó rộng hơn cả hai. private protected là phép VÀ: phải là lớp con và phải cùng assembly, nên nó hẹp nhất trong nhóm. Cả hai đều hiếm dùng trong thực tế."
    },
    {
      question: "Mẫu hợp đồng public, cài đặt internal có lợi gì?",
      answer: "Nó biến ranh giới kiến trúc thành thứ trình biên dịch ép buộc thay vì chỉ là quy ước trong tài liệu. Người viết code ở tầng khác không thể vô tình phụ thuộc vào lớp cài đặt vì họ không nhìn thấy nó; họ chỉ lấy được qua dependency injection dưới dạng interface."
    },
    {
      question: "Làm sao cho project test truy cập được thành viên internal?",
      answer: "Dùng InternalsVisibleTo trong file csproj hoặc dưới dạng attribute trong AssemblyInfo, nêu đích danh tên assembly test. Chỉ assembly được nêu tên mới nhìn thấy, phần còn lại vẫn bị chặn. Đổi sang public chỉ để test là phơi chi tiết cài đặt ra cho toàn bộ giải pháp."
    },
    {
      question: "Nếu phải mở internal ra để test thường xuyên thì sao?",
      answer: "Đó là dấu hiệu bạn đang test chi tiết cài đặt thay vì test hành vi qua API công khai. Những bài test như vậy sẽ vỡ mỗi lần refactor dù hành vi không đổi, nên chúng làm chậm việc cải tiến code thay vì bảo vệ nó."
    },
    {
      question: "Vì sao nên bắt đầu ở mức truy cập đóng nhất?",
      answer: "Vì mở rộng quyền truy cập là thay đổi tương thích ngược, không phá vỡ code nào. Còn siết lại thì làm hỏng mọi chỗ đang gọi, kể cả những project bạn không kiểm soát. Với thư viện dùng chung, mỗi thành viên public là một cam kết phải giữ ở các phiên bản sau."
    }
  ]}
/>
