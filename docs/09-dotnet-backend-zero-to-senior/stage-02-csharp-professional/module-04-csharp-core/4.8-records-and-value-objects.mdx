...
Hai thứ đầu là lý do record dùng làm **khoá `Dictionary`** được ngay, không cần override gì — đúng vấn đề đã nêu ở [bài 1.6](/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-01-programming-logic/1.6-arrays-and-lists).
...
2. **Ghi đè `Equals` thì phải ghi đè `GetHashCode`.** Quên là hỏng `Dictionary` và `HashSet` theo cách rất khó tìm — đúng vấn đề đã gặp ở [bài 1.6](../../stage-01-foundation/module-01-programming-logic/1.6-arrays-and-lists.mdx).
...
`record` ngăn việc **gán lại** thuộc tính `Tags`, nhưng không ngăn việc **sửa nội dung** danh sách mà nó trỏ tới. Một `record` chứa `List` là "bất biến" chỉ trên danh nghĩa — đúng cùng một vấn đề với `IReadOnlyList` ở [bài 4.3](4.3-encapsulation.mdx).
...
Chi tiết ở [bài 13.3](../../stage-04-database-production/module-13-entity-framework-core/13.3-dbcontext-and-entity-configuration.mdx).
...
**Gợi ý.** Bài này lặp lại thí nghiệm ở [bài 1.6](../../stage-01-foundation/module-01-programming-logic/1.6-arrays-and-lists.mdx) nhưng với khoá ghép hai trường — tình huống thực tế trong hệ thống nhiều tổ chức dùng chung.
