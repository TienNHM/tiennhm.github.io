...
public interface IRepository
{
    ...
    Task<List<Customer>> LayTatCaKhachHangAsync(CancellationToken ct);
    Task ThemKhachHangAsync(Customer c, CancellationToken ct);
    Task XoaKhachHangAsync(int id, CancellationToken ct);
    Task<Order?> LayDonHangAsync(int id, CancellationToken ct);
    Task<List<Order>> LayDonTheoKhachAsync(int customerId, CancellationToken ct);
    Task ThemDonHangAsync(Order o, CancellationToken ct);
    Task<List<Product>> LaySanPhamAsync(CancellationToken ct);
    Task CapNhatTonKhoAsync(int productId, int soLuong, CancellationToken ct);
    Task<Report> SinhBaoCaoAsync(DateTime tu, DateTime den, CancellationToken ct);
...
```csharp
var repoGia = new FakeRepository();   // phải cài đặt cả 15 phương thức
...
```csharp
var repoGia = Substitute.For<IRepository>();
repoGia.LayKhachHangAsync(42, Arg.Any<CancellationToken>()).Returns(khach);
```
...
{
    Task<Customer?> LayTheoIdAsync(int id, CancellationToken ct);
}

public interface ICustomerWriter
{
    Task ThemAsync(Customer c, CancellationToken ct);
...
```csharp
var docGia = Substitute.For<ICustomerReader>();   // cài đặt 1, dùng 1
docGia.LayTheoIdAsync(42, Arg.Any<CancellationToken>()).Returns(khach);
var service = new EmailService(docGia);
```
...
{
    public decimal TinhHoaHongQuy(Customer c)
    {
        var now = DateTimeOffset.UtcNow;                 // đầu vào ẩn
        var quy = (now.Month - 1) / 3 + 1;
        var dauQuy = new DateTimeOffset(now.Year, (quy - 1) * 3 + 1, 1, 0, 0, 0, TimeSpan.Zero);
        return c.DonHang.Where(o => o.NgayTao >= dauQuy).Sum(o => o.Total) * 0.05m;
    }
}
```
...
public class HoaHongService
{
    ...
    public HoaHongService(TimeProvider time) => _time = time;
...
```csharp
[Fact]
public void HoaHongQuy4TinhDungVaoGiayCuoiCungCuaNam()
{
    var time = new FakeTimeProvider(
        new DateTimeOffset(2026, 12, 31, 23, 59, 59, TimeSpan.Zero));

    var khach = new Customer("An");
    khach.ThemDon(new Order(new DateTime(2026, 9, 30), 10_000_000m));   // quý 3 — không tính
    khach.ThemDon(new Order(new DateTime(2026, 10, 1), 20_000_000m));   // quý 4 — tính
    khach.ThemDon(new Order(new DateTime(2026, 12, 31), 5_000_000m));   // quý 4 — tính

    var service = new HoaHongService(time);

    Assert.Equal(1_250_000m, service.TinhHoaHongQuy(khach));   // (20tr + 5tr) * 5%
}
```
...
```csharp
[Fact]
public void QuaGiaoThuaThiQuyDatLai()
{
    var time = new FakeTimeProvider(
        new DateTimeOffset(2026, 12, 31, 23, 59, 59, TimeSpan.Zero));
    var service = new HoaHongService(time);
    var truoc = service.TinhHoaHongQuy(khach);

    time.Advance(TimeSpan.FromSeconds(2));    // bước qua năm mới

    var sau = service.TinhHoaHongQuy(khach);
    Assert.Equal(0m, sau);                    // quý 1 năm 2027, chưa có đơn nào
}
```
