...
Interface vẫn `public` vì tầng nghiệp vụ cần chúng — nhưng interface nằm ở `Crm.Application`, không nằm ở đây, đúng như [bài 4.5](4.5-interface.mdx) đã phân tích. Vậy thứ duy nhất `Crm.Infrastructure` cần công khai là **lớp đăng ký DI**:
...
Nguyên tắc chung: **biến quy ước thành ràng buộc kỹ thuật bất cứ khi nào có thể**. Đây là cùng một ý với việc dùng `enum` thay `string` ở [bài 4.8](4.8-records-and-value-objects.mdx), và dùng constructor để chặn dữ liệu sai ở [bài 4.2](4.2-class-and-object.mdx) — đẩy việc kiểm tra từ "phải nhớ" sang "không thể sai".
