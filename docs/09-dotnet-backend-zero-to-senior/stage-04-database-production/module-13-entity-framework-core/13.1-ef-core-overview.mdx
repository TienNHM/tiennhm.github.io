...
**Chính xác thì phương thức nào gây ra chuyển?** Là **bất kỳ phương thức nào trả về `IEnumerable<T>` thay vì `IQueryable<T>`**. Từ điểm đó trở đi, mọi `Where`, `Select`, `OrderBy` là LINQ to Objects chạy trong tiến trình .NET, không còn là LINQ to Entities dịch sang SQL.
...
Toàn bộ lead của tenant được nạp về, rồi mới lọc và lấy 20. Sửa bằng cách đổi kiểu trả về thành `IQueryable<Lead>` — hoặc, tốt hơn, trả về `List<LeadDto>` đã hoàn chỉnh để nơi gọi **không thể** ghép thêm gì.
