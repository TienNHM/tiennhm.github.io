...
description: "EF Core làm gì bên dưới, chi phí thật của ORM, change tracker, khi nào dùng Dapper, và vì sao LINQ không phải SQL."
sidebar_position: 2
---
...
flowchart LR
    ...
    C --> D[(Database)]
    D --> E[DbDataReader]
    E --> F[Materialize<br/>tao doi tuong]
    F --> G[Change tracker<br/>luu ban sao goc]
    G --> H[Entity tra ve code]
```
...
```csharp
var customer = await db.Customers.FindAsync(42);   // EF luu BAN SAO gia tri goc
customer.Name = "Tên mới";                          // ban chi doi doi tuong
await db.SaveChangesAsync();                        // EF SO SANH va sinh UPDATE
```

EF Core sinh ra:

```sql
UPDATE Customers SET Name = @p0 WHERE Id = @p1;   -- CHI cot Name
```
...
Console.WriteLine(ReferenceEquals(a, b));    // True — CUNG mot doi tuong
```
...
Mặt trái: `DbContext` sống lâu tích luỹ entity và **trả về dữ liệu cũ**. Đó là lý do `DbContext` phải là `Scoped` ([bài 7.5](../../stage-02-csharp-professional/module-07-dependency-injection/7.5-service-lifetimes.mdx)).
...
```csharp
// SAI — .ToList() keo CA BANG ve roi moi loc
...
Sau `.ToList()`, kiểu trở thành `IEnumerable<T>` và mọi `Where` sau đó chạy **trong bộ nhớ ứng dụng**. Code biên dịch được, chạy được, và trên máy dev với 100 dòng thì không ai để ý.
...
// DUNG — nguoi goi van them dieu kien vao SQL duoc
...
```csharp
// KHONG dich duoc — EF Core nem exception luc chay
var customers = await db.Customers
    .Where(c => MyCustomValidator.IsValid(c.Email))     // ham C# tuy y
    .ToListAsync(ct);
```
...
```csharp
// 1. Viet lai bang bieu thuc dich duoc
.Where(c => c.Email.EndsWith("@company.com"))

// 2. Chuyen sang bo nho MOT CACH CO CHU DICH, sau khi da loc
var candidates = await db.Customers
    .Where(c => c.Status == CustomerStatus.Active)      // loc o SQL truoc
    .ToListAsync(ct);
...
```csharp
// EF Core 7+: mot cau UPDATE, khong nap entity nao
await db.Leads
...
builder.Services.AddDbContext<CrmDbContext>(options =>
{
    ...
    if (builder.Environment.IsDevelopment())
    {
        options.LogTo(Console.WriteLine, LogLevel.Information)
               .EnableSensitiveDataLogging()     // hien GIA TRI tham so
               .EnableDetailedErrors();
    }
});
```

`EnableSensitiveDataLogging` hiện giá trị tham số trong log — rất hữu ích khi gỡ lỗi và **tuyệt đối không bật ở production** ([bài 8.8](../../stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.8-logging-with-serilog.mdx)).
...
<Checklist
  title="Danh sách rà soát EF Core cơ bản"
  items={[
    { text: "Truy vấn chỉ đọc dùng AsNoTracking, hoặc context mặc định NoTracking." },
    { text: "Không có .ToList() nào đặt trước bộ lọc." },
    { text: "Phương thức trả về IQueryable khi người gọi còn cần thêm điều kiện." },
    { text: "Không có biểu thức nào không dịch được trong Where đi thẳng xuống SQL." },
    { text: "Lọc trong bộ nhớ chỉ xảy ra sau khi đã thu hẹp tập ở SQL." },
    { text: "Dùng FromSql thay vì FromSqlRaw khi cần SQL thuần." },
    { text: "Cập nhật hàng loạt dùng ExecuteUpdateAsync thay vì nạp rồi sửa." },
    { text: "Chèn hàng loạt dùng bulk copy, không dùng SaveChanges." },
    { text: "Log SQL được bật ở Development." },
    { text: "EnableSensitiveDataLogging không bật ở Production." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Change tracker làm gì và tốn kém ở đâu?",
      answer: "Nó giữ bản sao giá trị gốc của mỗi entity đã nạp để biết cái nào thay đổi khi SaveChanges, nhờ đó EF chỉ sinh UPDATE cho cột thật sự đổi. Cái giá là gấp đôi bộ nhớ cho mỗi dòng và chi phí dựng đối tượng, nên nạp hàng chục nghìn entity có tracking là chậm và tốn bộ nhớ."
    },
    {
      question: "Vì sao nên đặt NoTracking làm mặc định?",
      answer: "Vì quên AsNoTracking chỉ làm chậm, còn quên AsTracking thì SaveChanges không làm gì, và đó là lỗi rõ ràng dễ phát hiện. Với API chỉ trả dữ liệu ra ngoài thì NoTracking gần như luôn đúng và nhanh hơn khoảng 20 tới 50 phần trăm."
    },
    {
      question: "IQueryable khác IEnumerable thế nào trong EF Core?",
      answer: "IQueryable là một cây biểu thức chưa chạy, mỗi toán tử chỉ thêm một nút cho tới khi gặp ToList hay tương tự. Sau khi gọi ToList thì kiểu thành IEnumerable và mọi bộ lọc sau đó chạy trong bộ nhớ ứng dụng, tức bạn đã kéo cả bảng về rồi mới lọc."
    },
    {
      question: "Vì sao không nên trả về IEnumerable từ phương thức truy vấn?",
      answer: "Vì IQueryable kế thừa IEnumerable nên code vẫn biên dịch, nhưng người gọi thêm điều kiện sẽ lọc trong bộ nhớ thay vì trong SQL. Trả về IQueryable giữ được khả năng ghép thêm điều kiện vào câu SQL."
    },
    {
      question: "EF Core xử lý biểu thức không dịch được ra sao?",
      answer: "Từ EF Core 3.0, nó ném InvalidOperationException lúc chạy thay vì âm thầm đánh giá ở client. Đây là thay đổi tốt vì trước đó truy vấn như vậy sẽ kéo cả bảng về mà không ai biết, nhưng nó nghĩa là lỗi chỉ xuất hiện lúc chạy chứ không phải lúc biên dịch."
    },
    {
      question: "Khi nào EF Core không phù hợp?",
      answer: "Chèn hàng loạt hàng trăm nghìn dòng qua SaveChanges là chậm, nên dùng bulk copy. Truy vấn báo cáo phức tạp với window function hay CTE đệ quy thì Dapper hoặc SQL thuần tốt hơn. Cập nhật hàng loạt từng là vấn đề nhưng EF Core 7 đã có ExecuteUpdateAsync."
    }
  ]}
/>
