---
title: "14.7 — 7. Quartz.NET (Alternative Overview)"
description: "Quartz mạnh về lịch và clustering, misfire là khái niệm không có ở Hangfire, DisallowConcurrentExecution, và so sánh với Hangfire."
sidebar_position: 7
---
...
<Checklist
  title="Danh sách rà soát Quartz"
  items={[
    { text: "Đã chọn giữa Quartz và Hangfire dựa trên lịch có phải trung tâm bài toán không." },
    { text: "Mọi trigger khai rõ múi giờ." },
    { text: "Chính sách misfire được chọn có chủ đích cho từng trigger." },
    { text: "Không dùng IgnoreMisfires cho job chạy dày." },
    { text: "Job chạy lâu có [DisallowConcurrentExecution]." },
    { text: "PersistJobDataAfterExecution luôn đi cùng DisallowConcurrentExecution." },
    { text: "Job dùng IServiceScopeFactory để lấy service scoped." },
    { text: "context.CancellationToken được truyền xuống mọi lời gọi async." },
    { text: "Lỗi được bọc trong JobExecutionException với refireImmediately false." },
    { text: "WaitForJobsToComplete được bật để shutdown gọn gàng." },
    { text: "Cluster có đồng hồ đồng bộ và job vẫn idempotent." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Quartz và Hangfire giải bài toán gì khác nhau?",
      answer: "Hangfire là hàng đợi job: đưa việc vào rồi nó chạy, có retry và dashboard sẵn. Quartz là bộ lập lịch: trigger phức tạp, lịch loại trừ ngày lễ, và clustering có cơ chế nhận lại job khi node chết. Với CRM điển hình thì Hangfire thường đúng hơn; Quartz đáng khi lập lịch chính là nghiệp vụ."
    },
    {
      question: "Misfire là gì?",
      answer: "Là khi một trigger đáng lẽ đã chạy nhưng không chạy được, vì server đang tắt, không còn thread rảnh, hoặc job trước còn chạy. Quartz có bốn chính sách xử lý; Hangfire không có khái niệm này mà luôn chạy ngay khi server lên."
    },
    {
      question: "Chính sách misfire nào nguy hiểm?",
      answer: "IgnoreMisfires với job chạy dày. Job mỗi 5 phút mà server tắt 3 tiếng sẽ chạy bù 36 lần liên tiếp ngay khi lên, thường làm quá tải đúng lúc hệ thống vừa khởi động. Với job dày thì DoNothing hợp lý hơn."
    },
    {
      question: "DisallowConcurrentExecution làm gì?",
      answer: "Ngăn job chồng lên chính nó khi lần chạy trước chưa xong. Trong cluster nó áp trên toàn cluster, chỉ một node chạy tại một thời điểm, nên bạn không phải tự viết khoá phân tán như với BackgroundService."
    },
    {
      question: "Vì sao PersistJobDataAfterExecution phải đi cùng DisallowConcurrentExecution?",
      answer: "Vì nếu job chạy song song mà cùng lưu dữ liệu sau khi chạy, chúng sẽ ghi đè lẫn nhau và trạng thái lưu lại là không xác định."
    },
    {
      question: "Clustering của Quartz có đảm bảo job chạy đúng một lần không?",
      answer: "Không. Node chưa chết hẳn nhưng ngừng báo danh sẽ khiến node khác nhận lại job đang chạy dở, nên job có thể chạy hai lần. Vì vậy job vẫn phải idempotent, và các node phải có đồng hồ đồng bộ qua NTP."
    }
  ]}
/>
