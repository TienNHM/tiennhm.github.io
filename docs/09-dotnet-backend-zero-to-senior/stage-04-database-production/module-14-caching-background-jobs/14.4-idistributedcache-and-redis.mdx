...
Redis cung mang:   ~0,5-1 mili giay (mang + serialize + deserialize)
...
builder.Services.AddStackExchangeRedisCache(options =>
{
    ...
    options.InstanceName = "crm:";            // tien to cho MOI khoa
});
```
...
options.ConfigurationOptions = new ConfigurationOptions
{
    ...
    Ssl = true,
    AbortOnConnectFail = false,           // KHOI DONG duoc khi Redis chua san sang
    ConnectRetry = 5,
    ConnectTimeout = 5000,
    SyncTimeout = 1000,                   // that bai NHANH, dung cho lau
    DefaultDatabase = 0,
};
```
...
1. **Kiểu phải serialize được.** Entity EF Core có navigation vòng tròn sẽ ném exception hoặc sinh JSON khổng lồ. Cache DTO ([bài 14.3](14.3-imemorycache.mdx)).
...
{
    logger.LogWarning(ex, "Cache hong cho {Key}, bo qua", key);
...
1. **Có tenant id** — nếu không là rò rỉ dữ liệu ([bài 14.2](14.2-caching-fundamentals.mdx)).
...
```csharp
// DUNG — cache la toi uu, khong phai phu thuoc bat buoc
public async Task<CustomerDto?> GetAsync(int id, CancellationToken ct)
{
    ...
    {
        _logger.LogWarning(ex, "Redis khong kha dung, doc thang database");
    }
    ...
    if (customer is not null)
    {
        ...
        catch (RedisConnectionException) { /* bo qua — khong quan trong */ }
    }
...
Health check cho Redis nên trả `Degraded`, không phải `Unhealthy` ([bài 8.10](../../stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.10-health-checks.mdx)):
...
// Bo dem nguyen tu — khong lam duoc voi IDistributedCache
...
Ví dụ thứ hai giải quyết đúng giới hạn của rate limiter in-memory ở [bài 9.7](../../stage-03-aspnet-core-backend/module-09-web-api-professional/9.7-rate-limiting.mdx).
...
```
KEYS *              -> quet TOAN BO keyspace, CHAN Redis (don luong)
FLUSHALL            -> xoa moi thu
...
```csharp
// DUNG — SCAN duyet theo lo, khong chan
...
Đây là nền tảng của xoá theo tag ở [bài 14.5](14.5-cache-invalidation-strategies.mdx).
