...
Câu hỏi quyết định đã nêu ở [bài 9.10](../../stage-03-aspnet-core-backend/module-09-web-api-professional/9.10-hosted-service-background-jobs.mdx): **"nếu job này không chạy một lần, có ai biết không?"**
...
builder.Services.AddHangfire(config => config
    ...
    .UseSqlServerStorage(connectionString, new SqlServerStorageOptions
    {
        ...
        QueuePollInterval = TimeSpan.Zero,          // dung thong bao, khong poll
...
API chỉ đưa job vào hàng đợi; một tiến trình worker riêng xử lý. Nhờ đó job nặng không làm chậm request, và bạn scale hai thứ độc lập — cùng lý do ở [bài 9.10](../../stage-03-aspnet-core-backend/module-09-web-api-professional/9.10-hosted-service-background-jobs.mdx).

### 14.7.3 — Tham số phải nhỏ

```csharp
// SAI — serialize CA doi tuong vao database
...
```csharp
// SAI — chay hai lan la gui hai email
...
```csharp
// DUNG — kiem tra truoc, ghi nhan sau
public async Task SendWelcomeAsync(int customerId)
{
    var customer = await _db.Customers.FindAsync([customerId]);
    if (customer is null) return;                          // da bi xoa

    if (customer.WelcomeEmailSentAt is not null)
    {
        _logger.LogInformation("Email chao mung da gui cho {Id}, bo qua", customerId);
        return;                                             // DA GUI
    }
...
Vẫn còn một khoảng trống: tiến trình chết **giữa** gửi email và lưu cờ. Đó là bản chất của at-least-once — bạn giảm xác suất chứ không loại bỏ được. Với thao tác thật sự không được lặp, cần idempotency key ở phía dịch vụ nhận ([bài 14.10](14.10-idempotency-and-retry.mdx)).
...
public async Task SyncAsync(int customerId)
{
    ...
    {
        // Du lieu sai -> retry KHONG giup gi
        _logger.LogError(ex, "Du lieu khach hang {Id} khong hop le", customerId);
        await _alerts.NotifyAsync($"Sync that bai cho khach hang {customerId}");
        return;                                 // ket thuc, khong retry
    }
}
```
...
if (failedCount > 10) await _alerts.NotifyAsync($"{failedCount} job that bai");
```
...
4. **Không phải message bus.** Không có publish/subscribe, không có định tuyến — đó là việc của [bài 14.9](14.9-message-bus-intro.mdx).
