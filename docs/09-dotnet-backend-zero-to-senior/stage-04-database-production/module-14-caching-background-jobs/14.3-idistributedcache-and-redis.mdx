---
title: "14.3 — 3. IDistributedCache và Redis"
description: "Chi phí serialize, IDistributedCache che mất sức mạnh Redis, thiết kế khoá, Redis chết thì sao, và lệnh không bao giờ chạy trên production."
sidebar_position: 3
---
...
<Checklist
  title="Danh sách rà soát cache phân tán"
  items={[
    { text: "Chỉ cache vào Redis những thứ đắt để tạo ra, không phải chỉ hơi chậm." },
    { text: "InstanceName được đặt nếu nhiều ứng dụng dùng chung Redis." },
    { text: "AbortOnConnectFail = false để ứng dụng khởi động được khi Redis chưa lên." },
    { text: "SyncTimeout đủ thấp để thất bại nhanh thay vì chờ lâu." },
    { text: "Khoá cache có tenant id, phiên bản, và sinh từ hằng số." },
    { text: "Deserialize được bọc try/catch và coi lỗi là cache miss." },
    { text: "Redis không khả dụng không làm hỏng request; có fallback về nguồn." },
    { text: "Health check Redis trả Degraded, không trả Unhealthy." },
    { text: "IConnectionMultiplexer đăng ký Singleton." },
    { text: "Không có KEYS hay FLUSHALL nào trong code." },
    { text: "Xoá nhóm khoá dùng tập tag, không dùng SCAN theo mẫu." },
    { text: "maxmemory và maxmemory-policy được cấu hình đúng theo nội dung Redis chứa." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao cache một truy vấn nhanh vào Redis lại phản tác dụng?",
      answer: "Vì một lượt đi về Redis mất khoảng nửa tới một mili giây, cộng chi phí serialize và deserialize thường còn lớn hơn cả mạng. Cache truy vấn 2 mili giây vào Redis là làm chậm hệ thống; chỉ nên cache những thứ đắt để tạo ra."
    },
    {
      question: "InstanceName trong cấu hình Redis để làm gì?",
      answer: "Nó thêm tiền tố vào mọi khoá, cần thiết khi nhiều ứng dụng dùng chung một Redis. Không có nó, khoá của ứng dụng này va chạm với ứng dụng kia."
    },
    {
      question: "Làm sao xử lý việc đổi hình dạng DTO đã cache?",
      answer: "Đưa phiên bản vào khoá cache. Khi đổi DTO thì tăng phiên bản, cache cũ tự bị bỏ qua và hết hạn, nên bạn không cần xoá gì khi triển khai. Thêm trường thường vẫn deserialize được, nhưng xoá hoặc đổi kiểu thì sẽ ném exception."
    },
    {
      question: "Redis chết thì ứng dụng nên làm gì?",
      answer: "Vẫn phục vụ được, chỉ chậm hơn, vì cache là tối ưu chứ không phải phụ thuộc bắt buộc. Bọc lời gọi cache trong try catch và đi thẳng nguồn khi lỗi, và health check Redis nên trả Degraded chứ không phải Unhealthy để pod không bị rút khỏi load balancer."
    },
    {
      question: "IDistributedCache che mất những gì của Redis?",
      answer: "Bộ đếm nguyên tử, tập hợp và sorted set, xoá theo mẫu khoá, pub sub, và Lua script. Nó chỉ có năm thao tác cơ bản. Mẫu thực dụng là dùng IDistributedCache cho cache thông thường và IConnectionMultiplexer cho những chỗ cần sức mạnh Redis."
    },
    {
      question: "Vì sao KEYS là lệnh nguy hiểm trên production?",
      answer: "Redis chạy đơn luồng cho lệnh, nên KEYS quét toàn bộ keyspace sẽ chặn mọi lệnh khác trong nhiều giây với keyspace lớn. Dùng SCAN để duyệt theo lô, hoặc tốt hơn là duy trì một tập khoá theo tag để xoá theo nhóm."
    }
  ]}
/>
