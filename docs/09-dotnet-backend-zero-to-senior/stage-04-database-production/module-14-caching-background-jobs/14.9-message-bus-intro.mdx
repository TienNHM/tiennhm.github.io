...
```csharp
// 1. Chi id — consumer tu nap du lieu
public record CustomerCreated(int CustomerId);

// 2. Du du lieu — consumer khong can goi nguoc
    ...
    : IConsumer<CustomerCreated>
{
    public async Task Consume(ConsumeContext<CustomerCreated> context)
    {
        ...
        logger.LogInformation("Gui email chao mung cho {CustomerId}", message.CustomerId);
...
```csharp
await _db.SaveChangesAsync(ct);                        // (1) luu database
...
await _db.SaveChangesAsync(ct);                        // luu customer VA message
await _bus.Publish(new CustomerCreated(id), ct);       // ghi vao bang outbox
```
...
Cái giá: thêm một bảng, thêm độ trễ bằng `QueryDelay`, và message có thể publish **hai lần** (tiến trình chết sau khi publish, trước khi đánh dấu đã gửi) — nên consumer vẫn phải **idempotent** ([bài 14.10](14.10-idempotency-and-retry.mdx)).

Mẫu này đã nói ở [bài 11.8](../../stage-03-aspnet-core-backend/module-11-signalr/11.8-ihubcontext-server-push-outside-hub.mdx) và chi tiết ở [Module 17](../../stage-05-senior-engineering/module-17-distributed-systems/).
...
welcome-email_skipped  -> message khong co consumer
```
...
| Hàng đợi trong tiến trình | **`Channel`** — [bài 6.9](../../stage-02-csharp-professional/module-06-async-programming/6.9-channels-and-producer-consumer.mdx) |
