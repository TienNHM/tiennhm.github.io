...
SET STATISTICS TIME ON;    -- CPU va thoi gian thuc
...
```sql
-- CustomerCode la VARCHAR(20), tham so la NVARCHAR
WHERE CustomerCode = @code
-- => CONVERT_IMPLICIT tren COT => index khong dung duoc
```
...
```sql
-- 1. OPTION (RECOMPILE) — plan moi moi lan. Tot cho SP chay it, tham so lech nhieu.
SELECT * FROM Leads WHERE Status = @Status OPTION (RECOMPILE);

-- 2. OPTIMIZE FOR UNKNOWN — dung thong ke trung binh, khong sniff
SELECT * FROM Leads WHERE Status = @Status OPTION (OPTIMIZE FOR UNKNOWN);

-- 3. OPTIMIZE FOR gia tri cu the — khi biet ro truong hop pho bien
SELECT * FROM Leads WHERE Status = @Status OPTION (OPTIMIZE FOR (@Status = 'New'));

-- 4. Tach thanh cac SP rieng cho truong hop lech nhieu
```
...
```sql
-- 1. OPTION (RECOMPILE) — don gian nhat, hieu qua nhat
SELECT * FROM Leads
WHERE (@Status IS NULL OR Status = @Status)
  AND (@UserId IS NULL OR AssignedToUserId = @UserId)
OPTION (RECOMPILE);
...
```sql
-- 2. Dynamic SQL co tham so (LUU Y: van phai parameterized)
...
```csharp
// SAI — mot truy van cho MOI customer
...
```sql
-- SQL Server: top 20 theo tong thoi gian
SELECT TOP 20
