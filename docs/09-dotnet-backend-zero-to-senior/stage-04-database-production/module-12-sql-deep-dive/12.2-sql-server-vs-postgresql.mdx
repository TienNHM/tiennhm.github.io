---
title: "12.2 — 2. SQL Server so với PostgreSQL"
description: "Khác biệt thật sự: MVCC và bloat, collation phân biệt hoa thường, JSON vs JSONB, và những bẫy khi chuyển từ SQL Server sang PostgreSQL."
sidebar_position: 2
---
...
`RCSI` gần như luôn nên bật cho ứng dụng OLTP — nó xoá bỏ phần lớn tranh chấp đọc/ghi. Cái giá là `tempdb` phải chịu tải version store. Chi tiết ở [bài 12.7](12.6-transactions-and-locking.mdx) và trong bài [SQL isolation level](/blog/sql-isolation-level-lost-update-phantom-read).
...
<Checklist
  title="Danh sách rà soát khác biệt giữa hai hệ"
  items={[
    { text: "PostgreSQL: đã cấu hình autovacuum cho bảng ghi nhiều." },
    { text: "PostgreSQL: có giám sát table bloat." },
    { text: "SQL Server: đã bật READ_COMMITTED_SNAPSHOT cho ứng dụng OLTP." },
    { text: "Email và định danh được chuẩn hoá chữ thường khi ghi." },
    { text: "Biết rõ collation của database và hệ quả phân biệt hoa thường." },
    { text: "PostgreSQL: dùng snake_case không nháy kép, không mang PascalCase sang." },
    { text: "PostgreSQL: cột thời gian dùng TIMESTAMPTZ, không dùng TIMESTAMP." },
    { text: "PostgreSQL: dùng jsonb, không dùng json." },
    { text: "Dữ liệu có cấu trúc rõ ràng nằm ở cột thật, không nhét vào JSON." },
    { text: "Không cố viết SQL chạy được trên cả hai hệ." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "MVCC của PostgreSQL mang lại gì và có cái giá nào?",
      answer: "Mỗi UPDATE tạo phiên bản mới thay vì sửa dòng cũ, nên người đọc không bao giờ chặn người ghi và ngược lại. Cái giá là dòng chết tích luỹ, và bảng cập nhật thường xuyên sẽ phình lên gấp nhiều lần nếu VACUUM không kịp chạy. Đây là nguyên nhân phổ biến khiến PostgreSQL đột nhiên chậm sau vài tháng."
    },
    {
      question: "READ_COMMITTED_SNAPSHOT trên SQL Server để làm gì?",
      answer: "Nó bật cơ chế tương tự MVCC, xoá bỏ phần lớn tranh chấp giữa đọc và ghi ở mức READ COMMITTED. Gần như luôn nên bật cho ứng dụng OLTP; cái giá là tempdb phải chịu tải version store."
    },
    {
      question: "Bẫy collation khi chuyển từ SQL Server sang PostgreSQL là gì?",
      answer: "SQL Server mặc định không phân biệt hoa thường còn PostgreSQL thì có, nên truy vấn email viết hoa sẽ không tìm thấy dòng lưu chữ thường. Code không đổi gì và không báo lỗi, chỉ là đăng nhập thất bại; đây là lỗi tốn nhiều thời gian nhất khi chuyển đổi."
    },
    {
      question: "Nên xử lý phân biệt hoa thường thế nào?",
      answer: "Tốt nhất là chuẩn hoá về chữ thường khi ghi, với một CHECK constraint bảo vệ, khi đó index thường hoạt động bình thường. Hai cách khác là dùng extension citext hoặc tạo index trên biểu thức lower của cột."
    },
    {
      question: "JSON trên SQL Server khác jsonb trên PostgreSQL ra sao?",
      answer: "SQL Server lưu JSON như một chuỗi, nên muốn index phải tạo cột tính toán cho từng trường và bạn phải biết trước sẽ truy vấn theo trường nào. PostgreSQL có jsonb là kiểu dữ liệu thật với GIN index phục vụ mọi khoá trong tài liệu mà không cần khai trước."
    },
    {
      question: "Hiệu năng có nên là lý do chọn giữa hai hệ không?",
      answer: "Không. Với ứng dụng CRM, cả hai đều nhanh hơn nhiều so với chất lượng truy vấn và index của bạn: một index thiếu làm chậm gấp trăm lần trong khi khác biệt giữa hai hệ thường dưới hai mươi phần trăm. Chọn theo license, hệ sinh thái, extension cần dùng và kỹ năng của team."
    }
  ]}
/>
