---
title: "15.12 — Mini case study"
description: "Container bị OOM kill mỗi 4 giờ dù ứng dụng không rò rỉ bộ nhớ: .NET GC không biết về cgroup limit."
sidebar_position: 12
---
...
<Checklist
  title="Danh sách rà soát bộ nhớ container"
  items={[
    { text: "Đã so sánh RSS của tiến trình với GC Heap Size, không chỉ nhìn heap." },
    { text: "Container nhỏ dùng Workstation GC hoặc có giới hạn heap tường minh." },
    { text: "Nếu giới hạn heap, dùng phần trăm thay vì giá trị cố định." },
    { text: "Chừa khoảng 25% memory limit cho native memory." },
    { text: "Có đặt requests.memory và limits.memory." },
    { text: "Không đặt limits.cpu nếu không có lý do cụ thể." },
    { text: "Có cảnh báo khi working set vượt 85% giới hạn." },
    { text: "Có theo dõi Restart Count của pod." },
  ]}
/>
...
<FAQSection
  items={[
    {
      question: "Vì sao heap ổn định mà container vẫn bị OOM kill?",
      answer: "Vì GC Heap Size chỉ đếm object đang sống, không đếm vùng nhớ mà GC đã đặt trước, native memory, thread stack và assembly đã nạp. Kubernetes nhìn vào RSS của tiến trình, và khoảng cách giữa hai con số có thể lên tới hàng GB."
    },
    {
      question: "Vì sao Server GC gây vấn đề trong container nhỏ?",
      answer: "Vì nó tạo một heap cho mỗi core CPU và được thiết kế để đánh đổi bộ nhớ lấy thông lượng, tức không thu gom chỉ vì đã dùng nhiều. Trên máy chủ nhiều core với container giới hạn nhỏ, nó tính toán như thể có rất nhiều bộ nhớ."
    },
    {
      question: "Đánh đổi khi chuyển sang Workstation GC là gì?",
      answer: "Nó dùng một heap và thu gom thường xuyên hơn, nên tốn ít bộ nhớ hơn nhiều nhưng thông lượng giảm khoảng mười tới mười lăm phần trăm. Với container nhỏ và tải vừa phải, đó là đánh đổi đúng."
    },
    {
      question: "Vì sao dùng DOTNET_GCHeapHardLimitPercent tốt hơn giá trị cố định?",
      answer: "Vì nó tự thích ứng khi bạn đổi memory limit của container, nên không phải đồng bộ hai con số bằng tay và không có nguy cơ chúng lệch nhau sau một lần chỉnh."
    },
    {
      question: "Vì sao thường không nên đặt limits.cpu?",
      answer: "Vì khi vượt giới hạn CPU, Kubernetes không giết pod mà bóp nó, khiến ứng dụng chậm đi một cách khó chẩn đoán vì mọi chỉ số trông bình thường trừ độ trễ. Với ứng dụng có đỉnh tải ngắn, throttling gây hại nhiều hơn lợi."
    },
    {
      question: "Chỉ số nào là con số Kubernetes dùng để quyết định giết pod?",
      answer: "container_memory_working_set_bytes. Chỉ nhìn GC Heap Size sẽ bỏ sót hoàn toàn vấn đề, vì nó không bao gồm vùng nhớ GC đặt trước và native memory."
    }
  ]}
/>
