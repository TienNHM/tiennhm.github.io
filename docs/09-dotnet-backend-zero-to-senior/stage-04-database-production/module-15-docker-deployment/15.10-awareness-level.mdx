...
**Pod là thứ tạm thời.** Nó bị thay bất cứ lúc nào: khi deploy, khi node cần bảo trì, khi scheduler cân bằng lại, khi bạn sửa một biến môi trường. Mọi thiết kế ứng dụng phải giả định điều đó — đúng như [bài 15.2](15.2-docker-fundamentals.mdx).
...
**1. `memory limit` điều khiển GC.** .NET đọc giới hạn cgroup và đặt heap khoảng 75% của nó ([bài 15.2](15.2-docker-fundamentals.mdx)). Đặt 512Mi nghĩa là heap khoảng 384Mi; phần còn lại cho runtime, thread stack và bộ nhớ native.
...
Đã nói ở [bài 8.10](../../stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.10-health-checks.mdx), nhưng đây là nơi nó thành cấu hình thật:
...
Nếu `ShutdownTimeout` lớn hơn `terminationGracePeriodSeconds`, Kubernetes gửi **SIGKILL** trước khi ứng dụng dọn xong — và mọi cấu hình shutdown gọn gàng trở thành vô nghĩa ([bài 15.3](15.3-dockerfile-for-aspnet-core.mdx)).
