---
title: "Thêm 4 index làm INSERT chậm 6 lần: cái giá không ai nhắc khi bảo bạn đánh index"
slug: chi-phi-ghi-cua-index
description: "Mọi hướng dẫn tối ưu đều bảo thêm index, rất ít bài nói về hoá đơn. Đo trên PostgreSQL 16 với cùng 200.000 dòng: bảng không index chèn hết 287,8 ms và nặng 10,2 MB, bảng bốn index mất 1.722,3 ms và nặng 27 MB. Bài viết phân tích chi phí ghi, quy tắc leftmost prefix, index trùng lặp, và cách quyết định index nào đáng giữ."
keywords: [chi phi ghi cua index, index lam cham insert, write amplification index, composite index leftmost prefix, index trung lap, redundant index, index selectivity, covering index, index only scan, khi nao khong nen danh index, thiet ke index, database design index, postgresql index size, pg_stat_user_indexes, index khong duoc dung bao gio, toi uu database ghi nhieu]
...
# Thêm 4 index làm INSERT chậm 6 lần
...
Bài này đào sâu [bài Thiết kế database](/docs/database/learn-sql-in-30-days/27-database-design-best-practices) và [bài Index](/docs/database/learn-sql-in-30-days/15-index) trong series học SQL 30 ngày. Mọi số liệu đo thật trên PostgreSQL 16.11.
...
## Leftmost prefix: thứ tự cột là một quyết định thiết kế {#leftmost-prefix}
...
## Ba loại index nên xoá {#index-nen-xoa}
...
## Khi nào chi phí ghi đáng trả {#khi-nao-dang-tra}
...
<Checklist
  title="Rà soát index trước khi thêm cái mới"
  items={[
    { text: "Kiểm tra tỷ lệ đọc/ghi của bảng — bảng ghi nhiều thì mỗi index là thuế nặng" },
    { text: "Xem index sẵn có đã phủ được truy vấn theo leftmost prefix chưa" },
    { text: "Xoá index trùng lặp: (a) khi đã có (a, b)" },
    { text: "Dùng pg_stat_user_indexes tìm index có idx_scan = 0" },
    { text: "Với cột lệch nặng, cân nhắc index một phần thay vì index đầy đủ" },
    { text: "Đo lại thời gian ghi sau khi thêm index, đừng chỉ đo thời gian đọc", checked: true }
  ]}
/>
...
|---|---|
| [Thiết kế database](/docs/database/learn-sql-in-30-days/27-database-design-best-practices) | Cân bằng đọc và ghi khi thiết kế schema |
| [Index](/docs/database/learn-sql-in-30-days/15-index) | Cấu trúc B-tree và cú pháp tạo index |
| [Tối ưu truy vấn SQL](/docs/database/learn-sql-in-30-days/28-sql-query-optimization) | Viết lại truy vấn trước khi nghĩ tới index mới |
| [Hiệu năng truy vấn](/docs/database/learn-sql-in-30-days/20-query-performance) | Đọc execution plan để biết index có được dùng không |
| [Transactions và ACID](/docs/database/learn-sql-in-30-days/16-transactions-acid) | Index làm transaction ghi dài hơn và giữ khoá lâu hơn |

Mặt kia của câu chuyện — index **có** nhưng **không được dùng** — nằm ở [bài về sargability](/blog/sql-index-khong-duoc-dung-sargable). Hai bài đó là hai nửa của cùng một câu hỏi: bạn đang trả chi phí ghi cho những index nào, và trong số đó cái nào thật sự phục vụ được truy vấn của bạn.
...
<FAQSection
  ...
  items={[
    {
      question: "Thêm index làm chậm ghi bao nhiêu?",
      answer: "Đo trên PostgreSQL 16 với cùng 200.000 dòng, bảng không có index chèn xong trong 287,8 ms còn bảng có bốn index mất 1.722,3 ms, tức là chậm khoảng 6 lần. Dung lượng cũng tăng từ 10,2 MB lên 27 MB. Lý do là mỗi INSERT phải chèn thêm một mục vào từng cây B-tree, giữ cho các cây cân bằng, và ghi thêm vào write-ahead log cho từng thay đổi. Chi phí này phải trả kể cả khi không câu truy vấn nào dùng tới các index đó."
    },
    {
