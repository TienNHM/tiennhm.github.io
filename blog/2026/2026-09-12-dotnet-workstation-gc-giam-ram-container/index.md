---
title: "Vì sao .NET ngốn 96% RAM container dù app đang rảnh? Server GC và cách giảm 65% bộ nhớ"
...
# Vì sao .NET ngốn 96% RAM container dù app đang rảnh?
...
<Checklist
  title="Các bước áp dụng"
  items={[
    { text: "Đọc anon trong /sys/fs/cgroup/memory.stat để có con số thật, đừng tin docker stats" },
    { text: "Đo nhiều lần cách quãng — đứng yên là retained heap, tăng dần mới là rò rỉ" },
    { text: "Kiểm tra env trong container xem đã có cấu hình GC nào chưa" },
    { text: "Đặt DOTNET_gcServer=0 và DOTNET_GCConserveMemory=5" },
    { text: "Nâng trần RAM làm đệm — bổ sung chứ không thay thế việc đổi GC" },
    { text: "Recreate container rồi đo lại ở nhiều mốc thời gian, không kết luận ngay lúc vừa khởi động", checked: true }
  ]}
/>
...
<FAQSection
  ...
  items={[
    {
      question: "DOTNET_gcServer=0 có làm chậm ứng dụng không?",
      answer: "Có thể, nhưng tuỳ tải. Workstation GC chạy GC thường xuyên hơn với mỗi lần nhẹ hơn, nên độ trễ từng request thường ổn định hơn, còn throughput tổng thể khi tải cao và nhiều core thì thấp hơn Server GC. Với API lưu lượng thấp trong container nhỏ, khác biệt gần như không đo được, trong khi RAM tiết kiệm là thấy ngay. Với dịch vụ tải cao, phải benchmark trước khi đổi."
    },
    {
      question: "Làm sao phân biệt rò rỉ bộ nhớ với heap bị giữ lại?",
      answer: "Đo nhiều lần cách quãng và xem xu hướng. Rò rỉ thì bộ nhớ TĂNG DẦN theo thời gian và không giảm kể cả khi ứng dụng rảnh. Heap bị giữ lại thì ĐỨNG YÊN ở một mức. Trong ca này, ba lần đo cách nhau 5 giây cho cùng con số 987.6 MiB, và app không nhận request nào suốt 2 giờ — đó là heap bị giữ, không phải rò rỉ. Nếu nghi rò rỉ thật thì mới cần tới dotnet-counters, dotnet-dump và phân tích heap."
    },
    {
      question: "GCConserveMemory=5 nghĩa là gì, đặt bao nhiêu là đúng?",
      answer: "Đây là thang từ 0 đến 9 điều chỉnh mức độ tích cực của GC trong việc nén heap và trả bộ nhớ về hệ điều hành. 0 là mặc định, 9 là tiết kiệm bộ nhớ tối đa nhưng tốn CPU nhất. Giá trị 5 là điểm cân bằng hợp lý để bắt đầu. Nên tăng dần và đo, đừng nhảy thẳng lên 9."
    },
    {
      question: "Vì sao docker stats báo 96% mà anon chỉ 80%?",
      answer: "Vì MemUsage trong docker stats bao gồm cả page cache — bộ nhớ kernel dùng đệm file. Phần đó kernel thu hồi được bất cứ lúc nào khi thiếu RAM, nên nó không tạo áp lực thật và không gây OOM kill. Con số quyết định là anon trong /sys/fs/cgroup/memory.stat, phần bộ nhớ thật của tiến trình mà kernel không thu hồi được."
    },
    {
      question: "Vì sao .NET dừng quanh 768 MB trong container 1 GiB?",
      answer: "Khi phát hiện đang chạy trong container có giới hạn bộ nhớ, .NET tự đặt GC heap hard limit bằng 75% giới hạn cgroup. Với trần 1 GiB thì heap được phép dùng tới khoảng 768 MB. Cộng thêm bộ nhớ native, stack các luồng và JIT code sẽ ra con số anon khoảng 820 MB quan sát được. Muốn đổi tỷ lệ này thì dùng DOTNET_GCHeapHardLimitPercent."
    },
    {
      question: "Nâng trần RAM lên có phải là cách sửa không?",
      answer: "Không, đó chỉ là đệm. Nâng trần mà giữ Server GC thì GC heap hard limit cũng tăng theo 75%, và ứng dụng sẽ phình lên lấp đầy chỗ mới. Trong ca này trần được nâng từ 1G lên 1.5G nhưng đó là biện pháp bổ sung, thứ thật sự tạo ra mức giảm 65% là việc đổi sang Workstation GC."
    }
  ]}
/>
...
- [Module 19 — Performance Engineering](/docs/dotnet-backend-zero-to-senior/stage-05-senior-engineering/module-19-performance-engineering) — Performance engineering: profiling, allocation, GC, benchmarking — định lượng cải tiến cho hot path CRM.
...
- [2.10 — 8. .NET Runtime và Ecosystem](/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-02-computer-science-basics/2.10-dotnet-runtime-and-ecosystem) — Từ mã C# tới mã máy: IL, JIT, AOT.
