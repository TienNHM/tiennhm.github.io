...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-05-advanced-csharp/5.2-generics
...
Một lưu ý thực dụng về `IRepository` chung: nó hấp dẫn nhưng hay dẫn tới interface **chung chung tới mức vô dụng**. Mỗi thực thể thường có nhu cầu truy vấn riêng (`GetOverdueOrders`, `FindByEmail`), và nhồi hết vào một interface generic thì quay lại đúng vấn đề interface quá lớn ở [bài 4.5](https://tiennhm.io.vn/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.4-interface). Dùng generic cho phần thật sự chung, và viết interface riêng cho phần đặc thù.
...
Thêm một triệu số nguyên vào `ArrayList` và vào `List`, đo thời gian và bộ nhớ. Ghi lại tỉ lệ.
...
**Vì sao generics không có chi phí này.** Với kiểu giá trị, trình biên dịch JIT sinh **mã máy riêng** cho từng kiểu đóng. `List` bên trong dùng thẳng một mảng `int[]`, không có đối tượng trung gian nào. Với kiểu tham chiếu thì mã được dùng chung, cũng không phát sinh chi phí.

Đây là khác biệt quan trọng so với generics của Java, nơi kiểu bị xoá lúc biên dịch và `List` vẫn phải boxing.
...
Viết `class Animal` và `class Dog : Animal`. Thử gán `List` cho `List` và ghi lỗi biên dịch. Làm lại với `IEnumerable` và giải thích vì sao nó hợp lệ.

**Tiêu chí hoàn thành:** bạn giải thích được vì sao `List` **không thể** covariant, bằng một ví dụ cụ thể chứ không bằng lý thuyết.

Gợi ý và lời giải — Bài 2

**Gợi ý.** Giả sử C# **cho phép** gán `List` cho `List`. Bạn có thể viết dòng nào tiếp theo để phá vỡ hệ thống kiểu?
...
**Vì sao `List` không thể covariant — ví dụ cụ thể:**
...
Nếu covariance được phép cho `List`, bạn thêm được một `Cat` vào một danh sách mà kiểu thật của nó là `List`. Hệ thống kiểu bị phá vỡ, và lỗi chỉ lộ ra lúc chạy. Trình biên dịch chặn ngay từ đầu.

**Vì sao `IEnumerable` thì được.** Vì nó chỉ cho phép **lấy ra**, không cho phép **đưa vào**:
...
|---|---|---|---|
| `out T` | Covariant — con thay được cha | Chỉ đầu ra | `IEnumerable`, `IReadOnlyList` |
| `in T` | Contravariant — cha thay được con | Chỉ đầu vào | `IComparer`, `Action` |
| `T` | Bất biến | Cả hai | `List`, `IList` |

**Ứng dụng thực tế.** Đây là lý do nên trả về `IEnumerable` hoặc `IReadOnlyList` thay vì `List` từ API công khai — ngoài lý do đóng gói ở [bài 4.3](../module-04-csharp-core/4.2-encapsulation.mdx), nó còn cho người gọi sự linh hoạt về kiểu:

```csharp
public IReadOnlyList<Animal> LayTatCa() => _chos;   // trả List<Dog> được, nhờ covariance
```

**Một giới hạn cần biết.** Covariance chỉ áp dụng cho **kiểu tham chiếu**. `IEnumerable` không gán được cho `IEnumerable`, vì chuyển đổi đó cần boxing — và boxing thì không phải phép chuyển đổi tham chiếu đơn thuần.

#### Bài 3 — `static` trong lớp generic

Cài `Counter`, gán giá trị khác nhau cho ba kiểu đóng và in ra. Giải thích kết quả bằng lời của bạn.

**Tiêu chí hoàn thành:** bạn nêu được một ứng dụng hữu ích của đặc điểm này, và một cái bẫy của nó.

Gợi ý và lời giải — Bài 3

**Gợi ý.** `Counter` và `Counter` có phải cùng một lớp không? Hãy nghĩ về việc trình biên dịch JIT sinh mã riêng cho từng kiểu đóng.
...
**Giải thích.** `Counter` không phải một lớp — nó là một **khuôn** để sinh ra lớp. Mỗi kiểu đóng `Counter`, `Counter`, `Counter` là một **lớp riêng biệt** ở mức runtime, và mỗi lớp có vùng dữ liệu tĩnh riêng của nó.
...
`Counter.Value` là **một** biến duy nhất cho toàn bộ ứng dụng. Hai request đồng thời cùng tăng nó sẽ gặp đúng vấn đề đua dữ liệu đã chứng minh ở [bài 4.2](../module-04-csharp-core/4.1-class-and-object.mdx).
