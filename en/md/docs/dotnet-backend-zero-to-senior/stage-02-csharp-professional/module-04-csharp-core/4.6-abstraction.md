...
| Chỗ | Vấn đề |
|---|---|
| `IQueryable` | Người gọi viết truy vấn LINQ, và **những truy vấn đó chỉ EF Core dịch được**. Đổi sang Dapper là mọi nơi gọi đều hỏng. |
...
| `EntityEntry` | Kiểu này **thuộc về EF Core**. Tầng nghiệp vụ buộc phải tham chiếu EF Core chỉ để biên dịch. |
...
**Một lưu ý về hiệu năng.** Đừng dùng ngoại lệ cho luồng bình thường. Nếu "email đã tồn tại" là tình huống xảy ra thường xuyên chứ không phải ngoại lệ, hãy trả về `Result` như [Module 5](../module-05-advanced-csharp/) trình bày — ném và bắt ngoại lệ tốn kém hơn nhiều so với trả về một giá trị.
