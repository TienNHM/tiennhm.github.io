...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-04-csharp-core/4.8-records-and-value-objects
...
Tạo `record Order(int Id, List Tags)`, tạo hai thể hiện cùng nội dung, so sánh bằng `==`. Ghi lại kết quả và thử các cách sửa.

**Tiêu chí hoàn thành:** bạn biết cách nào **thật sự** sửa được — và cách nào chỉ *có vẻ* sửa được.

Gợi ý và lời giải — Bài 1

**Gợi ý.** `record` tự sinh `Equals` bằng cách gọi `Equals` của **từng thành phần**. Vậy câu hỏi thật là: `List.Equals` so sánh cái gì?
...
Bằng nhau **chỉ khi** hai record trỏ tới cùng một mảng nền. `ImmutableArray` bảo đảm nội dung không đổi được, nhưng phép so sánh bằng của nó vẫn dựa trên tham chiếu tới mảng bên dưới.
