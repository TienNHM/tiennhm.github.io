...
|-----------|-----------|------------|-------------|
| `IOptions` | Lúc app start | Không | Singleton, config không đổi |
| `IOptionsSnapshot` | Mỗi request | Có | Scoped, config có thể reload |
| `IOptionsMonitor` | Realtime | Có (callback) | Singleton, cần phản ứng khi config thay đổi |
