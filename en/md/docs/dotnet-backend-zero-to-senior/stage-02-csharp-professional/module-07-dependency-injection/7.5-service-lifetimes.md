...
**`DbContext`**: `AddDbContext()` đăng ký `Scoped` sẵn. Đừng đổi lifetime. Cần dùng ngoài request thì dùng `AddDbContextFactory` ([bài 6.6](../module-06-async-programming/6.6-task-parallel-library.mdx)).
...
builder.Services.AddSingleton<BaoCaoService>();                       // nhận CrmDbContext
```
...
```csharp
public class BaoCaoService(IServiceScopeFactory scopeFactory)
{
...
- Bắt buộc với `IAsyncEnumerable`, như [bài 6.7](../module-06-async-programming/6.7-async-patterns.mdx) đã nêu.
...
{
    public static int SoLan;
    public void Dispose() => SoLan++;
}
...
Console.WriteLine($"sau 1000 resolve: Dispose gọi {Disp.SoLan} lần");

(p as IDisposable)?.Dispose();
Console.WriteLine($"sau khi dispose provider: Dispose gọi {Disp.SoLan} lần");
```
...
**Một lưu ý về `HttpClient`.** Đây chính là lý do `IHttpClientFactory` tồn tại: `HttpClient` cài `IDisposable`, và cả hai cách dùng ngây thơ — tạo mới mỗi lần, hoặc đăng ký `Transient` — đều gây vấn đề, như [bài 2.3](../../stage-01-foundation/module-02-computer-science-basics/2.3-how-the-internet-works.mdx) đã đo.
