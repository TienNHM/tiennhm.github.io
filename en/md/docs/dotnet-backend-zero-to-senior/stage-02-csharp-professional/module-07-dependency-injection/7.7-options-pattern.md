# 7.7 — 6. Options Pattern

> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-07-dependency-injection/7.7-options-pattern
...
Tiêm `IOptionsSnapshot` vào một Singleton là **captive dependency** ([bài 7.5](7.4-service-lifetimes.mdx)) — nó bị giam và không bao giờ đọc lại nữa, im lặng.
...
Đây cũng là câu trả lời cho vấn đề ở [bài 7.7](7.6-program-cs-and-webapplicationbuilder.mdx): cần một service để cấu hình một service khác mà **không** gọi `BuildServiceProvider()`. `IConfigureOptions` làm điều tương tự cho việc thiết lập giá trị:
...
**Gợi ý.** `IOptionsSnapshot` được đăng ký là **`Scoped`**. Tiêm một `Scoped` vào một `Singleton` chính là captive dependency ở [bài 7.5](7.4-service-lifetimes.mdx).
...
1. **Giữ và giải phóng đăng ký.** `OnChange` trả về một `IDisposable`; bỏ nó đi là rò rỉ, đúng kiểu rò rỉ event ở [bài 5.5](../module-05-advanced-csharp/5.4-delegates-and-events.mdx).
...
3. **Không làm việc nặng hoặc chờ lâu.** Callback chạy đồng bộ; việc nặng nên được đẩy sang một hàng đợi như [bài 6.9](../module-06-async-programming/6.8-channels-and-producer-consumer.mdx) mô tả.
...
- **Bài trước:** [7.6 — 5. Program.cs và WebApplicationBuilder](7.6-program-cs-and-webapplicationbuilder.mdx)
- **Bài tiếp theo:** [7.8 — 7. Keyed Services (.NET 8)](7.8-keyed-services.mdx)
