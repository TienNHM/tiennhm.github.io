# 7.13 — Review and Assessment

> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-07-dependency-injection/7.13-review-and-assessment
> Ôn tập & tự đánh giá Dependency Injection: lifetime (Transient/Scoped/Singleton), Options Pattern và checklist trước khi sang phần Data.
...
3. Inject `IOptions` vào `LeadService`
4. Viết unit test kiểm tra `CanAssignLeadAsync` trả về `false` khi agent đã đạt `MaxLeadsPerAgent`
5. *Nâng cao*: dùng `IOptionsMonitor`, thay đổi `appsettings.json` lúc app đang chạy và quan sát callback
...
| Options Pattern | `IOptions` / `IOptionsSnapshot` / `IOptionsMonitor` — đọc config type-safe |
...
4. `IOptionsMonitor` khác `IOptions` ở?
...
- **Bài trước:** [7.12 — Ví dụ thực tế nhanh](7.12-quick-real-world-example.mdx)
