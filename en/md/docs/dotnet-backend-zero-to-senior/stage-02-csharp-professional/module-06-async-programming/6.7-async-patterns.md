...
`ValueTask` là struct, tránh heap allocation khi kết quả đã sẵn có (cache hit).
...
> **Rule:** Dùng `ValueTask` khi method thường xuyên có synchronous fast-path (cache, hot-path). Không dùng mặc định — `Task` vẫn là lựa chọn an toàn hơn.
