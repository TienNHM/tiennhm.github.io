# 6.7 — 6. Common Pitfalls

> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-02-csharp-professional/module-06-async-programming/6.7-common-pitfalls
...
Cách chữa là `async` lên tới tận trên cùng. Cơ chế deadlock đã mổ xẻ ở [bài 6.4](6.3-configureawait.mdx) và trong bài [Gọi `.Result` khi nào thì deadlock](https://tiennhm.io.vn/blog/deadlock-result-wait-csharp).
...
Quy tắc nhận biết: **nếu tham số nhận `Action`, `Action` hay một delegate trả `void`, thì lambda `async` bạn truyền vào là `async void`.** Hậu quả đã nói ở [bài 6.3](6.2-task-and-async-await.mdx): vòng lặp kết thúc trước khi việc xong, và exception làm sập tiến trình.
...
#### Bài 2 — Làm mất phần tử với `List`

Dùng `Parallel.ForEachAsync` với 1000 phần tử cùng `Add` vào một `List`. Chạy 10 lần và ghi lại số phần tử của mỗi lần. So sánh với `ConcurrentBag`.
...
**Vì sao mất.** `List.Add` không phải thao tác nguyên tử. Nó gồm ba bước:
...
| Nhu cầu | Kiểu |
|---|---|
| Thêm vào, không cần thứ tự | `ConcurrentBag` |
| Tra cứu theo khoá | `ConcurrentDictionary<K,V>` |
| Hàng đợi vào trước ra trước | `ConcurrentQueue` |
| Cần giữ thứ tự của nguồn | Thu thập bằng `Task.WhenAll` |

**Một lưu ý về `ConcurrentBag`.** Nó **không** bảo đảm thứ tự, và cũng không nhanh cho mọi tình huống — nó được tối ưu cho trường hợp cùng một luồng vừa thêm vừa lấy. Với việc chỉ thêm rồi đọc một lần ở cuối, `ConcurrentQueue` thường nhanh hơn.
...
| 5 | `Select(async ...)` lười | `IEnumerable` không được `await` | **Không** |
| 6 | `List` dùng chung | `.Add` trong ngữ cảnh song song | **Không** |
