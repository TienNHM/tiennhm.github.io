...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.9-health-checks
> Liveness và readiness khác nhau ở đâu, vì sao trộn hai loại gây restart loop, custom health check, tag, và cấu hình probe trong Docker/Kubernetes.
...
`Description` và `Exception` thường chứa connection string, tên máy chủ và thông điệp lỗi của database — đúng những thứ [bài 8.9](8.8-global-exception-handling.mdx) nói là không được lộ.
...
Và nhớ loại health check khỏi log truy cập ([bài 8.8](8.7-logging-with-serilog.mdx)) — probe mỗi 10 giây là 8.640 dòng log mỗi ngày cho mỗi instance.
...
- [ ] Health check không bao giờ ném exception, luôn trả HealthCheckResult.
...
### Liveness và readiness khác nhau thế nào?
...
### Vì sao liveness probe không được kiểm tra database?
...
### Custom health check cần theo nguyên tắc nào?

Ba nguyên tắc. Luôn có timeout, vì check không timeout sẽ treo cùng phụ thuộc và probe của orchestrator sẽ timeout dẫn tới restart loop. Không bao giờ ném exception mà trả HealthCheckResult.Unhealthy kèm exception. Và phải nhẹ, vì probe chạy mỗi 10 tới 30 giây trên mọi instance.

### Vì sao không trả Description ra endpoint công khai?
...
- **Bài trước:** [8.8 — 8. Global Exception Handling](8.8-global-exception-handling.mdx)
- **Bài tiếp theo:** [8.10 — Mở rộng và đào sâu](8.10-advanced-notes.mdx)
