# 8.1 — 1. Kiến trúc ASP.NET Core

> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-03-aspnet-core-backend/module-08-aspnet-core-fundamentals/8.1-aspnet-core-architecture
...
Nó cũng cấu hình logging (Console, Debug, EventSource), thiết lập Kestrel và IIS integration, đăng ký `IConfiguration`, `IWebHostEnvironment`, `ILogger`, và bật `ValidateScopes` **chỉ khi** môi trường là Development ([bài 7.5](../../stage-02-csharp-professional/module-07-dependency-injection/7.4-service-lifetimes.mdx)).
...
```csharp
app.MapGet("/config", (IConfiguration c) => new
{
    Value = c["Demo:Value"],
    Details = ((IConfigurationRoot)c).GetDebugView()
});
```
