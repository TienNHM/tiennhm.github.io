...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-05-senior-engineering/module-16-clean-architecture/16.15-mini-case-study
...
public class CrmService
{
    ...
    // 63 phuong thuc public, 4.217 dong
}
```
...
```
- Thoi gian chay bo test cua service nay:  8 phut (moi test dung 19 mock)
- So merge conflict trung binh moi tuan:   11 (ca doi sua cung file)
- Thoi gian dong bo cua developer moi:     ~3 ngay chi de hieu file nay
```
...
**Sai lầm 2 — tách theo tầng kỹ thuật.** Nỗ lực đầu tiên là tách `CrmService` thành `CrmReadService`, `CrmWriteService`, `CrmValidationService`. Kết quả: ba file lớn thay vì một, và **mỗi tính năng vẫn phải sửa cả ba** ([bài 16.11](16.11-anti-patterns-to-avoid.mdx)).
...
```csharp
// Buoc 1: tao handler, COPY logic nguyen ven (chua sua gi)
public sealed class ConvertLeadHandler(
    ILeadRepository leads,
    ICustomerRepository customers,
    IEventPublisher events)              // chi 3 dependency, khong phai 19
...
// Buoc 3: gio moi sua theo yeu cau nghiep vu moi
```
...
**3. Không có test.** Refactor code không có test là đánh bạc. Viết test đặc tả hành vi hiện tại trước — kể cả khi hành vi đó trông sai, vì có thể có ai đó đang phụ thuộc vào nó ([bài 16.10](16.10-testing-strategy-overview.mdx)).
...
- **Bài trước:** [16.14 — Mở rộng và đào sâu](16.14-advanced-notes.mdx)
- **Bài tiếp theo:** [16.16 — Ví dụ thực tế nhanh](16.16-quick-real-world-example.mdx)
