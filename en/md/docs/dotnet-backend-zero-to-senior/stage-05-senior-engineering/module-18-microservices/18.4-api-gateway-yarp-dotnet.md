...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-05-senior-engineering/module-18-microservices/18.4-api-gateway-yarp-dotnet
> Gateway làm gì và tuyệt đối không được làm gì, cấu hình YARP đầy đủ, health check, transform header, và vì sao gateway dễ thành điểm chết đơn.
...
- **Đường dẫn health check phải là `/health/ready`**, không phải `/health/live`. Liveness chỉ nói tiến trình còn sống; readiness nói nó sẵn sàng nhận request ([bài 15.9](../../stage-04-database-production/module-15-docker-deployment/15.9-production-monitoring.mdx)).
...
YARP tự động chuyển tiếp header `traceparent` theo chuẩn W3C Trace Context, nên nếu đã bật OpenTelemetry thì trace xuyên service hoạt động mà không cần cấu hình thêm ([bài 18.6](18.6-observability-three-pillars.mdx)).
...
app.MapGet("/mobile/dashboard", async (ISalesClient sales, IBillingClient billing, CancellationToken ct) =>
{
    ...
    await Task.WhenAll(leadsTask, invoicesTask);     // song song, khong tuan tu
...
`Task.WhenAll` ở đây quan trọng: gọi tuần tự thì độ trễ là **tổng**, gọi song song thì độ trễ là **max** ([bài 6.6](../../stage-02-csharp-professional/module-06-async-programming/6.6-task-parallel-library.mdx)).
...
**Danh sách rà soát gateway**

- [ ] Gateway không chứa quy tắc nghiệp vụ nào.
- [ ] Quyết định ở gateway chỉ dựa trên HTTP, không dựa trên dữ liệu nghiệp vụ.
- [ ] Có active health check và trỏ tới readiness, không phải liveness.
- [ ] Gateway chạy ít nhất hai instance sau load balancer.
- [ ] Gateway không giữ trạng thái cục bộ.
- [ ] Correlation id được sinh ở gateway và trả về trong response header.
- [ ] Timeout ở gateway ngắn hơn timeout của client.
- [ ] Có giám sát độ trễ riêng cho gateway.
- [ ] Nếu dùng BFF, mỗi BFF do đội frontend tương ứng sở hữu.
...
### Ranh giới giữa việc gateway nên làm và không nên làm là gì?

Gateway chỉ được ra quyết định dựa trên HTTP như đường dẫn, header và token. Nó không bao giờ được quyết định dựa trên dữ liệu nghiệp vụ như số tiền hay trạng thái khách hàng, vì khi đó quy tắc của một đội nằm trong hệ thống của đội khác.

### Vì sao active health check quan trọng với gateway?

Vì không có nó, gateway vẫn gửi request tới instance đã chết và người dùng nhận lỗi 502. Health check chủ động cho gateway loại instance hỏng khỏi vòng cân bằng tải trước khi có request nào bị ảnh hưởng.

### Vì sao health check phải trỏ tới readiness chứ không phải liveness?

Vì liveness chỉ nói tiến trình còn sống, còn readiness nói nó sẵn sàng nhận request. Một service vừa khởi động hoặc mất kết nối database vẫn sống nhưng chưa phục vụ được, và gửi request tới đó sẽ lỗi.

### Vì sao gateway là điểm chết đơn và xử lý thế nào?

Vì mọi request đều đi qua nó, nên gateway sập là cả hệ thống sập kể cả khi mọi service phía sau vẫn khoẻ. Phải chạy ít nhất hai instance sau load balancer, không giữ trạng thái cục bộ, giữ nó nhẹ, và giám sát riêng.

### Vì sao timeout ở gateway phải ngắn hơn timeout của client?

Vì nếu client bỏ cuộc trước, gateway vẫn giữ kết nối tới service phía sau. Dưới tải cao, số kết nối treo đó sẽ làm cạn tài nguyên của chính gateway.

### BFF khác gateway ở điểm nào?

Gateway là một cho cả hệ thống, chỉ định tuyến và xử lý mối quan tâm chung, do đội nền tảng sở hữu. BFF là một cho mỗi loại client, có logic ghép và cắt gọt dữ liệu cho UI cụ thể, và do chính đội frontend tương ứng sở hữu.
...
- **Bài trước:** [18.3 — 2. Service decomposition](18.3-service-decomposition.mdx)
- **Bài tiếp theo:** [18.5 — 4. Giao tiếp giữa các service](18.5-inter-service-communication.mdx)
