...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-05-senior-engineering/module-19-performance-engineering/19.2-benchmarkdotnet
...
Benchmark một hàm dùng `string.Split` và một hàm dùng `ReadOnlySpan`. So sánh cột `Allocated`.
...
**Gợi ý.** `string.Split` cấp phát một mảng **và** một chuỗi mới cho mỗi phần. `ReadOnlySpan` chỉ ghi nhớ vị trí bắt đầu và độ dài.
