...
- `lockMode` trong `` cùng `requestType="convert"` — dấu hiệu của **deadlock chuyển đổi khoá**: cả hai bên đang giữ khoá `S` và cùng muốn nâng lên `X`. Đây là biến thể rất hay gặp của mẫu "đọc rồi ghi trong cùng một transaction", và cách sửa là dùng `UPDLOCK` ngay từ lần đọc:
