...
> Nguồn: https://tiennhm.io.vn/en/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-02-computer-science-basics/2.6-api-and-json
> REST, JSON và cái bẫy số học: JavaScript không giữ nổi số nguyên 64-bit, nên một Id kiểu long sẽ âm thầm sai ở chữ số cuối. Kèm cách thiết kế response ổn định và xử lý lỗi cho đúng.
...
**3. Số tiền** — `double` của JSON gặp đúng vấn đề đã nói ở [bài 1.2](https://tiennhm.io.vn/docs/dotnet-backend-zero-to-senior/stage-01-foundation/module-01-programming-logic/1.2-variables-and-data-types). An toàn nhất là dùng chuỗi hoặc số nguyên đơn vị nhỏ nhất:
...
Lưu ý hiệu năng: `totalItems` trên bảng lớn đòi một câu `COUNT(*)` quét rất nhiều dòng. Với dữ liệu hàng triệu bản ghi, cân nhắc phân trang theo con trỏ (`cursor`) thay vì theo số trang — chi tiết ở bài [hiệu năng truy vấn](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/20-query-performance).
