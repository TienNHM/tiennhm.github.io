# Phần 3 - Next.js Best Practices

> Nguồn: https://tiennhm.io.vn/en/docs/agent-skills/agent-skills-02-nextjs-best-practices
> Tổng hợp guideline Next.js best practices cho Agent Skills – Routing, Rendering, Data Fetching, Images, Fonts, Metadata, API, Middleware, Environment, Performance, Security.
...
- **Nên:** Dùng `` cho ảnh trong app.

```diff
+ <Image src={url} alt="..." width={400} height={300} />
```

- **Không nên:** Dùng `` trực tiếp cho ảnh trong UI.
...
- **Nên:** Set `className` cho `` trong `layout.tsx`.
...
Dùng `metadata`/`generateMetadata` thay vì tự viết ``.
...
- **Nên:** Dùng `` cho internal links.

```diff
+ <Link href="/about">About</Link>
```

- **Không nên:** Dùng `` cho điều hướng nội bộ.
