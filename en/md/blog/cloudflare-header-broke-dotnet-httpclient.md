...
> Nguồn: https://tiennhm.io.vn/en/blog/cloudflare-header-broke-dotnet-httpclient
> On an e-commerce loyalty platform with roughly 3 million customers, an internal HTTP call started failing in production while the database, Kubernetes and the downstream service were all healthy. The culprit was Cloudflare's cf-ipcity header carrying the value Hồ Chí Minh. This post traces the investigation and shows where .NET actually throws — which is not where you would guess.
...
- Why headers cannot carry Vietnamese: see [part 2 on ASCII and obs-text](https://tiennhm.io.vn/blog/http-header-unicode-ascii-dotnet).
- How to fix it properly: see [part 3 on safe header forwarding](https://tiennhm.io.vn/blog/forward-http-header-an-toan-aspnet-core).
...
## Part 2 — The header that caused it all
...
That code exists for a legitimate reason: preserving correlation IDs, tokens, and tracing context across services. The problem is that it does not distinguish headers that are **ours** from headers **injected by infrastructure**. The fix is in [part 3](https://tiennhm.io.vn/blog/forward-http-header-an-toan-aspnet-core).

---

## Part 3 — The throw site is not where you would guess
...
## Part 4 — Why it gets in but cannot get out
...
## Part 5 — Why local would never have caught it
...
| Post | Content |
|---|---|
| [Can an HTTP header carry Vietnamese?](https://tiennhm.io.vn/blog/http-header-unicode-ascii-dotnet) | ASCII, obs-text, Latin-1, and how to encode properly when you need diacritics |
| [Forwarding headers safely in ASP.NET Core](https://tiennhm.io.vn/blog/forward-http-header-an-toan-aspnet-core) | Allowlists, `DelegatingHandler`, gateway filtering, and a test that keeps the fix |

---

## Frequently asked questions

### Why does HttpClient throw 'Request headers must contain only ASCII characters'?

Because the layer that serialises headers into bytes can only represent ASCII. What is notable is that this error does NOT appear when you add the header: measured on .NET 9.0.4, DefaultRequestHeaders.Add with the value 'Hồ Chí Minh' throws nothing at all, and TryAddWithoutValidation returns true. The exception only fires on SendAsync, and at that point not a single byte of the request has left the process.

### What is Cloudflare's cf-ipcity header and why does it cause errors?

cf-ipcity is one of the headers Cloudflare injects into incoming requests, containing the city name inferred from the client's IP. It is harmless in itself. It causes errors when an application forwards every incoming header verbatim onto an outgoing HttpClient call, because for visitors in Vietnam the value is a city name with diacritics — that is, non-ASCII — which the on-the-wire header format cannot carry.

### Why can ASP.NET Core receive a non-ASCII header when HttpClient cannot send one?
...
### Why does this bug never appear when running locally?
...
### Why do packet captures and downstream logs not help find this bug?
