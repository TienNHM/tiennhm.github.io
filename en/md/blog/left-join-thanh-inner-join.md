...
> Nguồn: https://tiennhm.io.vn/en/blog/left-join-thanh-inner-join
> Add one WHERE condition on a right-hand column and your LEFT JOIN silently turns into an INNER JOIN, taking with it exactly the rows you meant to keep. This post proves it with three numbers measured on PostgreSQL 16, explains SQL's logical processing order, and shows when a condition belongs in ON versus WHERE.
...
This post drills into [the JOIN lesson from my 30-day SQL series](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/10-join). Every number below is real output from PostgreSQL 16.11.
...
Not `TRUE`, not `FALSE`. And since `WHERE` keeps only rows evaluating to `TRUE`, Chi's row is dropped. That `UNKNOWN` mechanism is the [three-valued logic I wrote a separate post about](https://tiennhm.io.vn/blog/sql-null-not-in-three-valued-logic) — the same root cause showing up somewhere else.
...
## The exception: an anti-join is deliberate
...
`NOT EXISTS` states the intent directly — *"no such order exists"* — whereas `LEFT JOIN ... IS NULL` makes the reader reason backwards. And as covered in [the NULL post](https://tiennhm.io.vn/blog/sql-null-not-in-three-valued-logic), never replace it with `NOT IN`.

---

## How to spot that you have this bug
...
**`COUNT` comes out lower than expected.** Especially when counting per group — a group that should read zero disappears from the result entirely instead of showing 0. See [the GROUP BY and HAVING lesson](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/09-group-by-having) on how empty groups are handled.
...
## Learn more
This post drills into one point from my [30-day SQL series](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days):

| Lesson in the series | How it relates |
|---|---|
| [JOIN](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/10-join) | Join types and `ON` syntax |
| [SELECT and WHERE](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/03-select-where) | Why `WHERE` keeps only `TRUE` |
| [Subqueries](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/11-subquery) | `NOT EXISTS` in place of an anti-join |
| [GROUP BY and HAVING](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/09-group-by-having) | Empty groups and clause processing order |
| [Query performance](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/20-query-performance) | Reading an execution plan to see how the join really runs |
...
### How can I quickly audit my queries for this bug?

Find every query containing LEFT JOIN and check whether the WHERE clause mentions the right-hand table's alias; if it does and it is not an IS NULL test, it is almost certainly a bug. Another check is to change LEFT JOIN to INNER JOIN and compare row counts: if the two numbers are identical, the word LEFT in your query is doing nothing at all.
