...
### Why does code calling .Result work on my dev machine but hang in production?
...
### ASP.NET Core has no SynchronizationContext, so is .Result fine there?

No. You will not hit a context deadlock, but every block parks a thread pool thread waiting on I/O. The pool adds threads very slowly once past its minimum, so under load requests arrive faster than it grows and the system falls into thread pool starvation: latency climbs, requests queue, and it looks exactly like a hang. The symptoms resemble a deadlock but the cause is different, and the fix is still async all the way.

### Does ConfigureAwait(false) make .Result safe?
