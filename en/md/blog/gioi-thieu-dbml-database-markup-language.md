...
> Nguồn: https://tiennhm.io.vn/en/blog/gioi-thieu-dbml-database-markup-language
...
DBML lo phần mô tả và phiên bản hoá lược đồ; còn những nguyên tắc quyết định lược đồ đó trông như thế nào — chuẩn hoá, khoá, quan hệ — nằm ở bài [thiết kế database](https://tiennhm.io.vn/docs/database/learn-sql-in-30-days/27-database-design-best-practices) trong series học SQL 30 ngày.
...
DBML (Database Markup Language) là một ngôn ngữ DSL mã nguồn mở được thiết kế để định nghĩa và tài liệu hóa các cấu trúc, lược đồ cơ sở dữ liệu. Cú pháp của DBML đơn giản, nhất quán và dễ đọc hơn so với việc viết SQL DDL truyền thống.

### DBML có phụ thuộc vào hệ quản trị cơ sở dữ liệu nào không?

Không. DBML không phụ thuộc vào bất kỳ hệ quản trị cơ sở dữ liệu cụ thể nào như MySQL, PostgreSQL hay SQL Server, nên có thể sử dụng linh hoạt trong nhiều môi trường khác nhau.

### Định nghĩa quan hệ giữa các bảng trong DBML như thế nào?

Dùng từ khóa Ref. Quan hệ một-nhiều viết dạng: Ref: posts.user_id > users.id. Quan hệ một-một dùng dấu gạch ngang: Ref: user_profiles.user_id - users.id. Quan hệ nhiều-nhiều được mô tả qua một bảng trung gian với hai Ref trỏ tới hai bảng gốc.

### Có những công cụ nào hỗ trợ DBML?

dbdiagram.io là công cụ trực tuyến miễn phí vẽ sơ đồ ER từ mã DBML và export sang PNG, PDF, SQL. dbdocs.io tạo tài liệu cơ sở dữ liệu tự động từ mã DBML. DBML CLI chuyển đổi giữa SQL và DBML, import từ database hiện có và tích hợp với CI/CD pipeline. Ngoài ra còn có extension cho Visual Studio Code với syntax highlighting và preview sơ đồ.

### Vì sao DBML phù hợp với việc quản lý phiên bản mã nguồn?

Vì DBML là text-based nên rất phù hợp với hệ thống quản lý phiên bản, giúp theo dõi thay đổi schema hiệu quả, dễ dàng track changes và rollback khi cần, đồng thời giữ cho tài liệu database luôn được cập nhật.
