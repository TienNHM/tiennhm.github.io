...
## The architecture: one proxy, one network, labels on each container
...
## Trap 1: SSL mode Flexible causes a redirect loop
...
## Trap 2: HTTP-01 and the circular dependency
ACME **HTTP-01** is the default validation method used by most Traefik tutorials. It works like this: Let's Encrypt requests `http:///.well-known/acme-challenge/` on **port 80**, and your server must return the expected content.
...
That `resolvers` line is small but worth having. Traefik has to wait for the TXT record to propagate before telling Let's Encrypt to check. If it asks the host's resolver and that resolver has just cached an NXDOMAIN for `_acme-challenge.`, the wait drags on pointlessly or fails outright. Asking `1.1.1.1` and `8.8.8.8` directly skips that cache layer.
...
## Trap 3: DNS-01 creates a new constraint
...
## Trap 4: without a catch-all router, Cloudflare returns 526
...
## Trap 5: every app sees Cloudflare's IP
...
## Operations: what I observed over nine days
...
**Checklist for putting Traefik behind Cloudflare**

- [ ] Set Cloudflare SSL/TLS to Full (strict) — never Flexible
- [ ] Use DNS-01, not HTTP-01, even if HTTP-01 currently works
- [ ] Cloudflare token: Zone Read + DNS Edit, scoped to the zone and the VPS IP
- [ ] Declare resolvers 1.1.1.1 and 8.8.8.8 for the DNS challenge
- [ ] Set exposedbydefault=false so databases and adminer are never routed by accident
- [ ] Add forwardedheaders.trustedips with Cloudflare's IP ranges
- [ ] Add a catch-all router with priority=1 and explicit tls.domains
- [ ] chmod 600 acme.json before the first start
- [x] Every new zone needs a token that can see it — check the logs after adding a domain

That last item is the one that cost me the most, and it is still sitting in my logs.

---

## Things I deliberately have not done
